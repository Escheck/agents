...
public class IAMhagglerBayesianModel extends OpponentModel {
	...
	public void init(NegotiationSession negotiationSession, Map<String, Double> parameters) {
		...
		this.utilitySpace = (AdditiveUtilitySpace) negotiationSession.getUtilitySpace();
	...
	private void initEvaluatorHypotheses() {
		...
		for (int i = 0; i < utilitySpace.getNrOfEvaluators(); ++i) {
			...
			case REAL: {
				...
				lHypEvalReal.addParam(0,
						-lHypEvalReal.getLowerBound() / (lHypEvalReal.getUpperBound() - lHypEvalReal.getLowerBound()));
				...
				for (int k = 1; k <= totalTriangularFunctions; ++k) {
					...
					double lMaxPoint = lHypEvalReal.getLowerBound()
							+ (double) k * (lHypEvalReal.getUpperBound() - lHypEvalReal.getLowerBound())
			...
			case INTEGER: {
				...
				lHypEvalInteger.setOffset(-lHypEvalInteger.getLowerBound()
						/ (lHypEvalInteger.getUpperBound() - lHypEvalInteger.getLowerBound()));
				...
				lHypEvalInteger.setUpperBound(lIssue.getUpperBound());
				lHypEvalInteger.setLowerBound(lIssue.getLowerBound());
				lHypEvalInteger.setOffset(1.0 + lHypEvalInteger.getLowerBound()
						/ (lHypEvalInteger.getUpperBound() - lHypEvalInteger.getLowerBound()));
				lHypEvalInteger.setSlope(-1.0 / (lHypEvalInteger.getUpperBound() - lHypEvalInteger.getLowerBound()));
				lEvaluatorHypothesis = new EvaluatorHypothesis(lHypEvalInteger);
				lEvaluatorHypothesis.setDesc("downhill");
						...
						for (int j = 0; j < lDiscIssue.getNumberOfValues(); ++j) {
							if (j < k) {
								...
								lDiscreteEval.addEvaluation(lDiscIssue.getValue(j),
										1000 * (lDiscIssue.getNumberOfValues() - j
												- 1 / (lDiscIssue.getNumberOfValues() - k - 1) + 1));
						}
						lEvaluatorHypothesis = new EvaluatorHypothesis(lDiscreteEval);
						lEvalHyps.add(lEvaluatorHypothesis);
	...
	public double getExtremeEvaluation(Evaluator evaluator, Extreme extreme) {
		...
		case INTEGER:
			...
			switch (extreme) {
			case MAX:
				extremeEval = Math.max(integerEvaluator.getEvaluation(integerEvaluator.getUpperBound()),
						integerEvaluator.getEvaluation(integerEvaluator.getLowerBound()));
				break;
			case MIN:
				extremeEval = Math.min(integerEvaluator.getEvaluation(integerEvaluator.getUpperBound()),
						integerEvaluator.getEvaluation(integerEvaluator.getLowerBound()));
				break;
			}
			break;
		case REAL:
			...
			switch (extreme) {
			case MAX:
				extremeEval = Math.max(realEvaluator.getEvaluation(realEvaluator.getUpperBound()),
						realEvaluator.getEvaluation(realEvaluator.getLowerBound()));
				...
				break;
			case MIN:
				extremeEval = Math.min(realEvaluator.getEvaluation(realEvaluator.getUpperBound()),
						realEvaluator.getEvaluation(realEvaluator.getLowerBound()));
				if (realEvaluator.getFuncType() == EVALFUNCTYPE.TRIANGULAR) {
	...
	private void updateEvaluationFunctions() throws Exception {
		...
		for (int i = 0; i < this.domain.getIssues().size(); i++) {
			...
			for (EvaluatorHypothesis evaluatorHypothesis : evaluatorHypotheses.get(i)) {
				utility = getPartialUtility(bid, i) + getExpectedWeight(i) * evaluatorHypothesis.getEvaluator()
						.getEvaluation(utilitySpace, bid, utilitySpace.getIssue(i).getNumber());
			...
			for (EvaluatorHypothesis evaluatorHypothesis : evaluatorHypotheses.get(i)) {
				utility = getPartialUtility(bid, i) + getExpectedWeight(i) * evaluatorHypothesis.getEvaluator()
						.getEvaluation(utilitySpace, bid, utilitySpace.getIssue(i).getNumber());
				evaluatorHypothesis.setProbability(evaluatorHypothesis.getProbability()
						* conditionalDistribution(utility, previousBidUtility) / n);
			}
		}
	...
	private void updateWeights() throws Exception {
		...
		for (int i = 0; i < domain.getIssues().size(); i++) {
			...
			for (WeightHypothesis weightHypothesis : weightHypotheses.get(i)) {
				utility = getPartialUtility(bid, i) + weightHypothesis.getWeight() * getExpectedEvaluationValue(bid, i);
				weightHypothesis.setProbability(
						weightHypothesis.getProbability() * conditionalDistribution(utility, previousBidUtility) / n);
			}
		}
	...
	private double getExpectedEvaluationValue(Bid bid, int number) throws Exception {
		double expectedEval = 0;
		for (EvaluatorHypothesis evaluatorHypothesis : evaluatorHypotheses.get(number)) {
			expectedEval += evaluatorHypothesis.getProbability() * evaluatorHypothesis.getEvaluator()
					.getEvaluation(utilitySpace, bid, utilitySpace.getIssue(number).getNumber());
		}
		return expectedEval;
	}
	...
	public Set<BOAparameter> getParameterSpec() {
		...
		set.add(new BOAparameter("u", 0.0, "Use all bids (instead of only unique) if u > 0.0"));
		set.add(new BOAparameter("b", 1.0, "Concession speed e of assumed opponent TDT function"));
		return set;
	}
}
