...
public class BidSpace {
	...
	public BidSpace(AdditiveUtilitySpace space) throws Exception {
		...
		if (discountFactor >= 1) {
			discountFactor = 0; // compatibility with ANAC2010 where discount =
								// 0 meant no discount
		}
	 ...
	 */
	public ArrayList<BidDetails> Project(double[] pointToProject,
			double utility, int limit, AdditiveUtilitySpace utilitySpace,
			negotiator.boaframework.OpponentModel opponentModel) {
			...
			try {
				if (!noModel) {
					jointUtility = utilitySpace.getUtility(b)
							+ opponentModel.getBidEvaluation(b);
				} else {
					jointUtility = utilitySpace.getUtility(b); // ignore
																// opponent util
				}
			} catch (Exception e) {
				e.printStackTrace();
	 ...
	 *            the bid to try to add.
	 */
	private void addUniqueBid(ArrayList<BidDetails> bids, Bid bid,
		...
		try {
			bids.add(new BidDetails(bid, space.getUtility(bid), -1));
		} catch (Exception e) {
			e.printStackTrace();
		}
	}
