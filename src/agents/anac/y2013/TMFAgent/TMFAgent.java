...
public class TMFAgent extends Agent {
		...
		if (s != null) {
			...
			for (Issue issue : issues) {
				...
				if (issue.getType() == ISSUETYPE.DISCRETE) {
					...
					int k = Math.min(10,
							((IssueInteger) issue).getUpperBound() - ((IssueInteger) issue).getLowerBound());
					for (int i = 0; i <= k; i++) {
		...
		if (lastBid == null) {
			...
			try {
				...
				while (relevantBids.size() > 0
						&& (utilitySpace.getUtility(relevantBids.get(relevantBids.size() - 1).bid) < minimumRelevant))
	...
	private void AddOpponentBidToModel(Bid b, boolean isAgreed) {
		...
		for (int i = 0; i < utilitySpace.getDomain().getIssues().size(); i++) {
			...
			Object v = null;
			Issue issue = (Issue) utilitySpace.getDomain().getIssues().get(i);
			Value v1 = null;
			try {
			...
			if (issue.getType() == ISSUETYPE.DISCRETE) {
				...
				int bucket = (int) Math.round((double) (currValue - ((IssueInteger) issue).getLowerBound())
						/ (((IssueInteger) issue).getUpperBound() - ((IssueInteger) issue).getLowerBound()) * k);
				v = bucket; // not a real value - it just the "bucket name"
			} else if (issue.getType() == ISSUETYPE.REAL) {
				// throw the value to the closest bucket
				double currValue = ((ValueReal) v1).getValue();
				int k = 10;
				int bucket = (int) Math.round((currValue - ((IssueInteger) issue).getLowerBound())
						/ (((IssueInteger) issue).getUpperBound() - ((IssueInteger) issue).getLowerBound()) * k);
	...
	public Action chooseAction() {
		...
		try {
			if (lastBid != null && utilitySpace.getUtility(lastBid) > cb.utility)
				/*
			...
			else if (cb.utility < utilitySpace.getReservationValueUndiscounted())
				/*
