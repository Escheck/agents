	...
	public Bid GetMaxBid() throws Exception {
		...
		for (int i = 0; i < utilitySpace.getDomain().getIssues().size(); i++) {
			...
			if (currIssue.getType().equals(ISSUETYPE.INTEGER)) {
				...
				if (maxUtil > minUtil) {
					max = max.putValue(currIssue.getNumber(), new ValueInteger(issueInteger.getUpperBound()));
				} else {
					max = max.putValue(currIssue.getNumber(), new ValueInteger(issueInteger.getLowerBound()));
				}

			} else if (currIssue.getType().equals(ISSUETYPE.REAL)) {
				IssueReal issueReal = (IssueReal) currIssue;
				tempBidding = tempBidding.putValue(currIssue.getNumber(), new ValueReal(issueReal.getUpperBound()));
				maxUtil = utilitySpace.getUtility(tempBidding);
				tempBidding = tempBidding.putValue(currIssue.getNumber(), new ValueReal(issueReal.getLowerBound()));
				double minUtil = utilitySpace.getUtility(tempBidding);
				if (maxUtil > minUtil) {
					max = max.putValue(currIssue.getNumber(), new ValueReal(issueReal.getUpperBound()));
				} else {
					max = max.putValue(currIssue.getNumber(), new ValueReal(issueReal.getLowerBound()));
				}
				...
				for (int j = 0; j < issueDiscrete.getNumberOfValues(); j++) {
					tempBidding = tempBidding.putValue(currIssue.getNumber(), issueDiscrete.getValue(j));
				...
				max = max.putValue(currIssue.getNumber(), issueDiscrete.getValue(indexOfMaximumValue));
			}
		}
		...
		if (actionOfPartner instanceof Offer) {
			...
			try {
				...
				if (this.maxBidRecieved == null) { // The first offer.
					...
					} else if (this.utilitySpace.getUtility(this.maxBidRecieved) < this.utilitySpace
							.getUtility(this.bestOffer)) {
						this.maxBidRecieved = this.bestOffer;
					}
				} else if (utility > this.utilitySpace.getUtility(this.maxBidRecieved)) { // Not
																							// the
																							// first
																							// offer.
					this.maxBidRecieved = bid;
				...
				} else // Exploitation
				{
					...
					try {
						...
						if (utility >= this.MIN_UTILITY_ACCEPT
								&& utility >= this.utilitySpace.getUtility(this.maxBidRecieved)) {
							action = new Accept(this.getAgentID(), ((ActionWithBid) actionOfPartner).getBid());
