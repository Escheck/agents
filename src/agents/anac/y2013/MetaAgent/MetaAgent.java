...
public class MetaAgent extends Agent {
	...
	public void init() {
		...
		if (s != null) {
			...
			myAgent.internalInit(this.sessionNr, this.sessionsTotal, this.startTime, this.totalTime, this.timeline,
	...
	private void getDomainParams() {
		...
		Iterator<Entry<Objective, Evaluator>> issue = ((AdditiveUtilitySpace) utilitySpace).getEvaluators().iterator();
		...
		while (issue.hasNext()) { // every issue
			...
			if (e.getType() == EVALUATORTYPE.DISCRETE) {
				...
				tempEU = ((double) (((EvaluatorInteger) e).getUpperBound() + ((EvaluatorInteger) e).getLowerBound()))
						/ 2;
				tempStdevU = Math.sqrt((Math.pow(((EvaluatorInteger) e).getUpperBound() - tempEU, 2)
						+ Math.pow(((EvaluatorInteger) e).getLowerBound() - tempEU, 2)) / 2);
			} else if (e.getType() == EVALUATORTYPE.REAL) {
				tempEU = (((EvaluatorReal) e).getUpperBound() + ((EvaluatorReal) e).getLowerBound()) / 2;
				tempStdevU = Math.sqrt((Math.pow(((EvaluatorReal) e).getUpperBound() - tempEU, 2)
						+ Math.pow(((EvaluatorReal) e).getLowerBound() - tempEU, 2)) / 2);
			} else {
				tempEU = 0.5;
				tempStdevU = 0;
			}
		...
		double avgSize = ((double) sumSize) / ((double) ((AdditiveUtilitySpace) utilitySpace).getEvaluators().size());
		...
		double relsumUtility = 0, relcountUtility = 0, relstdevUtility = 0, stdevUtil = 0, countbids = 0, sumbids = 0,
				relevantStdevU = 0;
		...
		features.put("DiscountFactor",
				this.utilitySpace.getDiscountFactor() == 0 ? 1 : this.utilitySpace.getDiscountFactor()); // DiscountFactor
		...
		try {
			if (!agentInit) {
												...
												// initialized any agent)
				{
					...
					return new Offer(this.getAgentID(), utilitySpace.getMaxUtilityBid());
						...
						// initiator)
				{
					...
					getDomainParams();
					UpdateAllAgents();
					manager.SetAvgUtil(features.get("RelevantEU") * Math.pow(utilitySpace.getDiscountFactor(), 0.5),
							features.get("RelevantStdevU"));
					myAgent = manager.SelectBestAgent();
					myAgent.internalInit(this.sessionNr, this.sessionsTotal, this.startTime, this.totalTime,
							this.timeline, this.utilitySpace, this.parametervalues);
					myAgent.setName(this.getName());
					myAgent.setAgentID(this.getAgentID());
					myAgent.init();
			...
			if (a == null) {
				...
				if (a instanceof Offer && this.utilitySpace.getReservationValueWithDiscount(time) >= utilitySpace
						.getUtilityWithDiscount(((Offer) a).getBid(), time)) {
	...
	private ArrayList<Bid> GetDiscreteBids() {
		...
		for (Issue issue : utilitySpace.getDomain().getIssues()) { // for every
			...
			if (issue.getType() == ISSUETYPE.DISCRETE) {
				ArrayList<ValueDiscrete> valuesD = (ArrayList<ValueDiscrete>) ((IssueDiscrete) issue).getValues(); // get
																													// list
																													// of
																													// options/values
																													// for
			...
			for (Bid bid : bids) { // for each bid seen so far (init bids list)
				for (Value value : issueValues) { // for every value
					...
					for (Issue issue1 : utilitySpace.getDomain().getIssues())
						// go over all issues
						try {
							bidValues.put(issue1.getNumber(), bid.getValue(issue1.getNumber())); // each
																									// issue
																									// is
																									// entered
						} catch (Exception e) {
	...
	private Value GetRepresentorOfBucket(int bucket, Issue issue, int k, boolean isInteger) {
		...
		if (isInteger) {
			...
			boolean upperIsTheBest = ei.getEvaluation(((IssueInteger) issue).getUpperBound()) > ei
					.getEvaluation(((IssueInteger) issue).getLowerBound());
			if (upperIsTheBest) {
				if (bucket < k) {
					ans = ((double) (bucket + 1)) / k;
					ans = ans * (((IssueInteger) issue).getUpperBound() - ((IssueInteger) issue).getLowerBound())
		...
		boolean upperIsTheBest = ei.getEvaluation(((IssueReal) issue).getUpperBound()) > ei
				.getEvaluation(((IssueReal) issue).getLowerBound());
		if (upperIsTheBest) {
			if (bucket < k) {
				ans = ((double) (bucket + 1)) / k;
				ans = ans * (((IssueReal) issue).getUpperBound() - ((IssueReal) issue).getLowerBound())
