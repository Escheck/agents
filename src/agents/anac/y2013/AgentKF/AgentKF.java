...
public class AgentKF extends Agent {
		...
		try {
			...
			if (partner instanceof Offer) {
				...
				if (timeline.getCurrentTime() > 0.85) {
					BidHistory FinalBidHistory = currSessOppBidHistory.filterBetweenTime(timeline.getCurrentTime(),
							1.0);
		...
		for (int i = 0; i < AltList.size(); i++) {
			...
			for (Issue lIssue : issues) {
				...
				case DISCRETE:
					...
					double weight_d = ((AdditiveUtilitySpace) utilitySpace).getWeight(lIssueDiscrete.getNumber());
					if (theBid.getValue(lIssueDiscrete.getNumber()) == targetBid.getValue(lIssueDiscrete.getNumber()))
						Value += 1.0 * weight_d;
					break;
				case REAL:
					IssueReal lIssueReal = (IssueReal) lIssue;
					double weight_r = ((AdditiveUtilitySpace) utilitySpace).getWeight(lIssueReal.getNumber());
					if (theBid.getValue(lIssueReal.getNumber()) == targetBid.getValue(lIssueReal.getNumber()))
						Value += 1.0 * weight_r;
					break;
				case INTEGER:
					IssueInteger lIssueInteger = (IssueInteger) lIssue;
					double weight_i = ((AdditiveUtilitySpace) utilitySpace).getWeight(lIssueInteger.getNumber());
	...
	private double adjustTremor(double time) {
		if (currSessOppBidHistory.isEmpty()) {
			...
			double histry_avg = currSessOppBidHistory.filterBetweenTime(0.0, timeline.getCurrentTime())
					.getAverageUtility();
			if (avg > histry_avg) {
				return 0.3;
			} else {
	...
	public String getDescription() {
		return "ANAC 2013 - AgentKF (compatible with non-linear utility spaces)";
	}
