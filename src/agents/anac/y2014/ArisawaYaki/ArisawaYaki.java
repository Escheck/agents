...
public class ArisawaYaki extends Agent {
	...
	public void ReceiveMessage(Action opponentAction) {
		...
		if (opponentAction != null) {
			...
			currBidHistory.add(new BidDetails(opponentLastBid, getUtility(opponentLastBid), timeline.getTime()));
	...
	public Action chooseAction() {
		...
		if (opponentLastBid != null && getUtility(opponentLastBid) >= acceptUtility) {
	...
	private Bid getBidBySA(double target) throws Exception {
		double t = 1000000;
		double cool = 0.999;
		List<Issue> issues = utilitySpace.getDomain().getRandomBid(null).getIssues();
	...
	public String getDescription() {
		return "ANAC 2014 - ArisawaYaki (compatible with non-linear utility spaces)";
	}
}
