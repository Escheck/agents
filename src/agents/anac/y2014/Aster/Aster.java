...
public class Aster extends Agent {
	...
	public void ReceiveMessage(Action opponentAction) {
		try {
			if (opponentAction instanceof Offer) {
				...
				// ç›¸æ‰‹ã�®Bidã‚’ä¿�å­˜
				opponentBidHistory.add(convertBidDetails(opponentLastBid, time));
			}
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	public Action chooseAction() {
		...
		try {
			...
			double maxUtil = opponentMaxUtility > opponentPrevMaxUtility ? opponentMaxUtility : opponentPrevMaxUtility;
			acceptP = getAcceptProb.getAcceptProbability(opponentLastBid, maxUtil, time);
			...
			// BidæŽ¢ç´¢
			// ç›¸æ‰‹ã�®Bidã�®è¿‘å‚�æŽ¢ç´¢
			selectOpponentBidList = searchBid.searchOfferingBid(selectOpponentBidList, opponentLastBid, bidTarget);
			...
			if (!selectOpponentBidList.isEmpty()) {
				actionBid = chooseAction.nextOfferingBid(bidTarget, selectMyBidList, selectOpponentBidList);
			} else {
	...
	public void endSession(NegotiationResult result) {
		...
		try {
			...
			opponentPrevMinUtility = opponentBidHistory.getWorstBidDetails().getMyUndiscountedUtil();

			mySessionData = new MyPrevSessionData(agreementBidList, opponentBidHistory, result, opponentPrevMaxUtility,
	...
	private void beginSession() {
		...
		try {
			...
			double maxConcessionUtility = (1.0
					- ((opponentMaxUtility - opponentPrevMinUtility) / (1.0 - opponentPrevMinUtility))) * 100;
			...
			if (!agreementBidList.isEmpty()) {
				...
				for (Bid bid : agreementBidList) {
					selectAgreementBidList = searchBid.searchOfferingBid(selectAgreementBidList, bid, target);
				}
			}

			// è­²æ­©è¨­å®š
			getAcceptProb.setTargetParam(this.sessionNr, maxConcessionUtility, opponentMinUtility);
	...
	private Bid isConsiderable() {
		Bid actionBid = null;
		double concessionDeg = (opponentMaxUtility - opponentMinUtility) / (1.0 - opponentMinUtility);
		...
		} else if ((immediateDecision) && (opponentMaxUtility >= opponentPrevMaxUtility)) {
			actionBid = opponentMaxBid;
	...
	private boolean isAcceptable(double t) throws Exception {
		...
		double minUtil = myBidHistory.getWorstBidDetails().getMyUndiscountedUtil();
	...
	public String getDescription() {
		return "ANAC 2014 - Aster (compatible with non-linear utility spaces)";
	}
}
