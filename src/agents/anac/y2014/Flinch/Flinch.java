...
public class Flinch extends Agent {
		...
		try {
			...
			if (actionOfPartner == null) {
				...
				if (actionOfPartner instanceof Offer) {
					...
					if (bestBidFromOpponent == null
							|| offeredUtilFromOpponent > utilitySpace.getUtility(bestBidFromOpponent)) {
	...
	private double distance(Bid b1, Bid b2) throws Exception {
		...
		for (int i = 0; i < issues.size(); i++) {
			...
			default:
				throw new Exception("issue type " + issue.getType() + " not supported by Flinch");
			}
	...
	public Bid getRandomBid() throws Exception {
		...
		for (Issue lIssue : issues) {
			...
			default:
				throw new Exception("issue type " + lIssue.getType() + " not supported by Flinch LocalSearch");
			}
		}
	...
	private Bid GA_select(Bid[] population, double[] score, double N) throws Exception { // currently
																							// using
																							// roulette
	...
	private Bid GA_mutate(Bid b) throws Exception {
		...
		case REAL: {
			...
			int optionInd = randomnr.nextInt(lIssueReal.getNumberOfDiscretizationSteps() - 1);
			v = new ValueReal(lIssueReal.getLowerBound() + (lIssueReal.getUpperBound() - lIssueReal.getLowerBound())
		...
		default:
			throw new Exception("issue type " + issues.get(k).getType() + " not supported by Flinch LocalSearch");
		}
		assert v != null;
	...
	public String getDescription() {
		return "ANAC 2014 - Flinch (compatible with non-linear utility spaces)";
	}
}
