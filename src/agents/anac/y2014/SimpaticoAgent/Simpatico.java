...
public class Simpatico extends Agent {
		...
		try {
			for (int i = 0; i < issuesToChange.length; i++) {
				...
				case INTEGER:
					...
					break;
				default:
					throw new Exception("issue type not supported by " + getName());
				}
			}
		} catch (Exception e) {
			debug(e.getMessage());
	...
	private Action chooseRandomBidAction() {
		...
		try {
			...
			debug("Problem with received bid:" + e.getMessage() + ". cancelling bidding");
		}
	...
	private Bid getRandomBid() throws Exception {
		...
		debug("R 2.Generated a random bid. current utility: " + getUtility(bid) + ", tolerance: " + minimumUtilTolerance
				+ "; tries: " + absoluteTries);
	...
	public String getDescription() {
		return "ANAC 2014 - Simpatico (compatible with non-linear utility spaces)";
	}
