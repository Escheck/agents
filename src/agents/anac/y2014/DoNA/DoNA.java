...
public class DoNA extends Agent {
	...
	public void init() {
		...
		midReservation = (utilitySpace.getReservationValue() < 0.8 && utilitySpace.getReservationValue() > 0.2);// (minUtility+0.75*UtilityRange)
																												// &&
																												// utilitySpace.getReservationValue()
																												// >=
																												// (minUtility+0.375*UtilityRange));
	...
	public Action chooseAction() {
		...
		if (actionOfPartner != null) {
			Bid Nbid = getBestNigberBid(DefaultAction.getBidFromAction(actionOfPartner), 0);
		...
		if (highDiscount) {
			...
			try {
				...
				lastAction = cleanOffer(hashBids.get(arrSamples.get(countOffer)), lastBidOfOpponentUtil); // best
																											// guess
																											// if
																											// things
																											// go
			...
			Bid bid;
			if (currentTime >= 1 - FactorOfAverageResponseTime * (averageResponseTime + 2 * stiaResponseTime)) { // last
																													// last
																													// moment
			...
			} else if (currentTime >= 1 - Math.pow(2, 1) * FactorOfAverageResponseTime * averageResponseTime) { // one
																												// before
																												// last
																												// moment
				if (isPrinting)
				...
				if (isPrinting)
					System.out.println("currentTime: " + currentTime + " / "
							+ (1 - Math.pow(2, 1) * FactorOfAverageResponseTime * averageResponseTime));
				if (utilitySpace.getReservationValue() > utilOfBestBidOfOppenent
					...
					if (isPrinting)
						System.out.println("Accept3 Util: " + utilOflastBidOfOpponent);
					if (R > utilitySpace.getUtilityWithDiscount(lastBidOfOpponent, timeline.getTime()))
						return cleanEndNegotiation();
					...
					return new Offer(getAgentID(), this.opponentBidHistory.getBestBidInHistory());
				}
			}
			if (isPrinting)
		...
		if (currentTime >= 1 - FactorOfAverageResponseTime * (averageResponseTime + 2 * stiaResponseTime)) { // last
			...
			if (utilitySpace.getReservationValue() > utilOfBestBidOfOppenent
				...
				if (isPrinting)
					System.out.println("Accept5 Util: " + utilOflastBidOfOpponent);
				if (R > utilitySpace.getUtilityWithDiscount(lastBidOfOpponent, timeline.getTime()))
					return cleanEndNegotiation();
		...
		return FastLastMomentStrategyWithoutEndNegotiation(dblMinUtil - UtilityFactor + 0.25 * stia,
	...
	private Action EndNegotiationOrAcceptOrNewOfferr(Bid myOfferedBid) throws Exception {
		...
		// 3rd priority - bestOpponentBidUtility is the best choice
			...
			if (isPrinting)
				System.out.println("new Offer(this.opponentBidHistory.getBestBidInHistory())");
			return new Offer(getAgentID(), this.opponentBidHistory.getBestBidInHistory());
		}
		...
		// 4th priority - myOfferedBid
			...
			MyOfferedUtility = utilitySpace.getUtility(DefaultAction.getBidFromAction(lastAction));
			if (isPrinting)
				System.out.println("else MyOfferedUtility 333: " + MyOfferedUtility);
			return lastAction;
		}
	}
	...
	private Action chooseRandomBidAction(double minUtil) {
		...
		if (nextBid == null)
			return cleanOffer(hashBids.get(arrSamples.get(arrSamples.size() - 1)), lastBidOfOpponentUtil);
		if (lastBidOfOpponent == null)
			return new Offer(getAgentID(), nextBid);
		else {
			if (isPrinting)
				System.out.println("chooseRandomBidAction-lastBidOfOpponentUtil: " + lastBidOfOpponentUtil);
		...
		for (Issue lIssue : issues) {
			...
			case DISCRETE:
				...
				for (int optionIndex = 0; optionIndex < numOfValues; optionIndex++) {
					...
					newBid = currentBid;
					newBid = newBid.putValue(lIssue.getNumber(), lIssueDiscrete.getValue(optionIndex));
					if (deep > 0) {
			...
			case INTEGER:
				...
				for (int optionIndex2 = lIssueInteger.getLowerBound(); optionIndex2 < numOfValues; optionIndex2++) {
					newBid = currentBid;
					newBid = newBid.putValue(lIssue.getNumber(), new ValueInteger(optionIndex2));
					if (deep > 0) {
						newBid = getBestNigberBid(newBid, deep - 1);
					}
					newUtil = getUtility(newBid);
	...
	public Action FastStrategyWithoutEndNegotiation(double VirtulReservation) {
		...
		try {
			...
			if ((lastBidOfOpponentUtil > MyOfferedUtility || lastBidOfOpponentUtil >= VirtulReservation)
				...
				if (isPrinting)
					System.out.println("Accept7 Util: " + lastBidOfOpponentUtil);
				if (R > utilitySpace.getUtilityWithDiscount(lastBidOfOpponent, timeline.getTime()))
					return cleanEndNegotiation();
			...
			else if (bestOpponentBidUtility > MyOfferedUtility || bestOpponentBidUtility >= VirtulReservation) {
				if (isPrinting)
					System.out.println("bestOpponentBidUtility: " + bestOpponentBidUtility + " MyOfferedUtility: "
							+ MyOfferedUtility + " VirtulReservation: " + VirtulReservation);
				return new Offer(getAgentID(), this.opponentBidHistory.getBestBidInHistory());
			}
			// Reservation is the best choice
				...
				if (flagStopExplor == false && dbltime < 0.90) {
					...
					if (otherArrSamples.size() > 0) {
						...
						if (curBid == null) {
							...
							if (isPrinting)
								System.out.println(
										"countOffer: " + countOffer + " tmpDif: " + tmpDif + " randNum: " + randNum);
				...
				MyOfferedUtility = utilitySpace.getUtility(DefaultAction.getBidFromAction(lastAction));
				if (isPrinting)
					System.out.println("MyOfferedUtility 111: " + MyOfferedUtility);
				...
				MyOfferedUtility = utilitySpace.getUtility(DefaultAction.getBidFromAction(lastAction));
				if (isPrinting)
					System.out.println("MyOfferedUtility 222: " + MyOfferedUtility);
	...
	public Action FastLastMomentStrategyWithoutEndNegotiation(double StartMinUtil, double MinUtil,
		...
		int FactorOfAverageResponseTime = 1;
		if (dblMyRoundTime >= 1 - constForNumOfRounds * FactorOfAverageResponseTime * averageResponseTime) { // Other
																												// before-last
																												// moments
																												// -
																												// logarithmic
	...
	private Action cleanEndNegotiation() {
		...
		if (randIndex == countOffer)
			return new Offer(getAgentID(), hashBids.get(arrSamples.get(randIndex)));
		else
	...
	public String getDescription() {
		return "ANAC 2014 - DoNA (compatible with non-linear utility spaces)";
	}
}
