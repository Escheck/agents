...
public class Groupn extends AbstractNegotiationParty {
		...
		try {
			...
			if (OwnBids.numOfBidsProposed() == 0) {
				...
				if (ActionOfOpponent instanceof Offer) {
					...
					System.out.println(((Offer) ActionOfOpponent).getBid() + "Opponent's Offer");
					Boolean IsAccept = AcceptOffer(((Offer) ActionOfOpponent).getBid(), bid);
					...
					if (IsAccept && !IsTerminate) {
						action = new Accept(getPartyId(), ((Offer) ActionOfOpponent).getBid());
	...
	private boolean AcceptOffer(Bid opponentBid, Bid ownBid) {
		...
		if (currentUtility >= this.utilitythreshold || currentUtility >= nextRoundUtility) {
			...
			double predictMaxUtility = maximumThresholdUtility * this.discountFactor;
			System.out.println(predictMaxUtility);
			System.out.println("SCENARIO 2a");
			double bestMaxUtility = this.utilitySpace.getUtilityWithDiscount(bidOptions.getBestBidInHistory(),
					timeline);
			...
			if (bestMaxUtility > predictMaxUtility) {
				try {
					...
					if (utilitySpace
							.getUtility(opponentBid) >= utilitySpace.getUtility(bidOptions.getBestBidInHistory())
									- 0.1 /* 0.01 */) {
						return true;
					} else {
			...
			} else if (bestMaxUtility > this.utilitythreshold * Math.pow(this.discountFactor, timeline.getTime())) {
				try {
					// if the current offer is approximately as good as the best
					// one in the history, then accept it.
					if (utilitySpace
	...
	private Bid AgentOffer() {
		...
		List<Bid> candidateBids = this.getBidsInRange(this.MaxUtility - concession1, this.MaxUtility - concession2);

		System.out.println(value);
		switch (value) {

		case 1:
			bid = bidOptions.ChooseBid1(candidateBids, this.utilitySpace.getDomain());
			break;
		case 2:
			bid = bidOptions.ChooseBid2(candidateBids, this.utilitySpace.getDomain());
			break;
		case 3:
			bid = bidOptions.ChooseBid3(candidateBids, this.utilitySpace.getDomain());
			break;
		}
		return bid;
	}
	...
	private void calculateBidsBetweenUtility() {
		...
		try {
			...
			this.bidsBetweenUtility.get(maximumRounds - 1).add(this.Maximum_Utility_Bid);
	...
	private boolean TerminateNegotiation(Bid ownBid) {
		...
		try {
			...
			System.out.println(e.getMessage() + "Exception in method TerminateNegotiation part 1");
		}
		...
		if (currentUtility >= this.utilitythreshold || currentUtility >= nextRoundUtility) {
			...
			double currentMaxUtility = this.utilitySpace.getReservationValueWithDiscount(timeline);
	...
	public void receiveMessage(AgentID sender, Action action) {
		...
		if (ActionOfOpponent instanceof Offer) { // if Agent is not the first to
													...
													// offer first
			this.bidOptions.addOpponentBid(((Offer) ActionOfOpponent).getBid(), utilitySpace.getDomain(),
					this.utilitySpace, IDOfOpponent);
