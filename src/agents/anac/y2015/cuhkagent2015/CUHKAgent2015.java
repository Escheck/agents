...
public class CUHKAgent2015 extends AbstractNegotiationParty {
	...
	public Action chooseAction(List<Class<? extends Action>> possibleActions) {
		...
		try {
			...
			if ((!possibleActions.contains(Accept.class)) || (this.oppFirstBid_Agent1 == null)
					|| (this.oppFirstBid_Agent2 == null)) {
			...
			// else if (ActionOfOpponent instanceof Offer)
				...
				this.opponentBidHistory1.updateOpponentModel(this.oppPreviousBid_Agent1, utilitySpace.getDomain(),
						(AdditiveUtilitySpace) this.utilitySpace);
				this.opponentBidHistory2.updateOpponentModel(this.oppPreviousBid_Agent2, utilitySpace.getDomain(),
						(AdditiveUtilitySpace) this.utilitySpace);
						...
						if (timeline.getTime() > 0.9985 && estimateRoundLeft(true) < 5) {
							...
							if (IsAccept && !IsTerminate) {
								...
								if (this.toughAgent == true) {
									...
									action = new Offer(getPartyId(), bid);
									// this.toughAgent = true;
									System.out.println("this is really the last chance" + bid.toString()
											+ " with utility of " + utilitySpace.getUtility(bid));
								}
							}
			...
			if (debug) {
				...
				System.out.println(Double.toString(this.timeline.getTime()) + ","
						+ Double.toString(this.AvgUtilitythreshold) + "," + Double.toString(this.minUtilityUhreshold));
			...
			if (timeline.getTime() <= AvgConcedeTime) {
				if (this.discountingFactor <= 0.5) {
					this.minThreshold_Agent1 = (maximumOfBid * this.discountingFactor)
							/ Math.pow(this.discountingFactor, this.concedeTime_Agent1);
					this.minThreshold_Agent2 = (maximumOfBid * this.discountingFactor)
							/ Math.pow(this.discountingFactor, this.concedeTime_Agent2);
					this.utilitythreshold_Agent1 = maximumOfBid - (maximumOfBid - this.minThreshold_Agent1)
							* Math.pow((timeline.getTime() / this.concedeTime_Agent1), alpha1);
					this.utilitythreshold_Agent2 = maximumOfBid - (maximumOfBid - this.minThreshold_Agent2)
							* Math.pow((timeline.getTime() / this.concedeTime_Agent2), alpha1);
				} else {
					this.utilitythreshold_Agent1 = this.minThreshold_Agent1 + (maximumOfBid - this.minThreshold_Agent1)
							* (1 - Math.sin((Math.PI / 2) * (timeline.getTime() / this.concedeTime_Agent1)));
					this.utilitythreshold_Agent2 = this.minThreshold_Agent2 + (maximumOfBid - this.minThreshold_Agent2)
							* (1 - Math.sin((Math.PI / 2) * (timeline.getTime() / this.concedeTime_Agent2)));
				}
			} else {
				if (this.discountingFactor <= 0.5) {
					...
					this.utilitythreshold_Agent1 = this.minThreshold_Agent1
							+ (maximumOfBid - this.minThreshold_Agent1) / (1 - concedeTime_Agent1)
									* Math.pow((timeline.getTime() - this.concedeTime_Agent1), this.discountingFactor);
					this.utilitythreshold_Agent2 = this.minThreshold_Agent2
							+ (maximumOfBid - this.minThreshold_Agent2) / (1 - concedeTime_Agent2)
									* Math.pow((timeline.getTime() - this.concedeTime_Agent2), this.discountingFactor);
				}
			}
			...
			while (!accept) {
				...
				if (count < 2) {
					for (double temputil = this.AvgUtilitythreshold; temputil <= this.MaximumUtility; temputil += 0.01) {
						...
						double pchoose1 = (1 / this.relstdevUtility_Agent1 * (1 / Math.sqrt(2 * Math.PI)))
								* Math.exp(-Math.pow(temputil - this.relavgUtility_Agent1, 2)
										/ (2 * Math.pow(this.relstdevUtility_Agent1, 2)));
						double pchoose2 = (1 / this.relstdevUtility_Agent2 * (1 / Math.sqrt(2 * Math.PI)))
								* Math.exp(-Math.pow(temputil - this.relavgUtility_Agent2, 2)
										/ (2 * Math.pow(this.relstdevUtility_Agent2, 2)));
			...
			if (currentUtility >= this.AvgUtilitythreshold || currentUtility >= nextRoundUtility) {
				...
				if (currentMaximumUtility > predictMaximumUtility && timeline.getTime() > this.AvgConcedeTime) {
					try {
						...
						if (utilitySpace.getUtilityWithDiscount(opponentBid, timeline) >= currentMaximumUtility
								- 0.01) {
							System.out.println("he offered me " + currentMaximumUtility
					...
					try {
						...
						if (utilitySpace.getUtilityWithDiscount(opponentBid, timeline) >= currentMaximumUtility
							...
							System.out
									.println("test" + utilitySpace.getUtility(opponentBid) + this.AvgUtilitythreshold);
							this.concedeToOpponent = true;
							return false;
						}
					} catch (Exception e) {
	...
	private void calculateBidsBetweenUtility() {
		...
		try {
			...
			this.bidsBetweenUtility.get(this.NumberofDivisions - 1).add(counts[this.NumberofDivisions - 1],
				...
				for (int i = 0; i < this.NumberofDivisions; i++) {
					if (this.utilitySpace.getUtility(b) <= (i + 1) * this.SpacebetweenDivisions + this.MinimumUtility
							&& this.utilitySpace.getUtility(b) >= i * this.SpacebetweenDivisions
									+ Math.min(this.MinimumUtility_Agent1, this.MinimumUtility_Agent2)) {
	...
	private void chooseConcedeToDiscountingDegree() {
		try {
			...
			System.out.println("Exception in method chooseConcedeToDiscountingDegree");
		}
	}
	...
	private void updateConcedeDegree_smallDiscountingFactor() {
		...
		try {
			...
			System.out.println("Exception in method updateConcedeDegree_smallDiscountingFactor");
			this.except = 20;
		}
		// System.out.println("concedeToDiscountingFactor is " +
	...
	private void updateConcedeDegree_largeDiscountingFactor_Agent1() {
		try {
			...
			this.concedePartOfOppBehaviour_Agent1 = (((((this.relcountLowerBoundMid2_Agent1
					/ this.relcountUpperBoundMid2_Agent1) * this.slopeOfSlopeOfSessionMax2_Agent1)
					- ((this.relcountLowerBoundMid1_Agent1 / this.relcountUpperBoundMid1_Agent1)
			...
			this.except = 23;
			System.out.println("updateConcedeDegree_largeDiscountingFactor_Agent1 exception");
		}
	}
	...
	private void updateConcedeDegree_largeDiscountingFactor_Agent2() {
		try {
			...
			this.concedePartOfOppBehaviour_Agent2 = (((((this.relcountLowerBoundMid2_Agent2
					/ this.relcountUpperBoundMid2_Agent2) * this.slopeOfSlopeOfSessionMax2_Agent2)
					- ((this.relcountLowerBoundMid1_Agent2 / this.relcountUpperBoundMid1_Agent2)
			...
			this.except = 23;
			System.out.println("updateConcedeDegree_largeDiscountingFactor_Agent2 exception");
		}
	}
	...
	private void updateConcedeDegree_nonDiscountingFactor_Agent1() {
		try {
			...
			this.except = 25;
			System.out.println("updateConcedeDegree_nonDiscountingFactor_Agent1 exception");
		}
	}
	...
	private void updateConcedeDegree_nonDiscountingFactor_Agent2() {
		try {
			...
			this.except = 25;
			System.out.println("updateConcedeDegree_nonDiscountingFactor_Agent2 exception");
		}
	}
		...
		try {
			...
			if (this.relstdevUtility_Agent1 < 0.1) {
				...
				for (Iterator g = bidsUtil.iterator(); g.hasNext();) {
					...
					if (bidUtil >= this.oppFirstBidUtility_Agent1
							&& (bidUtil < (this.relavgUtility_Agent1 - this.relstdevUtility_Agent1)
									|| bidUtil > (this.relavgUtility_Agent1 + this.relstdevUtility_Agent1))) {
						this.relSqRootOfAvgUtility_Agent1 += (bidUtil - this.relavgUtility_Agent1)
								* (bidUtil - this.relavgUtility_Agent1);
				...
				this.relstdevUtility_Agent1 = Math
						.sqrt(this.relSqRootOfAvgUtility_Agent1 / this.relcountUtilityInSmallSD_Agent1);
			}

			this.minThreshold_Agent1 = this.relavgUtility_Agent1
					+ this.discountingFactor * this.relstdevUtility_Agent1 * this.reservationValue;
			...
			System.out.println(e.getMessage() + "exception in method CalculateMinThreshold_Agent1");
		}
	}
		...
		try {
			...
			if (this.relstdevUtility_Agent2 < 0.1) {
				...
				for (Iterator g = bidsUtil.iterator(); g.hasNext();) {
					...
					if (bidUtil >= this.oppFirstBidUtility_Agent2
							&& (bidUtil < (this.relavgUtility_Agent2 - this.relstdevUtility_Agent2)
									|| bidUtil > (this.relavgUtility_Agent2 + this.relstdevUtility_Agent2))) {
						this.relSqRootOfAvgUtility_Agent2 += (bidUtil - this.relavgUtility_Agent2)
								* (bidUtil - this.relavgUtility_Agent2);
				...
				this.relstdevUtility_Agent2 = Math
						.sqrt(this.relSqRootOfAvgUtility_Agent2 / this.relcountUtilityInSmallSD_Agent2);
			}

			this.minThreshold_Agent2 = this.relavgUtility_Agent2
					+ this.discountingFactor * this.relstdevUtility_Agent2 * this.reservationValue;
			...
			System.out.println(e.getMessage() + "exception in method CalculateMinThreshold_Agent2");
		}
	}
	...
	private ArrayList<Double> CalculateSumAndCount_Agent1(ArrayList<Double> bidsUtil) {
		try {
			this.oppFirstBidUtility_Agent1 = utilitySpace.getUtility(this.oppFirstBid_Agent1);
			if (this.reservationValue <= this.oppFirstBidUtility_Agent1) {
				this.MinimumUtility_Agent1 = this.oppFirstBidUtility_Agent1;
			...
			for (; myBidIterator.hasNext();) {
				try {
					...
					System.out.println(e.getMessage() + "exception in method CalculateSumAndCount_Agent1");
					return null;
				}
			}
	...
	private ArrayList<Double> CalculateSumAndCount_Agent2(ArrayList<Double> bidsUtil) {
		try {
			this.oppFirstBidUtility_Agent2 = utilitySpace.getUtility(this.oppFirstBid_Agent2);
			if (this.reservationValue <= this.oppFirstBidUtility_Agent2) {
				this.MinimumUtility_Agent2 = this.oppFirstBidUtility_Agent2;
			...
			System.out.println(e.getMessage() + "exception in method CalculateSumAndCount_Agent2");
			return null;
		}
	}
	...
	private void MeasureConcedePartOfOppBehaviour_Agent1() {
		try {
			...
			if (count_Agent1 < this.FirstTimeInterval) {
				if (utilitySpace
						.getUtility(this.oppPreviousBid_Agent1) > this.utility_maximum_from_opponent_Session1_Agent1) {
				...
				if (utilitySpace
						.getUtility(this.oppPreviousBid_Agent1) > this.utility_maximum_from_opponent_Session2_Agent1) {
					this.utility_maximum_from_opponent_Session2_Agent1 = utilitySpace
							.getUtility(this.oppPreviousBid_Agent1);
				}
			} else if (count_Agent2 < this.ThirdTimeInterval) {
				if (utilitySpace
						.getUtility(this.oppPreviousBid_Agent1) > this.utility_maximum_from_opponent_Session3_Agent1) {
				...
				this.midPointOfSlopeSessionMax1_Agent1 = (this.utility_maximum_from_opponent_Session2_Agent1
						+ this.utility_maximum_from_opponent_Session1_Agent1) / 2;
				this.midPointOfSlopeSessionMax2_Agent1 = (this.utility_maximum_from_opponent_Session3_Agent1
						+ this.utility_maximum_from_opponent_Session2_Agent1) / 2;
			...
			System.out.println(e.getMessage() + "exception in method MeasureConcedePartOfOppBehaviour_Agent1");
		}
	}
	...
	private void MeasureConcedePartOfOppBehaviour_Agent2() {
		try {
			...
			if (count_Agent2 < this.FirstTimeInterval) {
				if (utilitySpace
						.getUtility(this.oppPreviousBid_Agent2) > this.utility_maximum_from_opponent_Session1_Agent2) {
					this.utility_maximum_from_opponent_Session1_Agent2 = utilitySpace
							.getUtility(this.oppPreviousBid_Agent2);
				}
			} else if (count_Agent2 < this.SecondTimeInterval) {
				if (utilitySpace
						.getUtility(this.oppPreviousBid_Agent2) > this.utility_maximum_from_opponent_Session2_Agent2) {
					this.utility_maximum_from_opponent_Session2_Agent2 = utilitySpace
							.getUtility(this.oppPreviousBid_Agent2);
				}
			} else if (count_Agent2 < this.ThirdTimeInterval) {
				if (utilitySpace
						.getUtility(this.oppPreviousBid_Agent2) > this.utility_maximum_from_opponent_Session3_Agent2) {
				...
				this.midPointOfSlopeSessionMax1_Agent2 = (this.utility_maximum_from_opponent_Session2_Agent2
						+ this.utility_maximum_from_opponent_Session1_Agent2) / 2;
				this.midPointOfSlopeSessionMax2_Agent2 = (this.utility_maximum_from_opponent_Session3_Agent2
						+ this.utility_maximum_from_opponent_Session2_Agent2) / 2;
			...
			System.out.println(e.getMessage() + "exception in method MeasureConcedePartOfOppBehaviour_Agent2");
		}
	}
	...
	private Bid BidToOffer_original() {
		...
		try {
			...
			Bid bestBidOfferedByOpponent = this.getUtility(bestBidOfferedByOpponent1) > this
					.getUtility(bestBidOfferedByOpponent2) ? bestBidOfferedByOpponent2 : bestBidOfferedByOpponent1;
	...
	private boolean OtherAcceptCondition(boolean IsAccept) {
		try {
			if ((timeline.getTime() >= ((this.concedeTime_Agent2 + this.concedeTime_Agent1) / 2))
					&& (this.utilitySpace.getUtility(this.oppPreviousBid_Agent2) >= this.minUtilityUhreshold)) {
				IsAccept = true;
			}
		} catch (Exception e) {
			this.except = 27;
