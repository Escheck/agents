...
public class RandomDance extends AbstractNegotiationParty {
	...
	public Action chooseAction(List<Class<? extends Action>> validActions) {
		...
		for (String string : olderBidMap.keySet()) {
			...
			for (String player : utilityMap.keySet()) {
				...
				utility *= utilityMap.get(player).GetUtility(olderBidMap.get(string));
			}
			if (utility > maxval) {
				maxval = utility;
				maxPlayer = string;
		...
		try {
			offer = new Offer(getPartyId(), SearchBid(target, utilityMap, playerWeight));
			action = offer;
		} catch (Exception e) {
			// TODO 自動生成された catch ブロック
			e.printStackTrace();
	...
	public void receiveMessage(AgentID sender, Action action) {
		...
		if (utilityDatas.containsKey(sender.toString()) == false) {
			utilityDatas.put(sender.toString(), new PlayerDataLib(utilitySpace.getDomain().getIssues()));
		}
	...
	private double GetTarget(Map<String, PlayerData> datas) {
		...
		double target = 1.0 - (1.0 - max) * (Math.pow(timeline.getTime(), discountFactor));
	...
	private void myInit() {
		PlayerData playerData = new PlayerData(utilitySpace.getDomain().getIssues(), 1.0);
	 ...
	 */
	private Bid SearchBidWithWeights(Map<String, PlayerData> datas, Map<String, Double> weights) {
	 ...
	 * target以上で良い感じに
	 */
	private Bid SearchBid(double target, Map<String, PlayerData> datas, Map<String, Double> weights) throws Exception {
	...
	public String getDescription() {
		return "ANAC2015-23-RandomDance";
	}
	...
	public PlayerData(List<Issue> issues, double derta) {
		for (Issue issue : issues) {
			if (issue instanceof IssueDiscrete) {
				map.put(issue, new IssueDataDiscrete((IssueDiscrete) issue, derta));
			} else {
				map.put(issue, new IssueDataInteger((IssueInteger) issue, derta));
			}
		}
		this.derta = derta;
	}
