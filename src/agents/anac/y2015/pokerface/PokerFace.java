...
public class PokerFace extends AbstractNegotiationParty {
	...
	public void init(NegotiationInfo info) {
		...
		opponent_bid_list = new OpponentBidLists((AdditiveUtilitySpace) utilitySpace, true);
	...
	public Action chooseAction(List<Class<? extends Action>> validActions) {
		...
		if (timeline instanceof ContinuousTimeline) {
			time_left = Math.floor((total_time - timeline.getCurrentTime()) / average_time);
			time_after_walker = (total_time / average_time) * (1.0 - RANDOM_WALKER_TRESHOLD);
		...
		if (!validActions.contains(Accept.class)
			...
			if (((timeline instanceof ContinuousTimeline ? current_time : rounds)
					/ total_time) < RANDOM_WALKER_TRESHOLD) {
	...
	private Bid concederBid(List<Entry<Pair<Integer, Value>, Double>> ordered_pair) {
		int actions_left;
		int actions_after_walker;
		if (timeline instanceof ContinuousTimeline) {
			actions_left = (int) Math.round((total_time - current_time) / average_time);
	...
	private Bid generateConcedeBid(BigInteger step, Bid bid, List<Entry<Pair<Integer, Value>, Double>> ordered_pair,
			int issue_count) {
	...
	private double[] getIssueValues(Bid b) {
		...
		for (int j = 0; j < size; j++) {
			...
			double eval;
			try {
				Evaluator evaluator = ((AdditiveUtilitySpace) utilitySpace).getEvaluator(issue.getNumber());
				eval = evaluator.getWeight()
						* evaluator.getEvaluation((AdditiveUtilitySpace) utilitySpace, b, issue.getNumber());
			} catch (Exception e) {
				eval = 0.0;
			}
			w[j] = eval;
	...
	private void updateTime() {
		...
		if (timeline instanceof ContinuousTimeline) {
			...
			last_rounds_time[rounds % MA_SIZE] = (timeline.getCurrentTime() - current_time) / rounds;
			average_time += last_rounds_time[rounds % MA_SIZE] / MA_SIZE;
