...
public class MyBayesianOpponentModel extends OpponentModel {
	...
	public MyBayesianOpponentModel(AdditiveUtilitySpace pUS) {
		if (pUS == null)
		...
		for (int i = 0; i < numberOfIssues; i++) {
			...
			case DISCRETE:
				...
				IssueDiscrete lDiscIssue = (IssueDiscrete) (dDomain.getIssues()
						.get(i));
			...
			case REAL:
				lEvalHyps = new ArrayList<EvaluatorHypothesis>();
				aaEvaluatorHypothesis.add(lEvalHyps);
				IssueReal lRealIssue = (IssueReal) (dDomain.getIssues().get(i)); // Laptop
				// |
				// Harddisk
				// |
				// Monitor
		...
		for (int i = 0; i < pWeightHypothesis.length; i++) {
			for (int j = 0; j < pEvalHypothesis.size(); j++) {
				UtilitySpaceHypothesis lUSHyp = new UtilitySpaceHypothesis(
						dDomain, uUS, pWeightHypothesis[i],
