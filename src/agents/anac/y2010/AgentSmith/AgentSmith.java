...
public class AgentSmith extends Agent {
	...
	public void init() {
		...
		fPreferenceProfile = new PreferenceProfileManager(fBidHistory, (AdditiveUtilitySpace) this.utilitySpace);
		fBidStrategy = new SmithBidStrategy(fBidHistory, (AdditiveUtilitySpace) this.utilitySpace, fPreferenceProfile,
				getAgentID());
	}
	...
	public Action chooseAction() {
		...
		try {
			if (fBidHistory.getOpponentLastBid() != null
					&& utilitySpace.getUtility(fBidHistory.getOpponentLastBid()) > sMargin) {
				...
				if (firstRound && (fBidHistory.getMyLastBid() == null)) {
					...
					// accept!
					double utilOpponent = this.utilitySpace.getUtility(fBidHistory.getOpponentLastBid());
					double utilOur = this.utilitySpace.getUtility(fBidHistory.getMyLastBid());
