...
public class NiceTitForTat extends BilateralAgent {
	...
	protected void prepareOpponentModel() {
		opponentModel = new BayesianOpponentModelScalable((AdditiveUtilitySpace) utilitySpace);
	}
	...
	public Bid chooseCounterBid() {
		...
		log("Min/Max utility offered by opponent was " + round2(minimumOfferedUtilityByOpponent) + "/"
				...
				+ round2(minUtilityOfOpponentFirstBids) + " so his concession is " + round2(opponentConcession)
		...
		log("The gap to Nash is then " + round2(gapToNash) + ". I will add another bonus of " + round2(tit) + " (="
	...
	private Bid makeAppropriate(Bid myPlannedBid) {
		...
		if (bestUtilityByOpponent >= myPlannedUtility) {
			log("Opponent previously made a better offer to me than my planned util " + myPlannedUtility + ", namely  "
					+ bestUtilityByOpponent + ", so I make that bid instead.");
	...
	public boolean isAcceptable(Bid plannedBid) {
		...
		if (recentBidsSize > enoughBidsToCome) {
			log("I expect to see " + recentBidsSize + " more bids. I will only consider accepting when it falls below "
					+ enoughBidsToCome);
			return false;
		}
		...
		log("In the previous time window of size " + window + " I have seen " + n + " better offers (out of a total of "
				+ recentBidsSize + ") than " + round2(offeredUndiscountedUtility) + ", with avg util " + round2(avg));
	...
	protected Action makeAcceptAction() {
		...
		log("We expect to see " + expectedBids + " more bids. The AC wants to accept the offer of utility "
				+ round2(offeredUtility) + " (max offered = " + round2(bestBidUtility) + ").");
	...
	private List<Bid> getBidsOfUtility(double target) {
		...
		do {
			...
			if (size > 1 || (max >= 1 && size > 0)) {
				log(size + " bids of target utility " + round2(target) + " found in [" + round2(min) + ", "
						+ round2(max) + "]");
				return bids;
			}
		} while (max <= 1);
	...
	public String getDescription() {
		return "ANAC 2011 - Nice Tit for Tat";
	}
