...
public class ValueModelAgent extends Agent {
	...
	public Action chooseAction() {
		...
		try {
			...
			if (allBids == null) {
				...
				seperatedBids.init((AdditiveUtilitySpace) utilitySpace, opponentUtilModel);
			...
			if (actionOfPartner instanceof Offer) {
				...
				if (opponent == null) {
					...
					opponent = new OpponentModeler(bidCount, (AdditiveUtilitySpace) utilitySpace, timeline, ourBids,
							opponentBids, opponentUtilModel, allBids, this);
					opponentUtilModel.initialize((AdditiveUtilitySpace) utilitySpace, opponentBid);
				...
				if (opponentUtil > lowestApproved * 0.99 && !utilitySpace.isDiscounted()) {
					lowestApproved += 0.01;
	...
	public void chickenGame(double timeToGive, double concessionPortion, double acceptableThresh) {
		...
		if (opponentUtil >= planedThresh - 0.01) {
			lAction = new Accept(getAgentID(), ((ActionWithBid) actionOfPartner).getBid());
			return;
		}
	...
	public String getDescription() {
		return "ANAC 2011 - ValueModelAgent";
	}
}