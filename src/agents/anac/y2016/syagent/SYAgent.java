...
public class SYAgent extends AbstractNegotiationParty {
	...
	public Action chooseAction(List<Class<? extends Action>> validActions) {
		...
		Bid offerBid = bidSearch.getBid(utilitySpace.getDomain().getRandomBid(null),
	...
	public void receiveMessage(AgentID sender, Action action) {
		...
		if (action != null) {
			...
			if (action instanceof Inform && ((Inform) action).getName() == "NumberOfAgents"
				...
				if (isPrinting) {
					System.out.println("Number of Negotiator: " + negotiationInfo.getNegotiatorNum());
				}
			}
			// Accept の時
			if (action instanceof Accept) {
				...
				negotiationInfo.updateAcceptedBid(sender, DefaultAction.getBidFromAction(action)); // accept
																									// された
																									// bid
																									// を記録
			}
