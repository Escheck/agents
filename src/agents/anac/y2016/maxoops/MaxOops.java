...
public class MaxOops extends AbstractNegotiationParty {
	...
	private void init() throws Exception {
		...
		if (DEBUG) {
			...
			log1 = new PrintStream(new FileOutputStream(dir + "/log1.txt", false));
			log2 = new PrintStream(new FileOutputStream(dir + "/log2.txt", false));
			log3 = new PrintStream(new FileOutputStream(dir + "/log3.txt", false));
			error = new PrintStream(new FileOutputStream(dir + "/error.txt", false));
			System.setErr(error);
		} else {
		...
		while (bidIterator.hasNext()) {
			...
			stdUtil = Math.sqrt(stdUtil + Math.pow(util - meanUtil, 2) / (i + 1));
			i++;
		}
	...
	public void receiveMessage(AgentID sender, Action action) {
		...
		if (action instanceof Offer) {
			...
			opponentsBidHistories[opponent].add(new BidDetails(bid, util, time));
			...
			if (opponentsDistinctBids[opponent].filterUtility(util).isEmpty()) {
				opponentsDistinctBids[opponent].add(new BidDetails(bid, util, time));
				opponentsMinNumDistinctBids = opponentsDistinctBids[opponent].size();
				opponentsMaxNumDistinctBids = opponentsDistinctBids[opponent].size();
				for (int i = 0; i < numParties - 1; i++) {
					if (i == opponent)
						continue;
					opponentsMinNumDistinctBids = Math.min(opponentsMinNumDistinctBids,
							opponentsDistinctBids[i].size());
					opponentsMaxNumDistinctBids = Math.max(opponentsMaxNumDistinctBids,
							opponentsDistinctBids[i].size());
				}
			}
			...
			if (opponentsDistinctAccepts[opponent].filterUtility(util).isEmpty()) {
				opponentsDistinctAccepts[opponent].add(new BidDetails(lastBid, util, time));
				opponentsMinNumDistinctAccepts = opponentsDistinctAccepts[opponent].size();
				opponentsMaxNumDistinctAccepts = opponentsDistinctAccepts[opponent].size();
				for (int i = 0; i < numParties - 1; i++) {
					if (i == opponent)
						continue;
					opponentsMinNumDistinctAccepts = Math.min(opponentsMinNumDistinctAccepts,
							opponentsDistinctAccepts[i].size());
					opponentsMaxNumDistinctAccepts = Math.max(opponentsMaxNumDistinctAccepts,
							opponentsDistinctAccepts[i].size());
				}
			}
		}
		...
		try {
			...
			if (possibleActions.contains(Accept.class) || possibleActions.contains(Offer.class)) {
				if (myLastAction == null) {
					...
					if (maxUtil - secMaxUtil < (secMaxUtil - minUtil) * 1.1 && lastBid.equals(secMaxBid)) {
						System.out.println("Accepted!!!!");
						action = new Accept(getPartyId(), lastBid);
					} else {
						if (DMC.termination()) {
			...
			turn++;
			if (nextBid != null) {
				allBidHistory.add(new BidDetails(nextBid, TFC.thresholdFunc(), timeline.getTime()));
			}
