...
public class ClockworkAgent extends AbstractNegotiationParty {
	...
	public void receiveMessage(AgentID sender, Action action) {
		...
		} else if (action instanceof Accept && getUtilityWithDiscount(history.get(history.size() - 1)) > uFunctionC()) {
	...
	public double uBotlim(double t) {
		...
		botlim = (1 - Math.pow(t, 2)) * (dt * (1 - reservationValue)) + dt * reservationValue;
	...
	public Bid bidFunction() {
		...
		if (t < 0.3) {
			...
			while (count != 10000) {
				...
				if (getUtilityWithDiscount(bBid) >= f && getUtility(bBid) > getUtility(newBid))
					newBid = bBid;
	...
	public String getDescription() {
		return "ANAC2016 Clockwork Agent - Considers time.";
	}
}
