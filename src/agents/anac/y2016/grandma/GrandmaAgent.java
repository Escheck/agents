...
public class GrandmaAgent extends AbstractNegotiationParty {
	...
	public Action chooseAction(List<Class<? extends Action>> validActions) {
		...
		try {
			...
			if (validActions.contains(Accept.class) && isAcceptable(OppBidUtil)) {
				...
				Bid lastBid = BidHist.get((bidNum - lastOffsteps) % partiesN).getLastBid();
	...
	public void receiveMessage(AgentID sender, Action action) {
		...
		if (DefaultAction.getBidFromAction(action) != null) {
			...
			lastBid = BidHist.get(((bidNum + partiesN) - lastOffsteps) % partiesN).getLastBid();
			BH_update(bidNum % partiesN, lastBid);
		}
		if (lastBid != null) {
	...
	public String getDescription() {
		return "ANAC2016 Grandma: You are all the same in grandma's eyes";
	}
	...
	private void Counter_update(Bid recBid) throws Exception {
		for (int k = 0; k < issueN; k++) {
			// INTEGERS:
			if (utilitySpace.getDomain().getIssues().get(k).getType().toString().equals("INTEGER")) {
				// check if value outside bounds
				IssueInteger kIntIssue = (IssueInteger) utilitySpace.getDomain().getIssues().get(k);
				ValueInteger recBidInt = (ValueInteger) recBid.getValue(k + 1);
				Integer median = (kIntIssue.getLowerBound() + kIntIssue.getUpperBound()) / 2;
			...
			// DISCRETE:
			else if (utilitySpace.getDomain().getIssues().get(k).getType().toString().equals("DISCRETE")) {
				boolean found = false;
				if (IssueNames.get(k).isEmpty()) {
					...
					for (int j = 0; j < IssueNames.get(k).size(); j++) {
						 ...
						 */
						if (recBid.getValue(k + 1).toString().equals(IssueNames.get(k).get(j))) {
				...
				System.out.println("Wrong Issue Type:" + recBid.getValue(k + 1).getType());
			}
		}
	}
	...
	private void normaliseMean() {
		for (int i = 0; i < IssueAmounts.size(); i++) {
			// DISCRETE Issues
			if (utilitySpace.getDomain().getIssues().get(i).getType().toString().equals("DISCRETE")) {
				double tot = 0;
			...
			else if (utilitySpace.getDomain().getIssues().get(i).getType().toString().equals("INTEGER")) {
		...
		for (int i = 0; i < IssueNames.size(); i++) {
			...
			if (utilitySpace.getDomain().getIssues().get(i).getType().toString().equals("DISCRETE")) {
				for (int k = 0; k < IssueNames.get(i).size(); k++) {
					if (bid.getValue(i + 1).toString().equals(IssueNames.get(i).get(k))) {
						proxVal = proxVal + normIssueVals.get(i).get(k);
					}
				}
			} // INTEGERS
			else if (utilitySpace.getDomain().getIssues().get(i).getType().toString().equals("INTEGER")) {
				IssueInteger kIntIssue = (IssueInteger) utilitySpace.getDomain().getIssues().get(i);
				ValueInteger bidInt = (ValueInteger) bid.getValue(i + 1); // bid
																			// Integer
																			// value
				double nombidnorm = (double) (int) (bidInt.getValue() - kIntIssue.getLowerBound());
				double denbidnorm = (double) (int) (kIntIssue.getUpperBound() - kIntIssue.getLowerBound());
				Double bidNorm = nombidnorm / denbidnorm; // Normalised bid
															// Integer issue
															// value [0,1]
				double nomcountnorm = (double) (int) intIssCount.get(i).get(1);
				double dencountnorm = (double) (int) (intIssCount.get(i).get(0) + intIssCount.get(i).get(1));
	...
	private void lowBoundUtil_update(double lastUt) {
		...
		// trough negotiation)
		double discFact = DISC_SENSITIVITY / ((bidNum * (partiesN - 1)) / (partiesN * time));
		...
		lowBoundUtil = (MIN_UTIL_BOUND + (1 - MIN_UTIL_BOUND) * discMeanUtil) * tfact;// *Math.pow(discMeanUtil,
