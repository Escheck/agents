...
public class YXAgent extends AbstractNegotiationParty {
		...
		if (rounds > 10 && normalizedTime <= 0.9) {// Initialization of Value
			...
			for (Issue issue : issues) {
				...
				if (issue.getType().toString().equals("INTEGER")) {
					for (Value vv : oppValueFrequency.get(hardestOpp).get(issue).keySet()) { // Usual
																								// way
																								// of
																								// key
																								// retrieval
																								...
																								// loop
																								// through
						if (Integer.parseInt(vv.toString()) == Integer.parseInt(v.toString())) {
							calUtil += oppIssueWeight.get(hardestOpp).get(issue)
									* oppValueFrequency.get(hardestOpp).get(issue).get(vv);
							break;
						}
					}
				} else {
					calUtil += oppIssueWeight.get(hardestOpp).get(issue)
							* oppValueFrequency.get(hardestOpp).get(issue).get(v);
				}
			}
			...
			try {
				...
				myAction = oppUtility > calThreshold ? new Accept(getPartyId(), lastOpponentBid)
			...
			try {
				myAction = oppUtility > tempThreshold ? new Accept(getPartyId(), lastOpponentBid)
						: new Offer(getPartyId(), testBid);
			} catch (Exception e) {
				e.printStackTrace();
			}
	...
	private void updateValueIntegerWeight(AgentID sender) {
		for (Issue issue : issues) {
			if (issue.getType().toString().equals("INTEGER")) {
				ValueInteger currIssueValueInteger = (ValueInteger) lastOpponentBid.getValue(issue.getNumber());
				oppIssueIntegerValue.get(sender).get(issue).add(currIssueValueInteger);
			}
		}
			...
			for (Object o1 : oppIssueIntegerValue.keySet()) {
				for (Issue issue : issues) {
					if (issue.getType().toString().equals("INTEGER")) {
						...
						// lower range
						for (int i = 0; i < oppIssueIntegerValue.get(o1).get(issue).size(); i++) {
							int vv = Integer.parseInt(oppIssueIntegerValue.get(o1).get(issue).get(i).toString());
							if (vv <= mid) {
								sumLower += 1;
							} else {
								sumUpper += 1;
						...
						if (sumLower > sumUpper) {
							...
							for (int i = lowerB + 1; i < upperB; i++) {
								...
								oppValueFrequency.get(o1).get(issue).put(vI, adjustment);
							}
						} else {
	...
	private void updateModelOppIssueWeight(AgentID sender) {
		if (oppBidHistory.get(sender).size() != 0 && rounds >= 10) {
			Bid previousRoundBid = oppBidHistory.get(sender).get(oppBidHistory.get(sender).size() - 1);// -1
																										// since
																										// current
																										// lastOpponentBid
																										// not
																										// added
																										// yet
			double issueWeightFormula = (Math.pow((1 - normalizedTime), 10)) / (issues.size() * 100);
			double issueWeightInteger = (Math.pow((1 - normalizedTime), 10)) / (issues.size() * 10);
			double issueSum = 0.;
			double normalizedIssueW;

			for (Issue issue : issues) {
				Value prevIssueValue = previousRoundBid.getValue(issue.getNumber());
				Value currIssueValue = lastOpponentBid.getValue(issue.getNumber());

				if (issueContainIntegerType) { // For Discrete & Integer Mix
												// Domain
					if (issue.getType().toString().equals("INTEGER")) { // Integer
																		// Handling
						if (incrementIntegerIssueWeight(issue, prevIssueValue, currIssueValue)) {
							oppIssueWeight.get(sender).put(issue,
									oppIssueWeight.get(sender).get(issue) + issueWeightInteger);
						}
					...
					if (prevIssueValue == currIssueValue) {
						oppIssueWeight.get(sender).put(issue,
								oppIssueWeight.get(sender).get(issue) + issueWeightFormula);
					}
				}
			...
			for (Issue issue : issues) {
				normalizedIssueW = oppIssueWeight.get(sender).get(issue) / issueSum;
				oppIssueWeight.get(sender).put(issue, normalizedIssueW);
			}
		}
	}

	public boolean incrementIntegerIssueWeight(Issue issue, Value prevIssueValue, Value currIssueValue) {
		...
		if (pastV <= arraySlot[0] && currV <= arraySlot[0]) {
			sameSection = true;
		} else if (pastV <= arraySlot[1] && currV <= arraySlot[1] && pastV > arraySlot[0] && currV > arraySlot[0]) {
			sameSection = true;
		} else if (pastV <= arraySlot[2] && currV <= arraySlot[2] && pastV > arraySlot[1] && currV > arraySlot[1]) {
			sameSection = true;
		} else if (pastV <= arraySlot[3] && currV <= arraySlot[3] && pastV > arraySlot[2] && currV > arraySlot[2]) {
			sameSection = true;
		} else if (pastV <= arraySlot[4] && currV <= arraySlot[4] && pastV > arraySlot[3] && currV > arraySlot[3]) {
			sameSection = true;
		}
		return sameSection;
	}
	...
	private void updateModelOppValueWeight(AgentID sender) {
			...
			if (issueContainIntegerType) { // For Discrete & Integer Mix Domain
				if (!issue.getType().toString().equals("INTEGER")) {
					...
					oppValueFrequency.get(sender).get(issue).put(value,
							oppValueFrequency.get(sender).get(issue).get(value) + valueWeightInteger);
				}

			} else { // For Discrete Only Domain
				Value value = lastOpponentBid.getValue(issue.getNumber());
				oppValueFrequency.get(sender).get(issue).put(value,
						oppValueFrequency.get(sender).get(issue).get(value) + valueWeightFormula);
			}
		}
		...
		for (Issue issue : issues) {
			if (!issue.getType().toString().equals("INTEGER")) {
				...
				for (Value v : oppValueFrequency.get(sender).get(issue).keySet()) {
					currValueBase = oppValueFrequency.get(sender).get(issue).get(v);
				...
				for (Value v : oppValueFrequency.get(sender).get(issue).keySet()) {
					normalizedValue = oppValueFrequency.get(sender).get(issue).get(v) / maxValueBase;
					oppValueFrequency.get(sender).get(issue).put(v, normalizedValue);
				}
			}
		}
	}
	...
	private void issueIntegerHandler(AgentID sender) {
		oppIssueIntegerValue.put(sender, new HashMap<Issue, ArrayList<ValueInteger>>());
		for (Issue issue : issues) {
			if (issue.getType().toString().equals("INTEGER")) {
				oppIssueIntegerValue.get(sender).put(issue, new ArrayList<ValueInteger>());
			}
		}
	}
	...
	private void initModelOppValueFrequency(AgentID sender) {
		...
		oppValueFrequency.put(sender, new HashMap<Issue, HashMap<Value, Double>>());
		for (Issue issue : issues) {
			oppValueFrequency.get(sender).put(issue, new HashMap<Value, Double>());
			if (!issue.getType().toString().equals("INTEGER")) {
	...
	protected Value getRandomValue(Issue currentIssue) throws Exception {
		...
		case REAL:
			...
			currentValue = new ValueReal(
					lIssueReal.getLowerBound() + ((double) ((lIssueReal.getUpperBound() - lIssueReal.getLowerBound()))
							/ (lIssueReal.getNumberOfDiscretizationSteps())) * index);
			break;
		case INTEGER:
