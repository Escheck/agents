...
public class AgentLight extends AbstractNegotiationParty {
	...
	public void init(NegotiationInfo info) {
		...
		for (Issue issue : issueList) {
			...
			case INTEGER:
				...
				for (int i = integerIssue.getLowerBound(); i <= integerIssue.getUpperBound(); i++) {
			...
			default:
				try {
					throw new Exception("issue type " + issue.getType() + " not supported");
				} catch (Exception e) {
		...
		try {
			for (int i = 0; i < possibleBidNum; i++) {
				...
				issueInst = Int2Bid(i);
				ComparableBid bid = new ComparableBid(new Bid(utilitySpace.getDomain(), issueInst));
				bidList.add(bid);
			}
	...
	public Action chooseAction(List<Class<? extends Action>> Actions) {
		...
		if (!agentOrder.contains(getPartyId())) {
			...
			if (agentOrder.indexOf(getPartyId()) == (opponentNum - 1))
				...
				nextOpponent = opponentInfo.get(agentOrder.get(agentOrder.indexOf(getPartyId()) + 1));
			}
		}
		recordTimes++;

		double remainingRounds = (timeline.getTotalTime() - timeline.getCurrentTime())
		...
		if (recordTimes >= learningTimes) {
			...
			if (isPrinting) {
				System.out.println("worthyOpponent: " + worthyOpponent.getAgentID().toString());
			}
			int worthyOpponentBidSize = worthyOpponent.getAgentBidHistory().size();
			opponentCurBid = worthyOpponent.getAgentBidHistory().get(worthyOpponentBidSize - 1);
			// opponentPreBid =
		...
		double base = ((timeline.getTotalTime() - timeline.getCurrentTime()) / timeline.getTotalTime()) > 0.7
				? ((timeline.getTotalTime() - timeline.getCurrentTime()) / timeline.getTotalTime()) : 0.7;
		...
		minUtility = (reservationValue + (maxUtility - reservationValue) * Math.pow(base, learningExp))
				* discountFactor;
		...
		if (remainingRounds <= 3 && lastBidInHistory != null
				&& utilitySpace.getUtilityWithDiscount(lastBidInHistory, timeline.getTime()) >= minUtility
		...
		if (lastBidInHistory != null
				&& utilitySpace.getUtilityWithDiscount(lastBidInHistory, timeline.getTime()) >= minUtility
				&& !myAcceptBidRecord.contains(lastBidInHistory) && !bidPotential.contains(lastBidInHistory)
				|| (bestBidInPotential != null && !myAcceptBidRecord.contains(lastBidInHistory)
						&& !bidPotential.contains(lastBidInHistory)
						&& utilitySpace.getUtilityWithDiscount(lastBidInHistory, timeline.getTime()) >= utilitySpace
								.getUtilityWithDiscount(bestBidInPotential, timeline.getTime()))) {
			if (isPrinting) {
		...
		if (bestBidInPotential != null && remainingRounds <= 3
				&& utilitySpace.getUtilityWithDiscount(bestBidInPotential, timeline.getTime()) >= minUtility) {
			if (isPrinting) {
	...
	public OpponentInfo findOpponent(HashMap<AgentID, OpponentInfo> opponentInfo) {
		...
		for (AgentID agentId : opponentInfo.keySet()) {
			int opponentBidSize = opponentInfo.get(agentId).getAgentBidHistory().size();
			if (opponentBidSize >= 2) {
				Bid opponentCurBid = opponentInfo.get(agentId).getAgentBidHistory().get(opponentBidSize - 1);
				Bid opponentPreBid = opponentInfo.get(agentId).getAgentBidHistory().get(opponentBidSize - 2);
				double value = opponentInfo.get(agentId).getOpponentStandardDeviation()[1]
						+ opponentInfo.get(agentId).getOpponentAverage()[1];
				if (value < minValue && (opponentInfo.get(agentId).getOpponentLastAction() instanceof Offer)
	...
	public Action NegotiationAction(double minUtility) {
		...
		if (recordTimes > 1) {
			...
			tftUtility = (utilitySpace.getUtility(opponentCurBid)
					- utilitySpace.getUtility(worthyOpponent.getAgentBidHistory().get(0)))
					* (worthyOpponent.getOpponentStandardDeviation()[1] + worthyOpponent.getOpponentAverage()[1]);
		...
		if (isPrinting) {
			System.out.println("OpponentStandardDeviation()[0]: " + worthyOpponent.getOpponentStandardDeviation()[0]);
			System.out.println("OpponentStandardDeviation()[1]: " + worthyOpponent.getOpponentStandardDeviation()[1]);
			System.out.println("OpponentAverage()[0]: " + worthyOpponent.getOpponentAverage()[0]);
			System.out.println("OpponentAverage()[1]: " + worthyOpponent.getOpponentAverage()[1]);
		}
		if (offerUtility < reservationValue)
			offerUtility = reservationValue;

		if (offerUtility < maxUtility - (utilitySpace.getUtility(worthyOpponent.getBestBid())
				- utilitySpace.getUtility(worthyOpponent.getAgentBidHistory().get(0))))
			offerUtility = maxUtility - (utilitySpace.getUtility(worthyOpponent.getBestBid())
					- utilitySpace.getUtility(worthyOpponent.getAgentBidHistory().get(0)));
		...
		if (utilitySpace.getUtility(lastBidInHistory) > minUtility
				...
				|| (bestBidInPotential != null && !myAcceptBidRecord.contains(lastBidInHistory)
						&& utilitySpace.getUtilityWithDiscount(lastBidInHistory, time) > utilitySpace
	...
	public Bid chooseBestBid(double minUtility) {
		...
		while (((ceil > 0) && (ceil < bidList.size() - 1)) && ((utilitySpace
				.getUtilityWithDiscount(bidList.get(ceil).bid, time) > minUtility
				&& utilitySpace.getUtilityWithDiscount(bidList.get(ceil + 1).bid, time) > minUtility)
				|| (utilitySpace.getUtilityWithDiscount(bidList.get(ceil).bid, time) <= minUtility
						&& utilitySpace.getUtilityWithDiscount(bidList.get(ceil + 1).bid, time) <= minUtility))) {
			if (utilitySpace.getUtilityWithDiscount(bidList.get(ceil).bid, time) > minUtility) {
				if ((int) Math.ceil(bidList.size() * Math.pow(0.5, exp)) == 0) {
					ceil -= 1;
				} else {
					ceil -= (int) Math.ceil(bidList.size() * Math.pow(0.5, exp));
				}
			} else {
				if ((int) Math.floor(bidList.size() * Math.pow(0.5, exp)) == 0) {
					ceil += 1;
				} else {
					ceil += (int) Math.floor(bidList.size() * Math.pow(0.5, exp));
				}
			}
			exp++;
		}
	...
	public double getOppUtility(Bid bid) {
		...
		for (Issue issue : bid.getIssues()) {
			switch (issue.getType()) {
			case DISCRETE:
				utility += worthyOpponent.getValueWeights().get(issue).get(bidTemp.get(issue.getNumber()));
				break;
			case INTEGER:
				IssueInteger integerIssue = (IssueInteger) issue;
				double lowerBound = integerIssue.getLowerBound();
				double upperBound = integerIssue.getUpperBound();
				double lowerWeight = utility += worthyOpponent.getIssueWeights().get(issue)[0];
				double upperWeight = utility += worthyOpponent.getIssueWeights().get(issue)[1];
				utility = lowerWeight + (upperWeight - lowerWeight)
						* (((ValueInteger) bidTemp.get(issue.getNumber())).getValue() - lowerBound)
	...
	public void receiveMessage(AgentID sender, Action action) {
		...
		if (action != null) {
			if (action instanceof Inform && ((Inform) action).getName() == "NumberOfAgents"
				...
				opponentNum = ((Integer) ((Inform) action).getValue()).intValue();
				if (isPrinting) {
				...
				if (utilitySpace.getUtility(opponentInfo.get(sender).getBestBid()) < utilitySpace.getUtility(bid)) {
					opponentInfo.get(sender).setBestBid(bid);
				}
				opponentInfo.get(sender).setOpponentLastAction(action);
	...
	public HashMap<Integer, Value> Int2Bid(int number) {
		...
		for (int i = issueList.size() - 1; i >= 0; i--) {
			temp = numberTemp % (issue2Value.get(issueList.get(i).getNumber()).size());
			issueInst.put(issueList.get(i).getNumber(), issue2Value.get(issueList.get(i).getNumber()).get(temp));
			numberTemp = (numberTemp - temp) / (issue2Value.get(issueList.get(i).getNumber()).size());
		}
		return issueInst;
	}
	...
	private class ComparableBid implements Comparable<ComparableBid> {
		...
		public int compareTo(ComparableBid cBid) {
			...
			return (int) (utilitySpace.getUtilityWithDiscount(this.bid, time) * 1000)
					- (int) (utilitySpace.getUtilityWithDiscount(cBid.bid, time) * 1000);
		}
	...
	private class OpponentInfo {
		...
		public void setOpponentStandardDeviation(double[] opponentStandardDeviation) {
		...
		public void setValueWeights(HashMap<Issue, HashMap<Value, Integer>> valueWeights) {
		...
		public void updateBid(Bid bid) {
			...
			for (Issue issue : bid.getIssues()) {
				...
				case DISCRETE:
					...
					if (recordTimes > 1 && bid.getValue(issue.getNumber()) == bidHistory.get(bidHistory.size() - 2)
							.getValue(issue.getNumber())) {
						value.put(bid.getValue(issue.getNumber()), times + frequencyReward + unchangeReward);
						valueWeights.put(issue, value);
					}
					value.put(bid.getValue(issue.getNumber()), times + frequencyReward);
					valueWeights.put(issue, value);
					break;
				case INTEGER:
					int issueNum = ((IssueInteger) issue).getNumber();
					Value opponentValue = bid.getValue(issueNum);
					int opponentValueInteger = ((ValueInteger) opponentValue).getValue();

					int upperBound = ((IssueInteger) issue).getUpperBound();
					int lowerBound = ((IssueInteger) issue).getLowerBound();
					double midPoint = Math.ceil(lowerBound + (upperBound - lowerBound) / 2) + 1;

					if (midPoint > opponentValueInteger) {
						double distanceFromMidPoint = midPoint - opponentValueInteger;
						double normalizedDistanceFromMidPoint = distanceFromMidPoint / (midPoint - lowerBound);

						double total = 1;
						double lowerBoundWeight = issueWeights.get(issue)[0];
						double upperBoundWeight = issueWeights.get(issue)[1];

						double newLowEndEvaluation = lowerBoundWeight + lowerBoundWeight
								* normalizedDistanceFromMidPoint
								* Math.pow(1 - timeline.getCurrentTime() / timeline.getTotalTime(), learningExp);
						...
						double distanceFromMidPoint = opponentValueInteger - midPoint + 1;
						double normalizedDistanceFromMidPoint = distanceFromMidPoint / (upperBound - midPoint + 1);
						double total = 1;
						double lowerBoundWeight = issueWeights.get(issue)[0];
						double upperBoundWeight = issueWeights.get(issue)[1];

						double newHighEndEvaluation = upperBoundWeight + upperBoundWeight
								* normalizedDistanceFromMidPoint
								* Math.pow(1 - timeline.getCurrentTime() / timeline.getTotalTime(), learningExp);
			...
			opponentVariance = (opponentPowerSum / recordTimes) - Math.pow(opponentAverage[1], 2);
			opponentStandardDeviation[1] = Math.sqrt((opponentVariance >= 0 ? opponentVariance : 0));
		}
