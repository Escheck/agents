...
public class Caduceus extends AbstractNegotiationParty {
	...
	public double[] scores = UtilFunctions.normalize(new double[] { 500, 10, 5, 3, 1 });
	...
	public void init(NegotiationInfo info) {
		...
		discountFactor = info.getUtilitySpace().getDiscountFactor(); // read
																		// discount
																		// factor
		double reservationValue = info.getUtilitySpace().getReservationValueUndiscounted();
		...
		for (NegotiationParty agent : agents) {
			...
			agent.init(new NegotiationInfo(info.getUtilitySpace(), info.getDeadline(), info.getTimeline(),
					info.getRandomSeed(), info.getAgentID(),
					new DefaultPersistentDataContainer(null, PersistentDataType.DISABLED)));
		}
	...
	public Action chooseAction(List<Class<? extends Action>> validActions) {
		...
		if (scoreOfAccepts > scoreOfBids) {
			...
			return new Offer(getPartyId(), getRandomizedAction(agentsWithBids, bidsFromAgents));
		}

		return new Offer(getPartyId(), getBestBid());
	}

	private Bid getRandomizedAction(ArrayList<Integer> agentsWithBids, ArrayList<Bid> bidsFromAgents) {
