...
public class ParsAgent2 extends AbstractTimeDependentNegotiationParty {
	...
	public void clusterOpponentBid(ArrayList<Bid> bidHistory, String opParty, boolean repeat) {
	...
	public boolean refreshClusterCenters(ArrayList<Cluster> clusters) {
		...
		for (int i = 0; i < clusters.size(); ++i) {
			...
			float[][] dismatrix = new float[clus.getMembers().size()][clus.getMembers().size()];
	 ...
	 */
	public ParsAgent2(UtilitySpace utilitySpace, Map<DeadlineType, Object> deadlines, Timeline timeline,
			long randomSeed) {
	...
	public Action chooseAction(List validActions) {
		...
		try {
			...
			long remainTime = (3 * 60 * 1000) - (System.currentTimeMillis() - beginTime);
	...
	public Bid bestMutualCenters() {
		...
		for (int i = 0; i < clusterA.size(); ++i) {
			for (int j = 0; j < clusterB.size(); ++j) {
				float temp = calDist(clusterA.get(i).getCenter(), clusterB.get(j).getCenter());
				if (temp < min) {
					min = temp;
					centerindex = i;
					centerindex2 = j;
	...
	public void receiveMessage(AgentID sender, Action action) {
		...
		if (action != null && action instanceof Offer) {
			...
			BidUtility opBid;
			try {
				opBid = new BidUtility(newBid, utilitySpace.getUtility(newBid), System.currentTimeMillis());
				...
				calculateParamForOpponent((oppAName.equals(agentName) ? oppAPreferences : oppBPreferences), newBid);
			...
			if (!lastBid.equals(myLastBid)) {
				try {
					...
					opBid = new BidUtility(lastBid, utilitySpace.getUtility(lastBid), System.currentTimeMillis());
	...
	public Bid offerMyNewBid() {
		...
		try {
			...
			if (bidNN == null || utilitySpace.getUtility(bidNN) < getMyutility()) {
				...
				if (hasnotNull) {
					for (int i = 0; i < isues.size(); ++i) {
						...
						if (keyVal != null && dissues.get(i) instanceof IssueDiscrete) {
							...
							IssueDiscrete is = (IssueDiscrete) dissues.get(i);
							if (!loop)
								map.put(new Integer(i + 1), myBestBid.getValues().get(new Integer(i + 1)));
							else
								map.put(new Integer(i + 1), getRandomValue(dissues.get(i)));
					...
					try {
						bid = new Bid(utilitySpace.getDomain(), (HashMap) map.clone());
		...
		while (twocycle > 0) {
			...
			for (int i = 0; i < dissues.size(); ++i) {
				if (oppAPreferences.getRepeatedissue().get(dissues.get(i).getName()) != null) {
					HashMap vals = (HashMap) oppAPreferences.getRepeatedissue().get(dissues.get(i).getName());
					HashMap valsB = (HashMap) oppBPreferences.getRepeatedissue().get(dissues.get(i).getName());
					...
					if (valsB != null) {
						...
						if (twocycle == 2) {
							if (maxkey[0] != null && maxkeyB[0] != null && maxkey[0].equals(maxkeyB[0])) {
							...
							for (int m = 0; insideloop && m < 2; ++m) {
								for (int z = 0; insideloop && z < 2; ++z) {
									if (maxkey[m] != null && maxkeyB[z] != null && maxkey[m].equals(maxkeyB[z])) {
										ArrayList l = new ArrayList();
										l.add(maxkey[m]);
										l.add(maxB[z]);
										l.add(max[m]);
						...
						mutualList.add(i, null);
						oppBPreferences.getRepeatedissue().put(dissues.get(i).getName(), new HashMap());
					}
				} else {
					oppAPreferences.getRepeatedissue().put(dissues.get(i).getName(), new HashMap());
					mutualList.add(i, null);
				}
	...
	public Bid getNNBid(ArrayList<BidUtility> oppAB) {
				...
				try {
					...
					if (utilitySpace.getUtility(b) > getMyutility()) {
						...
						for (int k = 0; k < maxBids.size(); ++k) {
							if (utilitySpace.getUtility(maxBids.get(k)) < utilitySpace.getUtility(b)) {
								index = k;
								break;
							}
						}
					...
					if (utilitySpace.getUtility(newBid) >= getMyutility()) {
						int index = 0;
						for (int k = 0; k < maxBids.size(); ++k) {
							if (utilitySpace.getUtility(maxBids.get(k)) < utilitySpace.getUtility(newBid)) {
								index = k;
								break;
							}
						}
		...
		if (maxBids.size() != 0) {
			...
			for (int i = 0; i < loop; i++) {
				if (!rejectedBid.contains(maxBids.get(i)) && prob(maxBids.get(i)) > maxProb) {
	...
	public float calDist(Bid b1, Bid b2) {
		...
		for (int j = 0; j < dissues.size(); ++j) {
			...
			if (dissues.get(j) instanceof IssueDiscrete) {
				...
				if (!b1.getValues().get(keyVals[j]).toString()
						.equals(b2.getValues().get(keyCenterVals[j]).toString())) {
					distance += 1;
				}
			} else {
				float dis = ((Float.parseFloat(b1.getValues().get(keyVals[j]).toString()))
						- (Float.parseFloat(b2.getValues().get(keyCenterVals[j]).toString()))
	...
	public double getMyutility() {
		...
		if (clusterIsRefreshA && clusterA != null) {
			...
			for (int i = 0; i < clusterA.size(); ++i) {
				...
				float temp = normal - ((float) clusterA.get(i).getMembers().size()) / (float) oppABids.size();
			...
			clusterHistoryA.distributionFactor.add(clusterHistoryA.distributionFactor.size(), new Float(unormalA));
			clusterHistoryA.maxDistance.add(clusterHistoryA.maxDistance.size(), new Float(maxA));
			clusterHistoryA.prevCenters.add(clusterHistoryA.prevCenters.size(), new ArrayList(centersA));
			...
			for (int i = 0; i < clusterHistoryA.distributionFactor.size(); ++i) {
				...
				for (int j = i + 1; j < clusterHistoryA.distributionFactor.size(); ++j) {
					...
					for (int k = 0; k < numberOfclusters; ++k) {
						for (int l = 0; l < numberOfclusters; ++l) {
							temp1 = calDist(clusterHistoryA.prevCenters.get(i).get(k),
			...
			meanDistributionA = (meanDistributionA / clusterHistoryA.distributionFactor.size()) / 1.33f;
			meanDistanceA = (meanDistanceA / clusterHistoryA.distributionFactor.size()) / issueSize;

			float w1 = 0.4f, w2 = 0.2f, w3 = 0.4f;
			concessionA = (w1 * maxCenterDistanceA + w2 * meanDistanceA - w3 * meanDistributionA);
			myConcession = concessionA < concessionB ? concessionA : concessionB;
			myConcessionRate = myConcession > 0 && myConcession > 0.15d ? myConcession : 0.15d;
			...
			for (int i = 0; i < clusterB.size(); ++i) {
				...
				float temp = normal - ((float) clusterB.get(i).getMembers().size()) / (float) oppBBids.size();
			...
			clusterHistoryB.distributionFactor.add(clusterHistoryB.distributionFactor.size(), new Float(unormalB));
			clusterHistoryB.maxDistance.add(clusterHistoryB.maxDistance.size(), new Float(maxB));
			clusterHistoryB.prevCenters.add(clusterHistoryB.prevCenters.size(), new ArrayList(centersB));
			for (int i = 0; i < clusterB.size(); ++i) {
			...
			for (int i = 0; i < clusterHistoryB.distributionFactor.size(); ++i) {
				...
				for (int j = i + 1; j < clusterHistoryB.distributionFactor.size(); ++j) {
					...
					for (int k = 0; k < numberOfclusters; ++k) {
						for (int l = 0; l < numberOfclusters; ++l) {
							temp = calDist(clusterHistoryB.prevCenters.get(i).get(k),
			...
			meanDistributionB = (meanDistributionB / clusterHistoryB.distributionFactor.size()) / 1.33f;
			meanDistanceB = (meanDistanceB / clusterHistoryB.distributionFactor.size()) / issueSize;
	...
	private class OpponentPreferences {
		...
		public void setOpponentBids(ArrayList<ParsAgent2.BidUtility> opponentBids) {
