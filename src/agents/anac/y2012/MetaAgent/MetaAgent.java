...
public class MetaAgent extends Agent {
	...
	private void getDomainParams() {
		...
		params[1] = this.utilitySpace.getDiscountFactor() == 0 ? 1 : this.utilitySpace.getDiscountFactor(); // 1
																											// =
																											// discount
																											// factor.
	...
	private String getBestAgent() {
		...
		for (int k = 0; k < reg.length; k++) {
			...
			if (!(reg[k][0].equalsIgnoreCase("Nice Tit-for-Tat Agent") || reg[k][0].equalsIgnoreCase("IAMhaggler2011")
		...
		try {
			if (!agentInit) {
						...
						// initiator)
				{
					...
					myAgent.internalInit(0, 1, this.startTime, this.totalTime, this.timeline, this.utilitySpace,
