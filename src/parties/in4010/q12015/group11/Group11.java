...
public class Group11 extends AbstractNegotiationParty {
	...
	public void init(NegotiationInfo info) {
		...
		super.init(info);
		lastBid = new Bid(info.getUtilitySpace().getDomain());
	}
	...
	public void receiveMessage(AgentID sender, Action action) {
		...
		Bid currentOpBid = DefaultAction.getBidFromAction(action);
	...
	public void updateModel(Object sender, Bid opponentBid, double time) {
		...
		if (!opponentUtilitySpace.containsKey(sender.toString())) {
			...
			AdditiveUtilitySpace oppUSpace = (AdditiveUtilitySpace) getUtilitySpace().copy();
			...
			for (Entry<Objective, Evaluator> e : oppUSpace.getEvaluators()) {
				...
				try {
					...
					for (ValueDiscrete vd : ((IssueDiscrete) e.getKey()).getValues())
						((EvaluatorDiscrete) e.getValue()).setEvaluation(vd, 10);
				} catch (Exception ex) {
		...
		for (Integer i : lastDiffSet.keySet()) {
			if (lastDiffSet.get(i) == 0 && oppUSpace.getWeight(i) < maximumWeight)
				oppUSpace.setWeight(oppUSpace.getDomain().getObjectivesRoot().getObjective(i),
