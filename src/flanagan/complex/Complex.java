...
public class Complex {
	...
	private static char jori = 'j'; // i or j in a + j.b or a + i.b
									// representaion
									// default value = j
	private static boolean infOption = true; // option determining how infinity
												// is handled
	// if true (default option):
	// multiplication with either complex number with either part = infinity
	// returns infinity
	...
	// constructor - initialises both real and imag to the values of an existing
	// Complex
	...
	// Print an array to terminal window without text (message) but with a line
	// return
	...
	// Print an array to terminal window with text (message) but no line returns
	// except at the end
	...
	// Print an array to terminal window without text (message) but with no line
	// returns except at the end
	...
	// TRUNCATION
	// Rounds the mantissae of both the real and imaginary parts of Complex to
	// prec places
	// Static method
	...
	// Create a three dimensional array of Complex objects of dimensions n, m
	// and l
	// all real = zero and all imag = zero
	...
	// Subtract a double number to a double and return difference as Complex
	// [static method]
	...
	// Subtract a Complex number from this Complex number and replace this by
	// the difference
	...
	// Subtract a double number from this Complex number and replace this by the
	// difference
	...
	// infOption -> true; standard arithmetic overriden - see above (instance
	// variable definitions) for details
	...
	// Sets the infinity handling option in multiplication and division
	// opt = 0: infOption -> true; standard arithmetic overriden - see above
	// (instance variable definitions) for details
	...
	// Multiply a double number to a double and return product as Complex
	// [static method]
	...
	// Multiply this Complex number by a Complex number and replace this by the
	// product
	...
	public static Complex over(Complex a, Complex b) {
		Complex c = new Complex(0.0D, 0.0D);
		if (Complex.infOption && !a.isInfinite() && b.isInfinite())
			return c;
	...
	public static Complex over(Complex a, double b) {
		Complex c = new Complex(0.0D, 0.0D);
		if (Complex.infOption && Fmath.isInfinity(b))
			return c;
	...
	public static Complex over(double a, Complex b) {
		Complex c = new Complex();
		if (Complex.infOption && !Fmath.isInfinity(a) && b.isInfinite())
			return c;
	...
	// Divide a double number by a double and return quotient as Complex [static
	// method]
	...
	public Complex over(Complex a) {
		...
		if (Complex.infOption && !this.isInfinite() && a.isInfinite())
			return b;
	...
	public Complex transposedOver(double a) {
		Complex c = new Complex(0.0D, 0.0D);
		if (Complex.infOption && !Fmath.isInfinity(a) && this.isInfinite())
			return c;
	...
	// Division of this Complex number by a Complex number and replace this by
	// the quotient
	...
	// Division of this Complex number by a double and replace this by the
	// quotient
	...
	public static Complex inverse(Complex a) {
		Complex b = new Complex(0.0D, 0.0D);
		if (Complex.infOption && a.isInfinite())
			return b;
	...
	// Square of the absolute value (modulus) of a complex number [static
	// method]
	public static double squareAbs(Complex a) {
		return a.real * a.real + a.imag * a.imag;
	}

	// Square of the absolute value (modulus) of a complex number [instance
	// method]
	...
	// returns a Complex number raised to an integer, i.e. int, power [instance
	// method]
	...
	// LOGICAL FUNCTIONS
	// Returns true if the Complex number has a zero imaginary part, i.e. is a
	// real number
	...
	// Returns true if the Complex number has a zero imaginary part within the
	// limit lim, i.e. is a real number
	...
	// Returns true if the Complex number has a zero imaginary part less than
	// the percentage precent ofv the real part
	public static boolean isRealPerCent(Complex a, double percent) {
		boolean test = false;
		if (Math.abs(a.imag * 100.0 / a.real) <= Math.abs(percent))
			test = true;
		return test;
	}
	...
	public static boolean isRealPerCent(Complex[] a, double percent) {
		...
		int n = a.length;
		for (int i = 0; i < n; i++) {
			if (Math.abs(a[i].imag * 100.0 / a[i].real) > Math.abs(percent))
				test = false;
		}
		return test;
	}
	...
	public boolean isRealperCent(double percent) {
		boolean test = false;
		if (Math.abs(this.imag * 100.0 / this.real) <= Math.abs(percent))
			test = true;
		return test;
	}

	// Returns true if the Complex number has a zero real and a zero imaginary
	// part
	// i.e. has a zero modulus
	public static boolean isZero(Complex a) {
		boolean test = false;
		if (Math.abs(a.real) == 0.0D && Math.abs(a.imag) == 0.0D)
			test = true;
		return test;
	}
	...
	// Returns true if either the real or the imaginary part of the Complex
	// number
	...
	// returns true if the differences between the real and imaginary parts of
	// two complex numbers
