...
public class CounterOfferHumanNegotiationParty extends AbstractNegotiationParty {
	...
	public void receiveMessage(AgentID sender, Action arguments) {
		...
		if (opponentAction instanceof Accept && sender != this.getPartyId()) {
			JOptionPane.showMessageDialog(null, "" + sender + " accepted the last offer.");
		}

		if (opponentAction instanceof EndNegotiation) {
			JOptionPane.showMessageDialog(null, "" + sender + " canceled the negotiation session");
		}
	}
	...
	public Action chooseAction(List<Class<? extends Action>> possibleActions) {
		...
		try {
			ui = new EnterBidDialog2(this, getPartyId(), null, true, (AdditiveUtilitySpace) utilitySpace,
					possibleActions.contains(Accept.class) ? mostRecentOffer : null);
