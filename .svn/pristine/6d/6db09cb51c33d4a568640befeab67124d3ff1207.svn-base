...
public class AgentFSEGA extends Agent {
	...
	public void init() {
		...
		opponentModel = new MyBayesianOpponentModel((AdditiveUtilitySpace) utilitySpace);
		...
		Collections.sort(leftBids, new ReverseBidComparator((AdditiveUtilitySpace) utilitySpace));
	}
		...
		try {
			...
			case OFFER:
				...
				if (myLastAction != null) {
					...
					if (utilitySpace.getUtility(opponentBid) * 1.03 >= utilitySpace
						...
						// next bid
						if (utilitySpace.getUtility(opponentBid) > utilitySpace.getUtility(nextBid)) {
							action = new Accept(getAgentID(), opponentBid);
						}
					}
				} else {
	...
	private Bid getSmartBid(int pTimeCase) {
		...
		try {
			minArrayUtility = utilitySpace.getUtility(leftBids.get(leftBids.size() - 1));
		} catch (Exception e) {
			e.printStackTrace();
		}
		...
		if (pTimeCase == 2) {
			lowerAcceptableUtilLimit = myBestUtil
					- (Math.exp(Math.log(SIGMA_MAX) / (0.05 * timeline.getTotalTime())) * elapsedTimeNorm);
		} else {
		...
		while ((myNextUtility > lowerAcceptableUtilLimit) && (myNextUtility <= minUtilAllowed)) {
			...
			if (pTimeCase == 0) // use behaviour 0
			{
				try // catch getUtility exceptions
				{
					...
					if (myNextUtility < utilitySpace.getUtility(((Offer) lastOponentAction).getBid())) {
