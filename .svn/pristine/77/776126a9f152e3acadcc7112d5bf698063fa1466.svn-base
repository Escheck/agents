...
public class Atlas extends Agent {
	...
	public Action chooseAction() {
		...
		try {
			...
			if (actionOfPartner instanceof Offer) {
				...
				time_scale = (((partner_last_bid_time - partner_prev_last_bid_time) + time_scale * partner_bid_count)
						/ (partner_bid_count + 1));
			...
			action = new Accept(getAgentID(), ((ActionWithBid) actionOfPartner).getBid()); // best
																							// guess
																							// if
																							// things
																							// go
	...
	private boolean isAcceptable(Bid partnerBid, double offeredUtilFromOpponent, double myOfferedUtil, double time)
			throws Exception {
		// ‘ŠŽè‚ÌŒø—p’l
		...
		if (!tension && false_best_agreement_bid != null && P >= utilitySpace.getUtility(false_best_agreement_bid)) {
			...
			double mean_point = get_point * ((partner_guard_num + 0.5) / (parameter + 1))
					+ lost_point * ((partner_attack_num + 0.5) / (parameter + 1));
	...
	private Bid getBid() throws Exception {
		...
		if (!tension) {
			...
			if (timeline.getTime() > (last_time - time_scale * 2) && best_partner_bid != null
			...
			if (timeline.getTime() > (last_time - time_scale * 1) && !tension && false_best_agreement_bid == null
					&& best_partner_bid != null
					&& utilitySpace.getUtility(best_partner_bid) > utilitySpace.getReservationValue()) {
				attack_flag = true;
				return best_partner_bid;
			}
			if (timeline.getTime() > (last_time - time_scale * 1) && !tension && best_partner_bid == null
					&& false_best_agreement_bid != null
					&& utilitySpace.getUtility(false_best_agreement_bid) > utilitySpace.getReservationValue()) {
				attack_flag = true;
				return false_best_agreement_bid;
			}
		}
		...
		if (bid == null || utilitySpace.getUtility(bid) < border_bid_utility
				|| (tension && utilitySpace.getUtility(bid) < utilitySpace.getUtility(max_bid))) {
			try {
				for (int i = 0; i < SA_ITERATION; i++) {
					if (tension) {
						...
						if (bid == null || (utilitySpace.getUtility(bid) < border_bid_utility
								|| (utilitySpace.getUtility(check_bid) < utilitySpace.getUtility(bid))
				...
				System.out.println("Problem with received bid(SA):" + e.getMessage() + ". cancelling bidding");
			}
		}
	...
	private void getBorderUtility() throws Exception {
		...
		// ‹«ŠE’l‚Ì�X�V
		border_bid_utility = timeline.getTime() * (1.0 - utilitySpace.getDiscountFactor())
		...
		if (tension) {
			...
			if (SAMPLE_NUM < partner_bid_count) {
				compensate = (prev_offeredUtilFromOpponent - partner_bid_utility_weighted_average)
						* prev_offeredUtilFromOpponent * (1.0 - utilitySpace.getReservationValue());
				...
				if (best_partner_bid != null) {
					time_compensate = timeline.getTime()
							...
							* utilitySpace.getUtility(best_partner_bid) * (1.0 - utilitySpace.getReservationValue());
				...
				if (timeline.getTime() > 0.90) {
					border_bid_utility = border_bid_utility - 10 * (timeline.getTime() - 0.90)
							* (border_bid_utility - utilitySpace.getUtility(best_partner_bid));
				}
			}
		}
		...
		do {
			...
			while (temperature > 0.0001) {
				...
				for (int i = 0; i < step_num; i++) {
					...
					next_b = next_b.putValue(issueNumber, new ValueInteger(next_v)); // ‹ß–T‚ÌBid‚ð‹�‚ß‚é
				}
	...
	private Bid getBestBidbyNear(Bid baseBid) throws Exception {
		...
		for (int i = 0; i < lim; i++) {
			...
			for (int j = min; j <= max; j++) {
				current_b = current_b.putValue(issueNumber, new ValueInteger(j)); // ‹ß–T‚ÌBid‚ð‹�‚ß‚é
				current_u = utilitySpace.getUtility(current_b);
			...
			if (is_agreement && last_tension) {
				...
				if (false_best_agreement_bid == null) {
					...
					double get_utility_true = (utilitySpace.getDiscountFactor()
							- false_agreement_average_utility_with_discount) * (sessionsTotal - (sessionNr));
					// ‘¹Ž¸�i‹­‹C‚ÅŽ¸”s‚µ‚½‚Æ‚«‚ÉŽ¸‚¤Œø—p’l�i1‰ñ•ª�j�j
					double lost_utility_true = utilitySpace.getReservationValueWithDiscount(1.0)
							- false_agreement_average_utility_with_discount;
					// Šú‘Ò’l(‘ŠŽè‚ÌŽp�¨‚Í‹­‹CorŽã‹C‚Å‚»‚ê‚¼‚ê‚ÌŠm—¦‚Í‰ß‹Ž‚ÌŒð�Â‚ðŽQ�Æ‚·‚é)
					double utility_true = get_utility_true * ((partner_guard_num) / (parameter))
							+ lost_utility_true * ((partner_attack_num) / (parameter));
					// Šú‘Ò’l‚É‹­‹C‚Å‚ÌŽ¸”s‚ÌŽÀ�Ñ’l‚ð”½‰f
					utility_true += (utilitySpace.getReservationValueWithDiscount(1.0)
							- false_agreement_average_utility_with_discount) * true_failure_num;
		...
		try {
			...
			if (isAgreement) {
				...
				if (tension) {
					...
					if (true_best_agreement_bid == null
							|| utilitySpace.getUtility(lastBid) >= utilitySpace.getUtility(true_best_agreement_bid)) {
					...
					if (!first_offer_decision_flag) {
						...
						if (attack_flag) {
							...
							if (false_best_agreement_bid != null) {
								...
								if (utilitySpace.getUtility(lastBid) < utilitySpace.getUtility(false_best_agreement_bid)
						...
						// �‡ˆÓ‚µ‚½�Å‘åŒø—p’lBid‚ð‹L˜^
						if (false_best_agreement_bid == null || utilitySpace.getUtility(lastBid) >= utilitySpace
						...
						if (utilitySpace.getUtilityWithDiscount(lastBid,
							...
							false_agreement_best_utility = utilitySpace.getUtilityWithDiscount(lastBid,
									timeline.getTime());
						}
						...
						false_agreement_average_utility_with_discount = ((false_agreement_average_utility_with_discount
								* false_agreement_num)
								+ utilitySpace.getUtilityWithDiscount(lastBid, timeline.getTime()))
								/ (false_agreement_num + 1);
						false_agreement_num++;

						// ‹­‹C••ˆóƒtƒ‰ƒO‚ÌŠm’è
						if (utilitySpace.getDiscountFactor() < utilitySpace.getUtilityWithDiscount(lastBid,
								timeline.getTime()) && utilitySpace.getDiscountFactor() < 1.0
								&& utilitySpace.getUtility(false_best_agreement_bid) < 1.0) {
			...
			MySessionData mySessionData = new MySessionData(tension, isAgreement, lastBid, max_bid,
					true_best_agreement_bid, false_best_agreement_bid, true_failure_num, false_agreement_num,
					false_agreement_average_utility_with_discount, partner_attack_num, partner_guard_num, last_time,
					timeline.getTime(), tension_true_seal_flag, true_repeat_time, false_repeat_time,
	...
	public String getDescription() {
		return "ANAC 2014 - Atlas (compatible with non-linear utility spaces)";
	}
}
...
class MySessionData implements Serializable {
	...
	public MySessionData(Boolean tension, Boolean isAgreement, Bid lastBid, Bid max_bid, Bid true_best_agreement_bid,
			Bid false_best_agreement_bid, int true_failure_num, int false_agreement_num,
			double false_agreement_average_utility_with_discount, double partner_attack_num, double partner_guard_num,
			double last_time, double last_bid_time, boolean tension_true_seal_flag, double true_repeat_time,
