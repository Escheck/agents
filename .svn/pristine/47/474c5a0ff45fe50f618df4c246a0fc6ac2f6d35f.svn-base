...
public class Yushu extends Agent {
	...
	public Action chooseAction() {
		...
		try {
			...
			try {
				...
				action = new Accept(getAgentID(), ((ActionWithBid) actionOfOpponent).getBid());
			}
	...
	public double getTargetUtility() {
		...
		if ((lefttime < rte * tround) & (this.opponentHistory.size() > 3) & (averopu < 0.75)) {
			Utility.MINIMUM_BID_UTILITY = Utility.MINIMUM_BID_UTILITY - (0.75 - averopu) / 2.5;
		}
		Utility.MINIMUM_BID_UTILITY = Math.max(0.50, Utility.MINIMUM_BID_UTILITY); // no
																					// less
																					// than
																					// 0.5
		double time = timeline.getTime();
		// if(time>0.5)
		Utility.MINIMUM_BID_UTILITY = Utility.MINIMUM_BID_UTILITY * (Math.min(0.75, averopu) / 3 + 0.75);
		Utility.MINIMUM_BID_UTILITY = Math.max(Utility.MINIMUM_BID_UTILITY, averopu);
		double targetuti = HPOSSIBLEU - (HPOSSIBLEU - Utility.MINIMUM_BID_UTILITY) * Math.pow(time, eagerness);
		...
		if (lefttime > 50 * tround)
			targetuti = Math.max(targetuti, opponentUtis.get(this.bestTenBids.getFirst()) * 1.001);
		if (((lefttime < 10 * tround) & (opponentUtis.get(this.bestTenBids.getFirst()) > targetuti * 0.95))
			...
			for (int i = this.myHistory.size() - 1; i >= this.myHistory.size() - length; i--) {
				if (this.myHistory.get(i).equals(this.opponentHistory.get(this.bestTenBids.getFirst()))) {
					offered = true;
				}
			}
			if (offered) {
				...
				for (int i = 0; i < bestTenBids.size(); i++)
					if (this.opponentUtis.get(bestTenBids.get(i)) >= newtargetuti)
	...
	private Bid getNextBid(double targetuti) throws Exception {
		...
		while (bidsIter.hasNext()) {
			...
			double vlowbound;
			if (roundleft > 30)
				vlowbound = Math.max(this.opponentUtis.get(bestTenBids.get(0)), targetuti);
			else
				vlowbound = 0.96 * targetuti;
			if ((utitemp > vlowbound) & (utitemp < 1.08 * targetuti))
				candidates.add(new BidDetails(tmpBid, utilitySpace.getUtility(tmpBid), timeline.getTime()));
		...
		if (this.myHistory.size() > 10) {
			candidates.add(new BidDetails(nextBid, utilitySpace.getUtility(nextBid), timeline.getTime()));
			if (TEST_EQUIVALENCE) {
			...
			if (uti > this.opponentUtis.get(bestTenBids.getFirst())) {
				...
				for (int i = 1; i < bestTenBids.size(); i++) {
					if ((uti <= this.opponentUtis.get(bestTenBids.get(i - 1)))
							& (uti > this.opponentUtis.get(bestTenBids.get(i)))) {
						for (int j = 0; j < i; j++)
							newlist.add(bestTenBids.get(j));
						newlist.add(index);
	...
	public double averLastTResT(int length) {
		if (resptimes.size() < length)
			return 0;
		double total = 0;
		for (int i = resptimes.size() - 1; i > resptimes.size() - length - 1; i--)
			total = total + resptimes.get(i);
		return total / length;
	}
