...
public class Group11 extends AbstractNegotiationParty {
	...
	public Action chooseAction(List<Class<? extends Action>> validActions) {
		...
		if (currentTime > timeTostartGiveInReservationValue) {
			...
			reservationUtility = startReservationUtility - (0.4 * giveInProgress);
			utilitySpace.setReservationValue(reservationUtility);
		}
		...
		if (currentTime > 0.95 || (lastBid != null && lastBid.getMyUndiscountedUtil() > reservationUtility)) {
			return new Accept(getPartyId(), lastBid.getBid());
		} else {
			// Short negotiation
			if (thereWillNeverBeATrustedOpponentModel()) {
				...
				if (weTrustOurOpponentModel()) {
					...
					if (unknownCounter >= modifyPreviousCounter && unknownCounter >= modifySelfCounter)
	...
	private Action getActionForTactic(Tactics t) {
		...
		case RANDOM:
			...
			List<BidDetails> randomBids = possibleBids
					.getBidsinRange(new Range(getUtilitySpace().getReservationValue(), 1));
			if (randomBids.size() == 0) {
				return getActionForTactic(Tactics.GIVEIN);
			} else {
				return bid(randomBids.get(new Random().nextInt(randomBids.size())).getBid());
			}
		case BESTNASH:
		...
		case EDGEPUSHER:
			...
			List<BidDetails> allBetterBids = possibleBids.getBidsinRange(new Range(lastUtil, 1));
		...
		case GIVEIN:
			if (getTime() > 0.95)
				return new Accept(getPartyId(), allBids.getLastBid());
			else {
				// double currentTime = getTime();
	...
	private double getTime() {
		if (getDeadlines() != null) {
			Integer d = getDeadlines().getType() == DeadlineType.ROUND ? getDeadlines().getValue() : 0;
	...
	public void receiveMessage(AgentID sender, Action action) {
		...
		try {
			...
			if (opponent == null) {
				opponent = new OpponentUtilityModel(getUtilitySpace().getDomain());
			...
			if (action instanceof Offer) {
				...
				System.out.println("WARNING :: UNKNOWN ACTION :: " + action.getClass().getCanonicalName());
			}
	...
	private void sortOutcomeSpaceOnNashProduct() {
		...
		for (BidDetails bd : bids) {
			nashes.add(new BidDetailsWithNash(bd.getBid(), getNashUtilityProduct(bd.getBid(), opponentModels)));
		}

		Collections.sort(nashes, new Comparator<BidDetailsWithNash>() {
			@Override
			public int compare(BidDetailsWithNash lbdwn, BidDetailsWithNash rbdwn) {
				// Big value (1000000000) is because else the values would be so
				// small that Java would throw a 'MisuseOfContractException'
				return (int) (2000000000 * (lbdwn.getEstimatedNashValue() - rbdwn.getEstimatedNashValue()));
			}
		});
	 ...
	 * @return Nash product
	 */
	private double getNashUtilityProduct(Bid b, ArrayList<OpponentUtilityModel> opponentModels) {
