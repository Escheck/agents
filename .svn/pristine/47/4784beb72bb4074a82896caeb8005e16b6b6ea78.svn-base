...
public class SENGOKU extends AbstractNegotiationParty {
	...
	public void init(NegotiationInfo info) {
		...
		strategy = new strategy((AdditiveUtilitySpace) utilitySpace, negotiatingInfo, bidSearch);
	}
	...
	public Action chooseAction(List<Class<? extends Action>> validActions) {
		if (isPrinting) {
			System.out.println("-----------------MyTern----------------------" + timeline.getTime() * 180);
		}
		...
		if (time > 0.05) {
			try {
				if (isPrinting) {
					System.out.println("有効値：" + utilitySpace.getUtility(offeredBid));
				}
			} catch (Exception e) {
	...
	public Action OfferAction() {
		Bid offerBid = bidSearch.shiftBidSerch(generateRandomBid(), strategy.getThreshold(timeline.getTime()));
	...
	public Action lastAction() {
		if (isPrinting) {
			System.out.println("ラストリストの数" + negotiatingInfo.getLastOfferBidNum());
		}
