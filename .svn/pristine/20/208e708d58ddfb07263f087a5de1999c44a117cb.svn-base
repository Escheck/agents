...
public class ParsAgent extends AbstractTimeDependentNegotiationParty {
				...
				if (lastAction instanceof Accept) {
					...
					if (utilitySpace.getUtility(lastBid) > getMyutility()) {
						...
						if (utilitySpace.getUtility(b) < getMyutility())
							return new Offer(getPartyId(), getMybestBid(utilitySpace.getMaxUtilityBid(), 0));
						else
							return new Offer(getPartyId(), b);
	...
	public void receiveMessage(AgentID sender, Action action) {
		...
		if (action != null && action instanceof Offer) {
			...
			if (withDiscount == null) {
				try {
					if (utilitySpace.getUtilityWithDiscount(newBid, timeline) != utilitySpace.getUtility(newBid)) {
			...
			if (bidNN == null || utilitySpace.getUtility(bidNN) < getMyutility()) {
				...
				for (int i = 0; i < isues.size(); ++i) {
					...
					if (dissue instanceof IssueDiscrete) {
						...
						if (keyVal != null) {
							map.put(new Integer(i + 1), (ValueInteger) keyVal.get(0));
						} else { // keyVal==null
							// IssueInteger is = (IssueInteger) dissues.get(i);
							map.put(i + 1, new ValueInteger(MyBestValue(i)));
						}
					} else {
						throw new IllegalStateException("not supported issue " + dissue);
					}
	 ...
	 * @param i
	 */
	private void updateMutualList(List<List<Object>> mutualList, List<Issue> dissues, int twocycle, int i) {
		if (oppAPreferences.getRepeatedissue().get(dissues.get(i).getName()) != null) {
			HashMap<Value, Integer> vals = oppAPreferences.getRepeatedissue().get(dissues.get(i).getName());
			HashMap<Value, Integer> valsB = oppBPreferences.getRepeatedissue().get(dissues.get(i).getName());
	...
	public void calculateParamForOpponent(OpponentPreferences op, Bid bid) {
		...
		for (int i = 0; i < dissues.size(); ++i) {
			if (op.getRepeatedissue().get(dissues.get(i).getName()) != null) {
				...
				HashMap<Value, Integer> vals = op.getRepeatedissue().get(dissues.get(i).getName());
