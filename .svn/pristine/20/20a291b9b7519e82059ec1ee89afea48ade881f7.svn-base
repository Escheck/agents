		...
		try {
			...
			if (actionOfPartner == null) {
				...
				if (offeredUtility >= threshold)// If the utility of the bid
					...
					((offeredUtility < this.utilitySpace.getReservationValueWithDiscount(timeline))
							&& (timeline.getTime() > 177.0 / 180.0))
							&& (this.utilitySpace.getReservationValueWithDiscount(timeline) > this.utilitySpace
									.getUtilityWithDiscount(bidToOffer, timeline))) {
