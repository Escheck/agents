	...
	private void updateLearner() {
		...
		for (Integer i : lastDiffSet.keySet()) {
			if (lastDiffSet.get(i) == 0 && oppUtility.getWeight(i) < maximumWeight)
				...
				oppUtility.setWeight(domain.getObjectivesRoot().getObjective(i), oppUtility.getWeight(i) / totalSum);
		}
		...
		try {
			for (Entry<Objective, Evaluator> e : oppUtility.getEvaluators()) {
				...
				((EvaluatorDiscrete) e.getValue()).setEvaluation(
						opponentLastBid.getValue(((IssueDiscrete) e.getKey()).getNumber()),
						(LEARNING_VALUE_ADDITION + ((EvaluatorDiscrete) e.getValue()).getEvaluationNotNormalized(
		...
		try {
			...
			if (offerQueue.isEmpty() || offerQueue == null) {
				...
				if (opponentLastBid != null
						&& utilitySpace.getUtility(bestBid1) <= utilitySpace.getUtility(opponentLastBid)) {
			...
			if (opponentLastBid != null && (utilitySpace.getUtility(opponentLastBid) > lowestYetUtility || utilitySpace
					.getUtility(offerQueue.getFirst().getValue()) <= utilitySpace.getUtility(opponentLastBid))) {
				newAction = new Accept(getAgentID(), opponentLastBid);
			}
