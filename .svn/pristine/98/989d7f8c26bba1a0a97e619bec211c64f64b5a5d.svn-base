...
public class Nozomi extends Agent {
	...
	public void init() {
		try {
			...
			minGap = (double) utilitySpace.getDomain().getIssues().size();
	...
	private Action chooseBidAction(Bid partnerBid) {
		...
		try {
			if (prevPartnerBid == null) {
				...
				bidNumber++;
				double prevPartnerUtility = utilitySpace.getUtility(prevPartnerBid);
				...
				if (time > 90 && maxCompromiseUtility - maxUtilityOfPartnerBid < 0.1) {
					maxCompromiseUtility = (maxCompromiseUtility * 7.0 + maxUtilityOfPartnerBid * 3.0) / 10.0;
				}
				...
				case APPROACH:
					...
					if (utilitySpace.getUtility(nextBid) < maxCompromiseUtility
							|| utilitySpace.getUtility(nextBid) > maxCompromiseUtility * 105 / 95) {
						nextBid = getBetweenBid(nextBid, partnerBid, maxCompromiseUtility,
				...
				if (utilitySpace.getUtility(nextBid) <= utilitySpace.getUtility(prevBid) / 2.0) {
					nextBid = getApproachBid(partnerBid);
				...
				if (((utilitySpace.getUtility(nextBid) < maxCompromiseUtility
						|| utilitySpace.getUtility(nextBid) > maxCompromiseUtility * 105 / 95) && continuousKeep < 20)
						|| utilitySpace.getUtility(nextBid) <= utilitySpace.getUtility(prevBid) / 2.0
						|| utilitySpace.getUtility(nextBid) < maxUtilityOfPartnerBid) {
				...
				if (utilitySpace.getUtility(nextBid) == utilitySpace.getUtility(prevBid)) {
					continuousKeep++;
				} else {
					continuousKeep = 0;
				}
	...
	private void checkCompromise(double time) {
		...
		if (compromise) {
			...
			if (partnerDiff < diff * 0.90 || gap > Math.pow(1.0 - time / 100, 2.0) * 0.90) {
			...
			if (partnerDiff > diff * 0.90 || (time > 50 && updateMaxPartnerUtility)) {
	...
	private double getCompromiseProbability() {
		...
		try {
			double prevUtility = utilitySpace.getUtility(prevBid);
			double prevUtilityOfPartnerBid = utilitySpace.getUtility(prevPartnerBid);
	...
	private Bid getCompromiseBid(Bid partnerBid) {
		...
		try {
			...
			double gap = Math.abs(prevUtility - utilitySpace.getUtility(partnerBid));
			...
			for (Issue compromiseIssue : issues) {
				...
				Bid candidateBid = new Bid(utilitySpace.getDomain(), candidateValues);
	...
	private Value getCompromiseValue(Issue issue, Value val, Value partnerVal) {
		...
		Evaluator eval = ((AdditiveUtilitySpace) utilitySpace).getEvaluator(issue.getNumber());
	...
	private Value getHigherValue(Issue issue, Value value, HashMap<Integer, Value> values) {
		...
		case INTEGER:
			...
			try {
				...
				Bid candidateBid = new Bid(utilitySpace.getDomain(), values);
				if (utilitySpace.getUtility(candidateBid) > utilitySpace.getUtility(bid)) {
	...
	private void updateRestoreBid(Bid nextBid) {
		...
		for (Issue issue : issues) {
			int issueID = issue.getNumber();
			Evaluator eval = ((AdditiveUtilitySpace) utilitySpace).getEvaluator(issueID);

			try {
				evalGap += Math.abs(eval.getEvaluation(((AdditiveUtilitySpace) utilitySpace), nextBid, issueID)
						- eval.getEvaluation(((AdditiveUtilitySpace) utilitySpace), maxUtilityPartnerBid, issueID));
		...
		if (evalGap < minGap) {
			...
			try {
				if (utilitySpace.getUtility(nextBid) > utilitySpace.getUtility(restoreBid)) {
					restoreBid = nextBid;
				}
			} catch (Exception e) {
				e.printStackTrace();
		...
		try {
			...
			if (offeredUtil > maxUtility * 0.95 || offeredUtil >= prevUtility) {
				...
				for (Issue issue : issues) {
					...
					try {
						evalGap += Math.abs(eval.getEvaluation(((AdditiveUtilitySpace) utilitySpace), prevBid, issueID)
								- eval.getEvaluation(((AdditiveUtilitySpace) utilitySpace), partnerBid, issueID));
					} catch (Exception e) {
						evalGap += 1.0;
					}
				...
				if (time < 0.50) {
					if (prevPartnerBid != null && offeredUtil > maxCompromiseUtility
					...
					if (prevPartnerBid != null && offeredUtil > maxCompromiseUtility * 0.95
					...
					if (prevPartnerBid != null && offeredUtil > maxCompromiseUtility * 0.90
							&& offeredUtil > maxUtilityOfPartnerBid * 0.95) {
						...
						for (Issue issue : issues) {
							...
							case REAL:
								IssueReal issueReal = (IssueReal) issue;
								ValueReal valueReal = (ValueReal) restoreBid.getValue(issueID);
								ValueReal partnerValueReal = (ValueReal) partnerBid.getValue(issueID);
								double gap = Math.abs(valueReal.getValue() - partnerValueReal.getValue());
								if (gap > (issueReal.getUpperBound() - issueReal.getLowerBound()) / 100) {
									restoreEvalGap += 0.5;
								...
								break;
							default:
								if (!restoreBid.getValue(issueID).equals(partnerBid.getValue(issueID))) {
									restoreEvalGap += 0.5;
								}
								break;
							}
							...
							try {
								restoreEvalGap += Math
										.abs(eval.getEvaluation(((AdditiveUtilitySpace) utilitySpace), prevBid, issueID)
												- eval.getEvaluation(((AdditiveUtilitySpace) utilitySpace), partnerBid,
														issueID));
							} catch (Exception e) {
								restoreEvalGap += 1.0;
							}
