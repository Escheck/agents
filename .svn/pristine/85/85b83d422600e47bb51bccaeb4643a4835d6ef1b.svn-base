...
public class PhoenixParty extends AbstractNegotiationParty {
	...
	public void init() {
		try {
			...
			fixedInputSize = (int) Math.min(utilitySpace.getDomain().getNumberOfPossibleBids(), fixedInputSize);
			if (utilitySpace.getDomain().getNumberOfPossibleBids() < 50) {
				if (utilitySpace.getDomain().getNumberOfPossibleBids() <= 5) {
					utilityDivisionSize = (int) utilitySpace.getDomain().getNumberOfPossibleBids() / 2;
				} else if (utilitySpace.getDomain().getNumberOfPossibleBids() <= 11) {
					utilityDivisionSize = (int) utilitySpace.getDomain().getNumberOfPossibleBids() / 2;
				} else {
					utilityDivisionSize = (int) utilitySpace.getDomain().getNumberOfPossibleBids() / 3;
				}
			} else {
				utilityDivisionSize = Math.min((int) (utilitySpace.getDomain().getNumberOfPossibleBids() / 15),
						utilityDivisionSize);
			}
			...
			estimatedMean = minBidUtility + (minBidUtility + maxBidUtility) / 3.0;
	...
	public double getTime() {
		if (cctime) {
			return (timeline.getCurrentTime() - startTime) * (Math.pow(2 - delta, 0.5)) / (180.0);
		}
	...
	public void receiveMessage(AgentID sender, Action arguments) {
		...
		if (arguments instanceof Offer) {
			...
			if (countOpp == lastCountOpp + 1) {
				...
				controlFreqFunc(opponentPreviousBid, -100 * Math.pow(getTime(), 0.5));
			}
	...
	public Action chooseAction(List<Class<? extends Action>> possibleActions) {
		...
		double compareU = 1;
		try {
			if (possibleActions.contains(Accept.class) || possibleActions.contains(Offer.class)) {
				...
				if ((myLastBid == null) || (opponentPreviousBid1 == null) || (opponentPreviousBid2 == null)) {
					myLastBid = maxBid;
					uTargetForAcceptance = maxBidUtility;
					uTargetForBidding = maxBidUtility;
					...
					if (debug && getTime() > 0.02) {
						try {
							OriOut = System.out;
							logTxt = new PrintStream(new FileOutputStream(fileName, true));

							System.setOut(logTxt);
						} catch (FileNotFoundException ex) {
						}
						System.out.println(Double.toString(getTrueTime()) + "," + Double.toString(uTargetForAcceptance)
								+ "," + Double.toString(bound) + "," + Edelta + "," + Predict(getTime()));
						System.setOut(OriOut);
					}
					if (opponentPreviousBidUtility >= uTargetForBidding || (getTime() > (delta * 0.6 + theta * 0.3)
							&& opponentPreviousBidUtility >= uTargetForAcceptance)) {
						currentAction = new Accept(getPartyId(), opponentPreviousBid);
						System.out.println("Accccccccccccccccccccccccccccccccccc");
					} else {
						double reservationValue = theta * Math.pow(delta, currentTime);
						if (theta > 0 && ((currentTime - myLastBidTime) * timeline.getTotalTime() > opponentNoOfferTime
								|| ((domain.getNumberOfPossibleBids() < 100
										&& currentTime > delta * (1.0 - theta + bestReceivedBidUtility2))
										&& (theta >= bestReceivedBidUtility2))
										&& theta >= 0.4 * (maxBidUtility - utilitySpace.getUtility(opponentFirstBid2))
							...
							if (reservationValue > utilitySpace.getUtility(bidToOffer)) {
								currentAction = new EndNegotiation(getPartyId());
							}
				...
				if (currentTime >= endTime && domain.getNumberOfPossibleBids() > 5) {
					if (opponentPreviousBidUtility > theta) {
						if (theta == 0 || opponentPreviousBidUtility > theta * 2 || myLastBidUtility
	...
	private void generateUtilitySamples(int m) {
		...
		for (int i = 0; i < m; i++) {
			utilitySamples[i] = minBidUtility + i * (maxBidUtility - minBidUtility) / (m * 1.0);
		}
	}
	...
	public Bid BiddingMethod() throws Exception {
		try {
			...
			if (domain.getNumberOfPossibleBids() > 6400) {
				allBids.clear();
				while (limits <= Math.max(domain.getNumberOfPossibleBids() / 10, 888)) {
			...
			if (utilityDivisionSize > 30) {
				...
				if (domain.getNumberOfPossibleBids() < 1000) {
					if (rand.nextInt(10) >= 3 + delta * Math.pow(6, 1 - getTime())) {
						indexToFindBids += rand.nextInt(Math.min(utilityDivisionSize - 1 - indexToFindBids,
								Math.abs(indexToRandBids - indexToFindBids)));
					}
				} else {
					if (rand.nextInt(11) >= 3 + delta * Math.pow(6, 1 - getTime())) {
						indexToFindBids += rand.nextInt(Math.min(utilityDivisionSize - 1 - indexToFindBids,
								Math.abs(indexToRandBids - indexToFindBids)));
					}
				}
			...
			for (Bid tempbid : tempBL) {
				if (changingTime < getTime()) {
					if (heuristicRank(tempbid) * (0.95 + 0.05 * rand.nextDouble()) >= rank) {
						bid = tempbid;
			...
			if (rand.nextDouble() > delta * Math.min((Math.pow(Math.min(getTime() + 0.1, 0.8), 0.3)), 0.5)) {
			...
			double timeToPlay = (delta * 1.0 - theta) * rand.nextDouble() + 0.1 * delta;
			double durationToPlay = trainingTimeSeperator * delta;
			if (getTime() >= timeToPlay && getTime() <= timeToPlay + durationToPlay) {
				bid = myLastBid;
			}
			return bid;
		} catch (Exception e) {
	...
	private double heuristicRank(Bid bid) {
		...
		HashMap<Integer, Value> compareBidValues = opponentPreviousBid1.getValues();
		HashMap<Integer, Value> compareBidValues2 = opponentBestOfferBid1.getValues();
		HashMap<Integer, Value> compareBidValues3 = opponentFirstBid1.getValues();
	...
	private void getTargetUtility() throws Exception {
		...
		rate *= (0.3 + uTargetForAcceptance - Edelta + (1 - Math.pow(1.0, 1.3)) - Math.pow(theta, 1.5));
		rate /= opponentConcession();
		double opponentBidDistance = Math.abs(Edelta - uTargetForAcceptance) * compaction2;
		uTargetForBidding = (R * Math.pow(3, -getTime() * rate) + Edelta * Math.abs(1 - opponentBidDistance) / (2.0))
				/ (Math.abs(1 - opponentBidDistance) / (2.0) + Math.pow(3, -getTime() * rate));

		double baseValue = (Math.max(Elow, Edelta) + Edelta / uncertainty + uncertainty * Elow)
				/ (1.0 + 1 / uncertainty + uncertainty);
		double diffToElow = 1.2 * Math.abs(uTargetForBidding - baseValue);
		if (delta <= 1) {
			double x = 2 * (1 - getTime()) * delta, y = delta, z = (2 * getTime()) / delta;
			double uTargetForBiddingF1 = diffToElow * Math.pow(5,
					-(Math.pow(getTime(), 0.6) * (2.3 - firstAcceptTime)) * Math.pow(2 - delta, getTime()));
			double uTargetForBiddingF2 = diffToElow
					* (1 / ((Math.pow((2 * getTime()) * (2.3 - firstAcceptTime), delta + theta / 3.0) + 0.05) * 20)
							+ 0.4 * delta);
			double uTargetForBiddingF3 = diffToElow
					* (1 / ((Math.log((20.0 / (delta + theta / 3.0)) * getTime() * (2.3 - firstAcceptTime) + 1.1)) * 10)
							+ 0.3 * delta);
			uTargetForBidding = baseValue
					+ (z * uTargetForBiddingF3 + y * uTargetForBiddingF2 + x * uTargetForBiddingF1) / (x + y + z);
		}
		if ((selfVar < 0.05 || varianceOfOpponentUtilities < 0.05) && timeline.getCurrentTime() % 2 == 1) {
	...
	private void updateParas() {
		try {
			...
			for (double u : maxUtilitiesAsInputData) {
				varianceOfOpponentUtilities += Math.pow(u - meanOfOpponentUtilities, 2);
			}
	...
	private double Predict(double queryTimePt) {
		try {
			...
			return Math.max(Math.min(res[0].get(0, 0), maxBidUtility), minBidUtility);
		} catch (Exception ex) {
	...
	private double discountedExpectation(double time) {
		...
		double predictedUtility = (uTargetForAcceptance * Math.pow(0.5, getTime()) * (2 + w)
				+ Math.pow(0.5, 1 - getTime()) * 3 * w * (1.0 / PredictUncertainty(time)) * Predict(time)
				+ 2 * w * meanOfOpponentUtilities)
				/ (2 * w + Math.pow(0.5, getTime()) * (2 + w)
						+ Math.pow(0.5, 1 - getTime()) * 3 * w * (1.0 / PredictUncertainty(time)));
		return predictedUtility * Math.pow(Math.pow(1.0 + (0.1 * theta + 0.05), 0.7), getTime());
	}
	...
	private double lowestExpectation() throws Exception {
		...
		Elow = uFIrst + (maxBidUtility - uFIrst) * Math.pow(delta, 0.5 + (0.1) * rand.nextDouble()) / 2.5;
		return Elow * Math.pow(0.9, Math.pow(getTrueTime(), 2.0));
	}

	private double RFunction(double time) {
		try {
			double Elow = lowestExpectation() * Math.pow(0.8 + (0.2 * ppp), Math.pow(time, Math.pow(1.0, 0.65)));
			if (time >= 1) {
				return maxBidUtility;
			}
			omega = 1.26;
			if (getTime() > 10 * trainingTimeSeperator && getTime() < 0.3) {
				double comparePoint = opponentConcession() * discountedExpectation(time);
				double disToPoint = Math.abs(R - comparePoint) / Math.abs(maxBidUtility - Math
						.min(utilitySpace.getUtility(opponentFirstBid1), utilitySpace.getUtility(opponentFirstBid2)));
				ppp = disToPoint;
				omega = 1.16 + 0.5 * (disToPoint);
			}
			omega = Math.min(omega, 1.35);
			double component1 = Math.abs(1 - Math.pow(time, 1 / Math.pow(Math.abs(0.5), beta)));
			double component2 = Math.cos((1 - Math.pow(1.0, 1.3) * Math.pow(compaction2, 0.5)) / (omega));
	...
	private void recordOffer(Offer offerToRecord) throws Exception {
		...
		if (countOpp == lastCountOpp + 1) {
			...
			opponentBestOfferUtility1 = utilitySpace.getUtility(opponentBestOfferBid1);
			...
			double utilityToRecord = utilitySpace.getUtility(bidToRecord);
			opponentBestOfferUtility2 = utilitySpace.getUtility(opponentBestOfferBid2);
		...
		if (opponentFirstBid1 != null && opponentBestOfferBid2 != null) {
			compaction2 = 1 / Math.abs(maxBidUtility
					- Math.min(utilitySpace.getUtility(opponentFirstBid1), utilitySpace.getUtility(opponentFirstBid2)));
		}
		if (currentTimeIndex != timeSamples.length - 1) {
			if (getTime() > timeSamples[currentTimeIndex + 1]) {
				if (getTime() > 5 * trainingTimeSeperator) {
					EdeltaPast[currentTimeIndex] = discountedExpectation(getTime() + trainingTimeSeperator);
				}
				currentTimeIndex++;
			}
		}
		if (countOpp == lastCountOpp + 2) {
			(saveInTermsOfTime[currentTimeIndex])
					.add(Math.min(opponentPreviousBidUtility1, opponentPreviousBidUtility2));
