...
public class XianFaAgent extends AbstractNegotiationParty {
	...
	public Action chooseAction(List<Class<? extends Action>> validActions) {
		...
		if (timeline.getTime() > 0.9) {
			if (startTiming == false) {
				...
				avgRdTime = (avgRdTime * (timeRds - 1)) + (timeline.getTime() - prevTime);
				...
				if (acceptable()) {
					...
					if (opponentABest != null) {
						if (utilitySpace.getUtility(opponentABest) > resValue) {
							if (utilitySpace.getUtility(opponentABest) > (1.03 * utilitySpace.getUtility(partnerBid))) {
								stat_offerHistBest++;
	...
	private Action chooseBid(Bid partnerBid) {
		...
		try {
			if (partnerBid != null) {
				if (utilitySpace.getUtility(partnerBid) >= utilitySpace.getUtility(nextBid)) {
					act = 1;
					return (new Accept(getPartyId(), partnerBid));
				}
			}
	...
	private void updateSelf() {
		...
		if (!opponentA.equals("") && !opponentB.equals("")) {
			...
			cRateA = 1 - (Math.min(0.95, ((double) bidSet.size() / totalA) * 1000));
			cRateB = 1 - (Math.min(0.95, ((double) bidSetB.size() / totalB) * 1000));
			...
			if (rounds % 200 == 0) {
				...
				threshold = threshold - ((Math.max(cRateA, cRateB) * 0.00515) * ((1 - t) * (1 - t) * (1 / discount)));
				threshold = threshold
						- ((Math.max(1 - sRateA, 1 - sRateB) * 0.0116) * ((1 - t) * (1 - t) * (1 / discount)));
			}
			// printConsole();
		}
		if (attitude == 2)
	...
	public void printConsole() {
		if (rounds % 2 == 0) {
			...
			for (int i = 0; i < shortList.size(); i++) {
				try {
					System.out.println(utilitySpace.getUtility(shortList.get(i)));
				} catch (Exception e) {
			...
			try {
				System.out.println("utility of partnerBid: " + utilitySpace.getUtility(partnerBid));
			} catch (Exception e) {
				e.printStackTrace();
			}
		...
		if (action instanceof Offer) {
			...
			if (myTurnPrev) {
				...
				try {
					...
					else if (utilitySpace.getUtility(opponentABest) < getUtilitySpace().getUtility(partnerBid)) {
						opponentABest = new Bid(partnerBid);
					}
				} catch (Exception e) {
					e.printStackTrace();
	...
	public int partition(ArrayList<Bid> list, int low, int high) throws Exception {
		int last_small;
		double pivot;
		int mid = (low + high) / 2;
	...
	public void initTree() throws Exception {
		...
		for (int curDepth = 0; curDepth < maxDepth; curDepth++) { // for this
			...
			if (utilitySpace.getDomain().getIssues().get(curDepth).getType() == ISSUETYPE.DISCRETE) {
				id = (IssueDiscrete) utilitySpace.getDomain().getIssues().get(curDepth);
				ed = (EvaluatorDiscrete) ((AdditiveUtilitySpace) utilitySpace).getEvaluator(curDepth + 1);
				for (int curMember = 0; curMember < tree.getSizeOfLevel(curDepth); curMember++) { // for
																									// all
																									// nodes
																									// in
																									// this
					...
					for (int i = 0; i < id.getNumberOfValues(); i++) { // for
																		...
																		// this
																		// depth
						Node node = new Node(id.getNumber(), id.getValue(i), curDepth,
								ed.getEvaluation(id.getValue(i)));
						tree.getMemberInLevel(curDepth, curMember).add(node);
						tree.addNodeInDepth(node, curDepth + 1);
					}
				}
			} else if (utilitySpace.getDomain().getIssues().get(curDepth).getType() == ISSUETYPE.INTEGER) {
				ii = (IssueInteger) utilitySpace.getDomain().getIssues().get(curDepth);
				for (int curMember = 0; curMember < tree.getSizeOfLevel(curDepth); curMember++) { // for
																									// all
																									// nodes
																									// in
																									// this
																									// depth
					Node node = new Node(ii.getNumber(), new ValueInteger(ii.getUpperBound()), curDepth, 1);
					tree.getMemberInLevel(curDepth, curMember).add(node);
					tree.addNodeInDepth(node, curDepth + 1);
				}
			}
	...
	private void createLists() throws Exception {
		...
		System.out.println("utility of starting bid is: " + utilitySpace.getUtility(startingBid));
	...
	public Bid offerBid() throws Exception {
		if (rounds > (3000 * supposedNumber) || timeline.getTime() > (0.124 * supposedNumber)) {
			supposedNumber++;
		}
	...
	public ArrayList<Bid> insertionSort(ArrayList<Bid> list, int n) throws Exception {
		for (int i = 1; i < n; i++) {
			for (int j = i; (j > 0); j--) {
				// System.out.println("j is: " + j);
				if (utilitySpace.getUtility(list.get(j)) > utilitySpace.getUtility(list.get(j - 1))) {
			...
			for (Issue iss : bid.getIssues()) {
				for (int i = 0; i < issuesA.size(); i++) {
					if (issuesA.get(i).getIssNr() == iss.getNumber()) {
						double s = issuesA.get(i).getVal(bid.getValue(iss.getNumber()));
						if (s > 1)
							s = 1;
						score = score + s;
						break;
			...
			for (Issue iss : bid.getIssues()) {
				for (int i = 0; i < issuesB.size(); i++) {
					if (issuesB.get(i).getIssNr() == iss.getNumber()) {
						double s = issuesB.get(i).getVal(bid.getValue(iss.getNumber()));
						if (s > 1)
							s = 1;
						score = score + s;
						break;
	...
	public void initOpp() {
		for (int i = 0; i < utilitySpace.getDomain().getIssues().size(); i++) {
			AIssue iss = new AIssue(utilitySpace.getDomain().getIssues().get(i).getNumber());
			if (utilitySpace.getDomain().getIssues().get(i).getType() == ISSUETYPE.DISCRETE) {
				IssueDiscrete id = (IssueDiscrete) utilitySpace.getDomain().getIssues().get(i);
				...
				IssueInteger ii = (IssueInteger) utilitySpace.getDomain().getIssues().get(i);
			...
			if (act == 0) {
				...
				for (int i = 0; i < bid.getIssues().size(); i++) {
					for (int j = 0; j < issuesA.size(); j++) {
						if (bid.getIssues().get(i).getNumber() == issuesA.get(j).getIssNr()) {
							if (bid.getIssues().get(i).getType() == ISSUETYPE.DISCRETE) {
								issuesA.get(j).setVal(bid.getValue(bid.getIssues().get(i).getNumber()), totalA, 0);
							} else {
								// ignore
							}
						}
				...
				for (int i = 0; i < bid.getIssues().size(); i++) {
					for (int j = 0; j < issuesA.size(); j++) {
						if (bid.getIssues().get(i).getNumber() == issuesA.get(j).getIssNr()) {
							if (bid.getIssues().get(i).getType() == ISSUETYPE.DISCRETE) {
								issuesA.get(j).setVal(bid.getValue(bid.getIssues().get(i).getNumber()), totalA, 1);
							} else {
								// ignore
							}
						}
			...
			if (act == 0) {
				...
				for (int i = 0; i < bid.getIssues().size(); i++) {
					for (int j = 0; j < issuesB.size(); j++) {
						if (bid.getIssues().get(i).getNumber() == issuesB.get(j).getIssNr()) {
							if (bid.getIssues().get(i).getType() == ISSUETYPE.DISCRETE) {
								issuesB.get(j).setVal(bid.getValue(bid.getIssues().get(i).getNumber()), totalB, 0);
							} else {
								// ignore
							}
						}
				...
				for (int i = 0; i < bid.getIssues().size(); i++) {
					for (int j = 0; j < issuesB.size(); j++) {
						if (bid.getIssues().get(i).getNumber() == issuesB.get(j).getIssNr()) {
							if (bid.getIssues().get(i).getType() == ISSUETYPE.DISCRETE) {
								issuesB.get(j).setVal(bid.getValue(bid.getIssues().get(i).getNumber()), totalB, 1);
							} else {
								// ignore
							}
						}
