...
public class AgentYK extends Agent {
			...
			if (actionOfPartner instanceof Offer) {
				...
				if (isAccept(offeredBid)) {
					...
					if (0 <= leftTime && leftTime < 2 && myMinBid != null) {
						...
						if (diffMyOpp <= 0.25
								|| utilitySpace.getUtility(myMinBid) / 2 <= utilitySpace.getUtility(oppMaxBid)) {
							myBidManager(oppMaxBid);
							if (utilitySpace.getReservationValue() >= utilitySpace.getUtility(oppMaxBid))
					...
					if (utilitySpace.getUtility(nextBid) < utilitySpace.getUtility(oppMaxBid))
					...
					if (utilitySpace.getReservationValue() >= utilitySpace.getUtility(myMinBid))
	...
	private void refreshHistory() throws Exception {
		if (now > coefRefresh * termRefresh
				|| (1 <= coefRefresh * termRefresh && coefRefresh * termRefresh < 1.2 && now > 0.9)
				|| opponentHistory.size() + preRefresh > 1 + Math.pow(logTCN, 2)) {
			List<BidDetails> oppList = new ArrayList<BidDetails>(
					opponentHistory.filterBetweenUtility(oppAverage, 1.0).getHistory());
			List<BidDetails> myList = new ArrayList<BidDetails>(
					myHistory.filterBetweenUtility(0.0, myAverage).getHistory());
			opponentBidElementHistory = new BidElementHistory();
			opponentPairBidElementHistory = new PairBidElementHistory();
			myBidElementHistory = new BidElementHistory();
			myPairBidElementHistory = new PairBidElementHistory();
	...
	private void opponentBidManager(Bid oppBid) throws Exception {
		...
		if (!opponentBidPool.contains(oppBid.toString())) {
			...
			if (opponentHistory.isEmpty()) {
				...
				while (bei.hasNext())
					opponentBidElementHistory.add(new BidElementDetails(bei.next(), now));
				while (pbei.hasNext())
					opponentPairBidElementHistory.add(new PairBidElementDetails(pbei.next(), now));
			} else {
				oppAverage = 0.0;
		...
		if (basedBE > 0 && basedPBE > 0 && oppAverage <= utilitySpace.getUtility(oppBid)) {
			...
			sum = 0;
			while (bei.hasNext())
				sum += myBidElementHistory.getWeightedAppearanceCount(bei.next());
			additional += ((double) sum / basedBE);
			pbei = new PairBidElementIterator(oppBid);
			sum = 0;
			while (pbei.hasNext())
				sum += myPairBidElementHistory.getWeightedAppearanceCount(pbei.next());
		...
		sum = 0;
		while (bei.hasNext())
			sum += opponentBidElementHistory.getWeightedAppearanceCount(bei.next());
		baseBE = sum;
		pbei = new PairBidElementIterator(oppMaxBid);
		sum = 0;
		while (pbei.hasNext())
			sum += opponentPairBidElementHistory.getWeightedAppearanceCount(pbei.next());
		basePBE = sum;
	}
	...
	private void myBidManager(Bid myBid) throws Exception {
		...
		if (!myBidPool.contains(myBid.toString())) {
			...
			if (myHistory.isEmpty()) {
				...
				while (bei.hasNext())
					myBidElementHistory.add(new BidElementDetails(bei.next(), now));
				while (pbei.hasNext())
					myPairBidElementHistory.add(new PairBidElementDetails(pbei.next(), now));
			} else {
				myAverage = 0.0;
		...
		for (BidDetails bd : myHistory.getHistory()) {
			...
			while (bei.hasNext())
				basedBE += myBidElementHistory.getWeightedAppearanceCount(bei.next());
			pbei = new PairBidElementIterator(bd.getBid());
			while (pbei.hasNext())
				basedPBE += myPairBidElementHistory.getWeightedAppearanceCount(pbei.next());
		}
	...
	private Bid nextBid(Bid bid, int changeNum) {
		...
		for (Integer issueNumber : changeIssueNrs) {
			...
			case DISCRETE:
				IssueDiscrete lIssueDiscrete = (IssueDiscrete) domain.getIssues().get(issueN);
				int optionIndex = randomnr.nextInt(lIssueDiscrete.getNumberOfValues());
				nextBid = nextBid.putValue(issueNumber, lIssueDiscrete.getValue(optionIndex));
				break;
			case REAL:
				IssueReal lIssueReal = (IssueReal) domain.getIssues().get(issueN);
				...
				break;
			case INTEGER:
				IssueInteger lIssueInteger = (IssueInteger) domain.getIssues().get(issueN);
				int optionIndex2 = lIssueInteger.getLowerBound()
						+ randomnr.nextInt(lIssueInteger.getUpperBound() - lIssueInteger.getLowerBound());
				nextBid = nextBid.putValue(issueNumber, new ValueInteger(optionIndex2));
				break;
			default:
			}
		}
	...
	private double evaluateBid(Bid bid) throws Exception {
		...
		if (baseBE > 0 && basePBE > 0) {
			...
			if (MINIMUM_BID_UTILITY <= eval && eval < noValueUtil)
				eval += (allowedAdditional < additional ? allowedAdditional : additional);
		}

		if (timeBonus && !myBidPool.contains(bid.toString()) && myBidPool.size() <= opponentBidPool.size())
			eval += now;
	...
	public String getDescription() {
		return "ANAC 2014 - AgentYK (compatible with non-linear utility spaces)";
	}
}
