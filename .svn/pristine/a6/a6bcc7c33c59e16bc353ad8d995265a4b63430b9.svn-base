...
public class AgentBuyogMain extends AbstractNegotiationParty {
	...
	private static final double alphaDefault = 0, betaDefault = 0, issueWeightsConstant = 0.3,
			issueValuesConstant = 100, minimumHistorySize = 50, learningTimeController = 1.3; // Discount
																								// is
																								// added
																								// to
																								// the
	...
	private static final double leniencyAdjuster = 1, domainWeightController = 1.75, timeConcessionController = 1.8;
	...
	public Action chooseAction(List<Class<? extends Action>> possibleActions) {
		...
		if (infoA != null && infoB != null && infoA.getAgentDifficulty() != null
			...
			minimumPoint = utilitySpace.getDiscountFactor() * difficultAgent.getAgentDifficulty();
		}

		double acceptanceThreshold = minimumPoint
				+ (1 - minimumPoint) * (1 - Math.pow(timeline.getTime(), timeConcessionController));

		if (remainingRounds <= 3) {
			acceptanceThreshold = acceptanceThreshold * lastSecondConcessionFactor;
		}
		...
		if (possibleActions.contains(Accept.class)) {
			if (mostRecentBidsUtility >= acceptanceThreshold && mostRecentBidsUtility >= bestAgreeableBidsUtility
					&& remainingRounds <= 3) {
		...
		if (bestAgreeableBidsUtility > acceptanceThreshold) {
			...
			List<BidDetails> bidsInWindow = sortedUtilitySpace.getBidsinRange(range);
	...
	private BidDetails findNearestBidToKalai(List<BidDetails> bidsInWindow, Double infoAKalai, Double infoBKalai) {
	...
	private Double getDistance(BidDetails bid, Double infoAKalai, Double infoBKalai) {
		try {
			return Math.sqrt((1 - infoA.getAgentDifficulty())
					* Math.pow(infoA.getOpponentUtilitySpace().getUtility(bid.getBid()) - infoAKalai, 2)
					+ (1 - infoB.getAgentDifficulty())
							* Math.pow(infoB.getOpponentUtilitySpace().getUtility(bid.getBid()) - infoBKalai, 2));
		} catch (Exception e) {
			e.printStackTrace();
		}
		return 0D;
	...
	private void updateCommonBids(OpponentInfo otherInfo, Bid bid) {
		...
		if (otherInfo.containsBid(bid)) {
			try {
				this.AandBscommonBids.add(new BidDetails(bid, utilitySpace.getUtility(bid)));
			} catch (Exception e) {
				e.printStackTrace();
			}
		}
	...
	private OpponentInfo getOpponentInfoObjectOfOther(Object sender) {
		...
		if (infoA != null && infoA.getAgentID().equals(sender.toString())) {
			return infoB;
		} else if (infoB != null && infoB.getAgentID().equals(sender.toString())) {
			return infoA;
		}
	...
	private void updateOpponentModel(OpponentInfo senderInfo) {
		...
		if (senderInfo.getAgentBidHistory().size() >= minimumHistorySize) {
			LearningFunction function = new LearningFunction(
					senderInfo.getAgentBidHistory().getHistory().get(0).getMyUndiscountedUtil());
			...
			for (int i = 0; i < senderInfo.getBestBids().size(); i++) {
				xData[i] = senderInfo.getBestBids().getHistory().get(i).getTime();
				yData[i] = senderInfo.getBestBids().getHistory().get(i).getMyUndiscountedUtil();
			...
			double slopeStandardScale = Math.pow(Math.E, alpha) * beta * Math.pow(timeline.getTime(), beta - 1);
			double slopeFromZeroToOne = Math.atan(slopeStandardScale) / (Math.PI / 2);
			double adjustedLeniency = slopeFromZeroToOne + slopeFromZeroToOne / leniencyAdjuster;
		...
		BidDetails opponentsSecondLastBid = opponentsBidHistory.getHistory().get(opponentsBidHistory.size() - 2);
		HashMap<Integer, Boolean> changed = determineDifference(senderInfo, opponentsSecondLastBid, opponentsLatestBid);
		...
		double goldenValue = issueWeightsConstant
				* (1 - (Math.pow(timeline.getTime(), learningTimeController + utilitySpace.getDiscountFactor())))
				/ numberOfIssues;
		...
		for (Integer issueNumber : changed.keySet()) {
			if (!changed.get(issueNumber) && opponentUtilitySpace.getWeight(issueNumber) < maximumWeight) {
				opponentUtilitySpace.setWeight(
						opponentUtilitySpace.getDomain().getObjectivesRoot().getObjective(issueNumber),
						(opponentUtilitySpace.getWeight(issueNumber) + goldenValue) / totalSum);
			} else {
				opponentUtilitySpace.setWeight(
						opponentUtilitySpace.getDomain().getObjectivesRoot().getObjective(issueNumber),
		...
		try {
			for (Entry<Objective, Evaluator> issueEvaluatorEntry : opponentUtilitySpace.getEvaluators()) {
				if (issueEvaluatorEntry.getKey() instanceof IssueDiscrete) {
					((EvaluatorDiscrete) issueEvaluatorEntry.getValue()).setEvaluation(
							opponentsLatestBid.getBid()
									.getValue(((IssueDiscrete) issueEvaluatorEntry.getKey()).getNumber()),
							(int) (issueValuesConstant
									* (1 - Math.pow(timeline.getTime(),
											learningTimeController + utilitySpace.getDiscountFactor()))
									+ ((EvaluatorDiscrete) issueEvaluatorEntry.getValue()).getEvaluationNotNormalized(
					...
					if (midPoint > opponentsLatestValueForIssueAsInteger) {
						double distanceFromMidPoint = midPoint - opponentsLatestValueForIssueAsInteger;
						...
						double newLowEndEvaluation = ((EvaluatorInteger) issueEvaluatorEntry.getValue())
								.getEvaluation(lowerBound)
								+ (issueValuesConstant / 10000) * normalizedDistanceFromMidPoint
										* (1 - Math.pow(timeline.getTime(),
												learningTimeController + utilitySpace.getDiscountFactor()));
						double highEndEvaluation = ((EvaluatorInteger) issueEvaluatorEntry.getValue())
								.getEvaluation(upperBound);
						...
						double distanceFromMidPoint = opponentsLatestValueForIssueAsInteger - midPoint + 1; // because
																											// midPoint
																											// is
																											// included
																											// and
		...
		try {
			...
			BidSpace bidSpace = new BidSpace(this.utilitySpace, opponentUtilitySpace, true);
			...
			if (kalaiPoint > senderInfo.getBestBids().getBestBidDetails().getMyUndiscountedUtil()) {
				...
				senderInfo
						.setDomainCompetitiveness(senderInfo.getBestBids().getBestBidDetails().getMyUndiscountedUtil());
			}

			if (senderInfo.getAgentBidHistory().size() >= minimumHistorySize) {
				// TODO Changeable formulas
				double domainWeight = (1 - Math.pow(senderInfo.getLeniency(), domainWeightController));
				double agentDifficulty = (1 - domainWeight) * senderInfo.getLeniency()
	...
	private HashMap<Integer, Boolean> determineDifference(OpponentInfo senderInfo, BidDetails first,
			BidDetails second) {
		HashMap<Integer, Boolean> changed = new HashMap<Integer, Boolean>();
		try {
			for (Issue i : senderInfo.getOpponentUtilitySpace().getDomain().getIssues()) {
				if (i instanceof IssueDiscrete) {
					changed.put(i.getNumber(), (((ValueDiscrete) first.getBid().getValue(i.getNumber()))
							.equals((ValueDiscrete) second.getBid().getValue(i.getNumber()))) ? false : true);
				} else if (i instanceof IssueInteger) {
					changed.put(i.getNumber(), (((ValueInteger) first.getBid().getValue(i.getNumber()))
							.equals((ValueInteger) second.getBid().getValue(i.getNumber()))) ? false : true);
				}
			}
		} catch (Exception ex) {
			ex.printStackTrace();
	...
	private void initializeOpponentInfo(Object sender) {
		if (infoA == null) {
			infoA = new OpponentInfo(sender.toString(), (AdditiveUtilitySpace) utilitySpace);
		} else if (infoB == null) {
			infoB = new OpponentInfo(sender.toString(), (AdditiveUtilitySpace) utilitySpace);
		}
