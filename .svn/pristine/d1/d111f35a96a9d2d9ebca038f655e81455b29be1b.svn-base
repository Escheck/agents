...
public class AgentM extends Agent {
	...
	public void ReceiveMessage(Action opponentAction) {
		...
		try {
			if (this.prevSessOppBidHistory.getLastBid() != null) {
				if (getUtility(this.prevSessOppBidHistory.getBestBidDetails().getBid()) < getUtility(
						this.currSessOppBidHistory.getLastBid())) {
					this.concessionRate = Math.pow(getUtility(this.prevSessOppBidHistory.getWorstBidDetails().getBid())
							- getUtility(this.currSessOppBidHistory.getBestBidDetails().getBid()), 2.0);
				}
			}
		} catch (Exception e) {
			e.printStackTrace();
	...
	private Bid getRandomBid() throws Exception {
		...
		if (this.bidmax == null) {
			...
			this.concessionRate = Math
					.pow(getUtility(this.currSessOppBidHistory.getWorstBidDetails().getBid())
							- getUtility(this.currSessOppBidHistory.getBestBidDetails().getBid()), 2.0)
					+ timeline.getTime() / 10;
			if (this.endbid == null) {
			...
			while (((i++ < NUMBER_ITERATIONS) && (maxUtility < 0.999 - this.concessionRate))
				...
				for (j = 0; j < (issues.size() / 5); j++) {
					...
					if (j % 2 == 0) {
						...
						ValueInteger aaa = (ValueInteger) this.currSessOppBidHistory.getBestBidDetails().getBid()
								.getValue(issueNumber);
						test_value = Integer.valueOf(aaa.toString());
					}
					nextBid = nextBid.putValue(issueNumber, new ValueInteger(test_value));
					issueValue = Integer.valueOf(lIssueValue.toString()).intValue();
					issueIndex = randomnr.nextInt(issues.size());
					lIssueInteger = (IssueInteger) issues.get(issueIndex);
					issueNumber = lIssueInteger.getNumber();
					ValueInteger bbb = (ValueInteger) this.currSessOppBidHistory.getBestBidDetails().getBid()
							.getValue(issueNumber);
		...
		BidDetails opponentBid = new BidDetails(bidmax, utilitySpace.getUtility(bidmax), timeline.getTime());
	...
	private Bid nextBid(Bid bid) throws Exception {
		...
		for (int i = 0; i < numberIndexes; i++) {
			...
			nextBid = nextBid.putValue(issueNumber, new ValueInteger(optionIndex)); // Œ»�Ý‚ÌBid‚©‚çIssue‚Ì’l‚ð“ü‚ê‘Ö‚¦‚é
		}
		return nextBid;
	}
	...
	public String getDescription() {
		return "ANAC 2014 - AgentM (compatible with non-linear utility spaces)";
	}
}
