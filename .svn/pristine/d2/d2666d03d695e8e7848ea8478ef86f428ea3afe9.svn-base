	...
	public void receiveMessage(AgentID sender, Action action) {
		...
		if (action != null) {
			if (action instanceof Inform && ((Inform) action).getName() == "NumberOfAgents"
				...
				if (isPrinting) {
					System.out.println("NumberofNegotiator:" + negotiationInfo.getNegotiatorNum());
				}
			}
			...
			if (action instanceof Offer) {
				...
				// 交渉情報を更新
				try {
					negotiationInfo.updateOfferedValueNum(sender, offeredBid, time);
					negotiationInfo.updateBidAcceptNum(sender, offeredBid, time);
