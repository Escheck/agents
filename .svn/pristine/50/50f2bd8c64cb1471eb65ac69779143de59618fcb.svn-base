...
public class FourierTransform extends Canvas implements Serializable {
	...
	private static final long serialVersionUID = 1L; // serial version unique
														// identifier
	...
	private int originalDataLength = 0; // original data length value; the
										...
										// deletion or padding
	private int fftDataLength = 0; // working data length - usually the smallest
									// power of two that is either equal to
									// originalDataLength or larger than
									// originalDataLength
	...
	private double[] fftData = null; // array to hold a data set of complex
										...
										// real and imaginary parts, e.g. real_0
										// imag_0, real_1 imag_1, for the fast
										// Fourier Transform method
	private double[] fftCorr = null; // corresponding array to hold the data to
										// be correlated with first data set
	private double[] fftResp = null; // corresponding array to hold the response
										// to be convolved with first data set
	...
	private int windowOption = 0; // Window Option
									// = 0; no windowing applied (default) -
									...
									// = 7; Gaussian
	// all window names
	private String[] windowNames = { "no windowing applied", "Rectangular (square, box-car)", "Bartlett (triangular)",
			"Welch", "Hann (Hanning)", "Hamming", "Kaiser", "Gaussian" };
	...
	private Complex[] transformedDataComplex = null; // transformed data set of
														// Complex numbers
	...
	private double[][] powerSpectrumEstimate = null; // first row - array to
														// hold frequencies
														// second row - array to
														// hold estimated power
														// density (psd)
														// spectrum
	private boolean powSpecDone = false; // = false - PowerSpectrum has not been
											// called
											// = true - PowerSpectrum has been
											// called
	private int psdNumberOfPoints = 0; // Number of points in the estimated
										// power spectrum

	private int segmentNumber = 1; // Number of segments into which the data has
									// been split
	...
	private boolean overlap = false; // Data segment overlap option
										// = true; overlap by half segment
										// length - smallest spectral variance
										// per data point
										// good where data already recorded and
										// data reduction is after the process
										// = false; no overlap - smallest
										// spectral variance per conputer
										// operation
										// good for real time data collection
										// where data reduction is computer
										// limited
	...
	private double[][] correlationArray = null; // first row - array to hold
												// time lags
												// second row - correlation
												// between fftDataWindow and
												// fftCorrWindow
	private boolean correlateDone = false; // = false - correlation has not been
											// called
											// = true - correlation has been
											// called

	private int numberOfWarnings = 9; // Number of warnings
	private boolean[] warning = new boolean[numberOfWarnings]; // warnings - if
																// warning[x] =
																// true
																// warningText[x]
																// is printed
	...
	private int plotLineOption = 0; // PlotPowerSpectrum line option
									// = 0 points linked by straight line
									// [default option]
	...
	private double[][] timeFrequency = null; // matrix of time against frequency
												// mean square powers from
												// shoert time FT
												// first row = blank cell
												// followed by time vector
												// first column = blank cell
												// followed by frequency vector
												// each cell is then the mean
												// square amplitude at that
												// frequency and time
	...
	public FourierTransform(double[] realData) {
		...
		for (int i = this.originalDataLength; i < this.fftDataLength; i++)
			this.complexData[i].reset(0.0D, 0.0D);
	...
	public void setFftData(double[] fftdata) {
		...
		this.fftDataLength = FourierTransform.nextPowerOfTwo(this.originalDataLength);
		this.fftData = new double[2 * this.fftDataLength];
		for (int i = 0; i < 2 * this.originalDataLength; i++)
			this.fftData[i] = fftdata[i];
		for (int i = 2 * this.originalDataLength; i < 2 * this.fftDataLength; i++)
			this.fftData[i] = 0.0D;
		this.fftDataSet = true;
	...
	private double windowData(double[] data, double[] window, double[] weight) {
		...
		case 6:
			...
			double numer = 0.0D;
			for (int i = 0; i <= n; i++) {
				numer = FourierTransform
						.modBesselIo(Math.PI * this.kaiserAlpha * Math.sqrt(1.0D - Fmath.square(2.0D * i / n - 1.0D)));
	...
	public static double modBesselIo(double arg) {
		...
		if ((absArg = Math.abs(arg)) < 3.75) {
			poly = arg / 3.75;
			poly *= poly;
			bessel = 1.0D + poly * (3.5156229D + poly * (3.08989424D
					+ poly * (1.2067492D + poly * (0.2659732 + poly * (0.360768e-1 + poly * 0.45813e-2)))));
		} else {
			bessel = (Math.exp(absArg) / Math.sqrt(absArg)) * (0.39894228D + poly * (0.1328592e-1D
					+ poly * (0.225319e-2 + poly * (-0.157565e-2 + poly * (0.916281e-2 + poly * (-0.2057706e-1
							+ poly * (0.2635537e-1 + poly * (-0.1647633e-1 + poly * 0.392377e-2))))))));
		}
		return bessel;
	}
	...
	private void printWarnings(FileOutput fout) {
		...
		if (warning[7]) {
			...
			fout.print("The data has been split into " + this.segmentNumber + " segment/s of length "
					+ this.segmentLength);
			if (this.overlap) {
		...
		if (warning[8]) {
			fout.println("WARNING!");
			fout.println("Data length was not an integer power of two");
			fout.println("Data has been padded with " + (this.fftDataLength - this.originalDataLength)
	...
	// set overlap option - see above (head of program comment lines) for option
	// description
	...
	public void transform() {
		...
		if (this.originalDataLength != this.fftDataLength) {
			System.out.println("Fast Fourier Transform data length ," + this.originalDataLength
	...
	public void inverse() {
		...
		for (int i = 0; i < this.fftDataLength * 2; i++)
			this.transformedDataFft[i] = hold[i] / this.fftDataLength;
	...
	// the method checkPowerOfTwo() may be used to check this.
	...
	public double[][] powerSpectrum() {
		...
		if (!overlap && this.segmentNumber < 2) {
			...
			// fill transformed data arrays
			for (int i = 0; i < this.fftDataLength; i++) {
				this.transformedDataComplex[i].reset(this.transformedDataFft[2 * i],
						this.transformedDataFft[2 * i + 1]);
			}
			...
			for (int i = 1; i < this.psdNumberOfPoints; i++) {
				this.powerSpectrumEstimate[1][i] = Fmath.square(hold[2 * i]) + Fmath.square(hold[2 * i + 1])
						+ Fmath.square(hold[2 * this.segmentLength - 2 * i])
						+ Fmath.square(hold[2 * this.segmentLength - 2 * i + 1]);
			}

			// Normalise
			for (int i = 0; i < this.psdNumberOfPoints; i++) {
				this.powerSpectrumEstimate[1][i] = 2.0D * this.powerSpectrumEstimate[1][i]
						/ (this.fftDataLength * this.sumOfSquaredWeights);
			}
	...
	public double[][] powerSpectrum(String fileName) {
		...
		if (!FourierTransform.checkPowerOfTwo(this.segmentLength))
			throw new IllegalArgumentException("Fast Fourier Transform segment length ," + this.segmentLength
					+ ", is not an integer power of two");
	...
	private double[][] powerSpectrumSeg() {
		...
		double[] data = new double[2 * this.segmentLength]; // holds data and
															// transformed data
															// for working
															// segment
		this.psdNumberOfPoints = this.segmentLength / 2; // number of PSD points
		double[] segPSD = new double[this.psdNumberOfPoints]; // holds psd for
																// working
																// segment
		...
		for (int i = 1; i <= this.segmentNumber; i++) {
			...
			for (int j = 1; j < this.psdNumberOfPoints; j++) {
				segPSD[j] = Fmath.square(data[2 * j]) + Fmath.square(data[2 * j + 1])
						+ Fmath.square(data[2 * this.segmentLength - 2 * j])
						+ Fmath.square(data[2 * this.segmentLength - 2 * j + 1]);
			}
	...
	private double[][] powerSpectrumSeg(FileInput fin) {
		...
		double[] data = new double[2 * this.segmentLength]; // holds data and
															...
															// for working
															// segment
		this.weights = new double[this.segmentLength]; // windowing weights for
														// segment
	...
	public void printPowerSpectrum(String filename) {
		...
		if (this.windowOption == 6)
			title += ", alpha = " + this.kaiserAlpha;
		if (this.windowOption == 7)
			title += ", alpha = " + this.gaussianAlpha;
		fout.println(title);
	...
	// Display a log plot of the power spectrum within a defined frequency
	// window
	// graph title provided
	...
	private void plotPowerDisplay(double[][] spectrum, int low, int high, String graphTitle, String yLegend) {
		...
		if (this.windowOption == 6)
			graphTitle2 += " - alpha = " + this.kaiserAlpha;
		if (this.windowOption == 7)
			graphTitle2 += " - alpha = " + this.gaussianAlpha;
	...
	private double[][] correlation(int nLen) {
		...
		for (int i = 0; i < 2 * nLen; i++)
			this.transformedDataFft[i] = hold3[i];
	...
	public double[][] shortTime(double windowTime) {
		...
		if (!this.checkPowerOfTwo(windowLength)) {
			...
			System.out.println("Window length, provided as time, " + windowTime
	...
	public double[][] shortTime(int windowLength) {
		...
		if (!FourierTransform.checkPowerOfTwo(windowLength))
			throw new IllegalArgumentException(
					"Moving window data length ," + windowLength + ", is not an integer power of two");
		if (!this.fftDataSet)
			throw new IllegalArgumentException("No data has been entered for the Fast Fourier Transform");
		if (windowLength > this.originalDataLength)
			throw new IllegalArgumentException("The window length, " + windowLength
					+ ", is greater than the data length, " + this.originalDataLength + ".");
		...
		double[] data = new double[2 * windowLength]; // holds data and
														...
														// working window
		double[] winPSD = new double[this.numShortFreq]; // holds psd for
															// working window
		int isign = 1;
	...
	public void printShortTime(String filename) {
		if (!this.shortTimeDone) {
			...
			// Average points if output would be greater than a text file line
			// length
	...
	// graph method for plotShortTime short time Fourier Transform as a contour
	// plot
	public void graph(Graphics g) {
		...
		// Check and set parameters in case need to average or expand to match
		// fixed x-axis pixels
		...
		String totalTime = "Total time = " + (Fmath.truncate((xLen * (times[1] - times[0])) / pixelsPerXpoint, 3))
				+ " time units";
		g.drawString(totalTime, xBot, yBot + 80);

		String totalFreq = "Frequecy range = 0 to "
				+ (Fmath.truncate((yLen * (freqs[1] - freqs[0])) / pixelsPerYpoint, 3)) + " cycles per time unit";
		g.drawString(totalFreq, xBot, yBot + 100);

		g.drawString("Widow length = " + Fmath.truncate((this.segmentLength - 1) * this.deltaT, 3) + " time units",
				xBot, yBot + 120);
		String filter = "Window filter = " + this.windowNames[this.windowOption];
		if (this.windowOption == 6)
			filter += ", alpha = " + this.kaiserAlpha;
		if (this.windowOption == 7)
			filter += ", alpha = " + this.gaussianAlpha;
