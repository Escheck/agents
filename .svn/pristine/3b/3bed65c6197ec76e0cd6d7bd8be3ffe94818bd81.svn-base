...
public class AgentMR extends Agent {
	...
	public Action chooseAction() {
		...
		try {
			...
			if (actionOfPartner instanceof Offer) {
				...
				double offeredutil;
				if (discountFactor) {
					offeredutil = getUtility(partnerBid) * (1 / Math.pow(utilitySpace.getDiscountFactor(), time));
				} else {
				...
				if ((P > MINIMUM_ACCEPT_P) || (offeredutil > minimumBidUtility) || bidRunk.contains(partnerBid)) {
					...
					if (offereMaxUtility > minimumBidUtility) {
						...
						if (offereMaxUtility > reservation) {
							...
							action = new Offer(getAgentID(), bidRunk.get(bidRunk.size() - lastBidNumber));
						...
						if (offeredutil > minimumOffereDutil) {
							...
							action = new Offer(getAgentID(), bidRunk.get(currentBidNumber));
	...
	private void updateMinimumBidUtility(double time) {
		...
		minimumBidUtility = 1 - (1 / (1 + Math.exp(sigmoidGain * (time - sigmoidX))));
		...
		for (Issue lIssue : issues) {
			...
			case DISCRETE:
				...
				for (ValueDiscrete value : lIssueDiscrete.getValues()) {
					...
					if ((utility > minimumBidUtility) && (!bidRunk.contains(standardBid))) {
