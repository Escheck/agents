...
public class ParsCat extends AbstractNegotiationParty {
	...
	public void init(NegotiationInfo info) {
		...
		TimeLineInfo = info.getTimeline(); // read time line info
	...
	public Action chooseAction(List<Class<? extends Action>> validActions) {
		...
		try {
			...
			if (OtherAgentsBidHistory.getLastBid() == myBid) {
				return new Accept(getPartyId(), OtherAgentsBidHistory.getLastBid());
			} else {
	...
	private Bid getRandomBid() throws Exception {
		...
		while (counter == 1000) {
			...
			while (((getUtility(bid) < (tresh - xxx)) || (getUtility(bid) > (tresh + xxx))) && (counter < 1000));
			check = check + .01;
		}
		...
		if ((getUtility(bid) < getUtility(OtherAgentsBidHistory.getBestBidDetails().getBid()))
	...
	public void receiveMessage(AgentID sender, Action action) {
		...
		if (action instanceof Offer) {
			...
			try {
				BidDetails opponentBid = new BidDetails(bid, utilSpace.getUtility(bid), TimeLineInfo.getTime());
				u2 = utilSpace.getUtility(bid);
				t1 = TimeLineInfo.getTime();
				OtherAgentsBidHistory.add(opponentBid);

			} catch (Exception e) {
				EndNegotiationWithAnOffer end = new EndNegotiationWithAnOffer(this.getPartyId(), maxBid);
			}
		}
	}
