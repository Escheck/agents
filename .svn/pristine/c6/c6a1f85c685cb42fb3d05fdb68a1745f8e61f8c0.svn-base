...
public class Ngent extends AbstractNegotiationParty {
	...
	private class AgentData {
		...
		private AgentData(double minimumUtility, double initialConcedeTime, double initialUtilitythreshold,
				String agentNumber) {
		...
		private void ChangeIssueScore(Bid bid, String caseName) {
			try {
				...
				if (caseName.equals(string_AddIssueScore)) {
					...
					addedScore = -(1 - Math.exp(-timeline.getTime() / discountingFactor * 50));
				}
				// if(debug)
				// {
				...
				for (Issue lIssue : Issues) {
					...
					case DISCRETE:
						...
						ValueDiscrete value = (ValueDiscrete) bid.getValue(lIssue.getNumber());
						int index = lIssueDiscrete.getValueIndex(value);
						issueScore.get(issueNum - 1).set(index, issueScore.get(issueNum - 1).get(index) + addedScore);
						break;
					case REAL:
						IssueReal lIssueReal = (IssueReal) lIssue;
						ValueReal valueReal = (ValueReal) bid.getValue(lIssue.getNumber());
						int indexReal = (int) ((valueReal.getValue() - lIssueReal.getLowerBound())
						...
						issueScore.get(issueNum - 1).set(indexReal,
								issueScore.get(issueNum - 1).get(indexReal) + addedScore);
						break;
					case INTEGER:
						IssueInteger lIssueInteger = (IssueInteger) lIssue;
						ValueInteger valueInteger = (ValueInteger) bid.getValue(lIssue.getNumber());
						int indexInteger = valueInteger.getValue();
						issueScore.get(issueNum - 1).set(indexInteger,
								issueScore.get(issueNum - 1).get(indexInteger) + addedScore);
						break;
					default:
				...
				System.out.println(e.getMessage() + "exception in method changeIssueScore");
			}
		}
			...
			try {
				for (Issue Issue1 : Issues) {
					...
					case DISCRETE:
						...
						ValueDiscrete value1 = (ValueDiscrete) Bid1.getValue(issueNum);
						int index = lIssueDiscrete.getValueIndex(value1);
						if (domainSize > largeDomainSize) {
							score += Math.log(issueScore.get(lIssueDiscrete.getNumber() - 1).get(index));
						} else {
							score += issueScore.get(lIssueDiscrete.getNumber() - 1).get(index);
						}
						break;

					case REAL:
						IssueReal lIssueReal = (IssueReal) Issue1;
						ValueReal valueReal = (ValueReal) Bid1.getValue(issueNum);
						int indexReal = (int) ((valueReal.getValue() - lIssueReal.getLowerBound())
								/ (lIssueReal.getUpperBound() - lIssueReal.getLowerBound())
								* lIssueReal.getNumberOfDiscretizationSteps());
						if (domainSize > largeDomainSize) {
							score += Math.log(issueScore.get(lIssueReal.getNumber() - 1).get(indexReal));
						} else {
							score += issueScore.get(lIssueReal.getNumber() - 1).get(indexReal);
						}
						break;
					case INTEGER:
						IssueInteger lIssueInteger = (IssueInteger) Issue1;
						ValueInteger valueInteger = (ValueInteger) Bid1.getValue(issueNum);
						int indexInteger = valueInteger.getValue();
						if (domainSize > largeDomainSize) {
							score += Math.log(issueScore.get(lIssueInteger.getNumber() - 1).get(indexInteger));
						} else {
							score += issueScore.get(lIssueInteger.getNumber() - 1).get(indexInteger);
						}
						break;
					}
				}
				...
				System.out.println(e.getMessage() + " Exception in method calculateScoreValue");
				return 0;
			}
			return score;
		}
		...
		private void calculateBidScore(Bid Bid1) {
			try {
				...
				System.out.println(e.getMessage() + " Exception in method calculateBidScore");
			}
		}
	}
	...
	public void init(NegotiationInfo info) {
		...
		try {
			...
			this.agentData1 = new AgentData(this.reservationValue, this.initialConcedeTime,
					this.initialUtilitythreshold, this.string_agentData1Num);
			this.agentData2 = new AgentData(this.reservationValue, this.initialConcedeTime,
					this.initialUtilitythreshold, this.string_agentData2Num);
			this.countOpp = 0;
			...
			this.offeredBidsSD = (this.MaximumUtility - this.utilitySpace.getUtility(this.bid_minimum_utility)) / 4;
			...
			for (Issue lIssue : this.Issues) {
				...
				case REAL:
					IssueReal lIssueReal = (IssueReal) lIssue;
					for (int i = 0; i < lIssueReal.getNumberOfDiscretizationSteps(); i++) {
						templist1.add(i, Double.valueOf(1));
						templist2.add(i, Double.valueOf(1));
					}
					this.agentData1.issueScore.add(templist1);
				...
				case INTEGER:
					IssueInteger lIssueInteger = (IssueInteger) lIssue;
					for (int i = 0; i < lIssueInteger.getUpperBound() - lIssueInteger.getLowerBound() + 1; i++) {
						templist1.add(i, Double.valueOf(1));
						templist2.add(i, Double.valueOf(1));
					}
					this.agentData1.issueScore.add(templist1);
	...
	public Action chooseAction(List<Class<? extends Action>> possibleActions) {
		...
		try {
			...
			// else if (ActionOfOpponent instanceof Offer)
				...
				this.opponentBidHistory1.updateOpponentModel(this.agentData1.oppPreviousBid, utilitySpace.getDomain(),
						this.utilitySpace);
				this.opponentBidHistory2.updateOpponentModel(this.agentData2.oppPreviousBid, utilitySpace.getDomain(),
						this.utilitySpace);
				...
				if (estimateRoundLeft(true) > 10) {
					...
					if (IsAccept && !IsTerminate) {
						...
						if (this.concedeToOpponent == true) {
							...
							if (this.utilitySpace.getUtility(bid1) > this.utilitySpace.getUtility(bid2)) {
								bid = bid2;
							} else if (this.utilitySpace.getUtility(bid1) == this.utilitySpace.getUtility(bid2)
									&& Math.random() < 0.5) {
								bid = bid2;
							} else {
								bid = bid1;
					...
					if (timeline.getTime() > 0.99 && estimateRoundLeft(true) < 5) {
	...
	public void receiveMessage(AgentID sender, Action opponentAction) {
		...
		try {
																...
																// w.r.t. us
				{
					...
					if ((this.ActionOfOpponent instanceof Offer)) {
						...
						if (!this.agentData1.GetIsReceivedOppFirstBid()) {
							this.agentData1.oppFirstBid = ((Offer) this.ActionOfOpponent).getBid();
						...
						this.agentData1.oppPreviousBid = ((Offer) this.ActionOfOpponent).getBid();
						this.agentData1.ChangeIssueScore(this.agentData1.oppPreviousBid, this.string_AddIssueScore);
						UpdateOfferedBidsStat(this.utilitySpace.getUtility(this.agentData1.oppPreviousBid));
																		...
																		// us
				{
					...
					if ((this.ActionOfOpponent instanceof Offer)) {
						...
						if (!this.agentData2.GetIsReceivedOppFirstBid()) {
							this.agentData2.oppFirstBid = ((Offer) this.ActionOfOpponent).getBid();
						...
						this.agentData2.oppPreviousBid = ((Offer) this.ActionOfOpponent).getBid();
						this.agentData2.ChangeIssueScore(this.agentData2.oppPreviousBid, this.string_AddIssueScore);
						UpdateOfferedBidsStat(this.utilitySpace.getUtility(this.agentData2.oppPreviousBid));
					} else if ((this.ActionOfOpponent instanceof Accept)) {
			...
			System.out.println(e.getMessage() + " exception in method receiveMessage");
		}
	}
	...
	private void UpdateOfferedBidsStat(double bidU) {
		try {
			...
			// }
			this.offeredBidsAvg = (bidU - this.offeredBidsAvg) / newOfferedBidsRound + this.offeredBidsAvg;
			this.offeredBidsSD = Math.sqrt((this.offeredBidsRound * this.offeredBidsSD * this.offeredBidsSD
					+ (bidU - this.offeredBidsAvg) * (bidU - this.offeredBidsAvg)) / newOfferedBidsRound);
			...
			System.out.println(e.getMessage() + " exception in method UpdateOfferedBidsStat");
		}
	}
		...
		try {
			...
			if (currentUtility >= this.avgUtilitythreshold || currentUtility >= nextRoundUtility) {
				...
				double currentMaximumUtility1 = this.utilitySpace
						.getUtilityWithDiscount(opponentBidHistory1.getBestBidInHistory(), timeline);
				double currentMaximumUtility2 = this.utilitySpace
						.getUtilityWithDiscount(opponentBidHistory2.getBestBidInHistory(), timeline);
			...
			System.out.println(e.getMessage() + " Exception in method AcceptOpponentOffer");
			return true;
		}
	...
	private boolean TerminateCurrentNegotiation(Bid ownBid) {
		...
		try {
			...
			System.out.println(e.getMessage() + " Exception in method TerminateCurrentNegotiation");
			return true;
		}
	}
	...
	private void calculateBidsBetweenUtility() {
		try {
			this.MinimumUtility = Math.min(this.agentData1.MinimumUtility, this.agentData2.MinimumUtility);
			...
			while (myBidIterator.hasNext()) {
				...
				for (int i = 0; i < this.NumberofDivisions; i++) {
					if (this.utilitySpace.getUtility(b) <= (i + 1) * this.SpacebetweenDivisions + this.MinimumUtility
							&& this.utilitySpace.getUtility(b) >= i * this.SpacebetweenDivisions
									+ this.MinimumUtility) {
			...
			System.out.println(e.getMessage() + " Exception in method calculateBidsBetweenUtility");
		}
	}
	...
	private LinkedList<Bid> calculateBidsAboveUtility(double bidutil) {
		...
		try {
			...
			System.out.println(e.getMessage() + " Exception in method calculateBidsAboveUtility");
		}
		return Bids;
	}
	...
	private Bid RandomSearchBid() throws Exception {
		...
		for (Issue lIssue : issues) {
			...
			default:
				throw new Exception(
						"issue type " + lIssue.getType() + " not supported. Exception in method RandomSearchBid");
			}
		}
	...
	private List<Bid> getBidsBetweenUtility(double lowerBound, double upperBound) {
		...
		try {
			...
			System.out.println(e.getMessage() + " Exception in method getBidsBetweenUtility");
		}
		return bidsInRange;
	}
	...
	private void chooseConcedeToDiscountingDegree() {
		try {
			...
			System.out.println(e.getMessage() + " Exception in method chooseConcedeToDiscountingDegree");
		}
	}
	...
	private Bid regeneratebid(double bidUtil) {
		...
		try {
			...
			for (this.CurrentrangeNumber = this.NumberofDivisions - 1; check == 0; this.CurrentrangeNumber--) {
				...
				Bids.addAll(this.bidsBetweenUtility.get(this.CurrentrangeNumber));
			}
			// }
			// else
			// {
			...
			if (Bids.isEmpty()) {
				...
				if (Bids.isEmpty()) {
					...
					for (int i = 0; i < Bids.size(); i++) {
						...
						Bid bid = Bids.get(i);
						double agent1BidScore = this.agentData1.issueScoreBids.get(i) / this.agentData1.maxScore;
						double agent2BidScore = this.agentData2.issueScoreBids.get(i) / this.agentData2.maxScore;
						double ourBidScore = this.utilitySpace.getUtility(bid);
						dist = (1 - agent1BidScore) * (1 - agent1BidScore) + (1 - agent2BidScore) * (1 - agent2BidScore)
						...
						if (dist < min) {
							...
							// }
								...
								&& this.utilitySpace.getUtility(bid) > this.utilitySpace.getUtility(ans)) {
							ans = bid;
						}
					}

					if (Math.random() > 0.95) {
						LinkedList<Bid> sortedBidsList = SortBidsList(Bids, distList);
						ans = sortedBidsList.get((int) (sortedBidsList.size() / 2
								+ (int) (Math.random() * (Bids.size() - sortedBidsList.size() / 2))));
						// if(debug)
						// {
						// file = new PrintStream(new FileOutputStream(fileName,
						// true));
			...
			System.out.println(e.getMessage() + " Exception in method regeneratebid");
			ans = this.bid_maximum_utility;
		}
	...
	private HashMap<Bid, Double> UpdateBidsUtilityScore(HashMap<Bid, Double> bidsUtilityScore) {
		try {
			...
			for (; myBidIterator.hasNext();) {
				Bid bid = myBidIterator.next();
				bidsUtilityScore.put(bid, (this.utilitySpace.getUtility(bid) / this.MaximumUtility));
			}
		} catch (Exception e) {
			System.out.println("Exception in UpdateBidsUtilityScore: " + e.getMessage());
		}
	...
	private AgentData updateConcedeDegree_smallDF(AgentData agentData) {
		...
		try {
			if (agentData.agentNum.equals(this.string_agentData1Num)) {
				opponnetToughnessDegree = this.opponentBidHistory1.getConcessionDegree();
				// if(debug)
				// {
				// file = new PrintStream(new FileOutputStream(fileName, true));
				// System.setOut(file);
				...
				opponnetToughnessDegree = this.opponentBidHistory2.getConcessionDegree();
				// if(debug)
				// {
				// file = new PrintStream(new FileOutputStream(fileName, true));
				// System.setOut(file);
			...
			double temp = this.concedeTime_original
					+ weight * (1 - this.concedeTime_original) * Math.pow(opponnetToughnessDegree, gama);
			agentData = updateConcedeDegree_smallDF_UpdateConcedeTime(agentData, temp);
			// if(debug)
			// {
			// file = new PrintStream(new FileOutputStream(fileName, true));
			// System.setOut(file);
			...
			System.out.println(e.getMessage() + " Exception in method updateConcedeDegree_smallDF");
		}
		// System.out.println("concedeToDiscountingFactor is " +
	...
	private AgentData updateConcedeDegree_smallDF_UpdateConcedeTime(AgentData agentData, double newConcedeTime) {
		try {
			...
			System.out.println(e.getMessage() + " exception in method updateConcedeDegree_smallDF_UpdateConcedeTime");
		}
		// System.out.println("concedeToDiscountingFactor is " +
		// this.concedeToDiscountingFactor + "current time is " +
		// timeline.getTime() + "original concedetodiscoutingfactor is " +
	...
	private AgentData updateConcedeDegree_UpdateMinU(AgentData agentData, String DFname) {
		try {
			if (agentData.IsOppFirstBid) {
				...
				if (DFname.equals("Small")) {
					...
					if (this.domainSize > 100) {
						...
						agentData = calMinU(bidsUtil, agentData, relSumUtility, relCountUtility);
					} else {
						...
						if (relCountUtility > (bidsUtil.size() / 2)) {
							...
							for (int i = sortedBidsList.size() / 2; i < sortedBidsList.size(); i++) {
								double bidUtil = (Double) this.utilitySpace.getUtility(sortedBidsList.get(i));
								// if(debug)
								// {
								// file = new PrintStream(new
								// FileOutputStream(fileName, true));
							...
							for (Iterator f = bidsUtil.iterator(); f.hasNext();) {
								...
								if (bidUtil >= agentData.oppFirstBidUtility) {
									relSqRootOfAvgUtility += (bidUtil - relAvgUtility) * (bidUtil - relAvgUtility);
								}
							}
							agentData.relStdevUtility = Math.sqrt(relSqRootOfAvgUtility / relCountUtility);
					...
					// }
					agentData.minThreshold = CalWhetherOppFirstBidGoodEnough(agentData.minThreshold,
					...
					// agentData.minThreshold);
					// }
					agentData.minThreshold = Compare_MinThreshold_And_SecondMax(agentData.minThreshold,
			...
			System.out.println(e.getMessage() + " exception in method updateConcedeDegree_UpdateMinU");
		}
		// System.out.println("concedeToDiscountingFactor is " +
		// this.concedeToDiscountingFactor + "current time is " +
		// timeline.getTime() + "original concedetodiscoutingfactor is " +
	...
	private AgentData calMinU(LinkedList<Double> bidsUtil, AgentData agentData, double relSumUtility,
			double relCountUtility) {
		try {
			double relAvgUtility = 0;
			double relSqRootOfAvgUtility = 0;
			...
			relStdevUtility = Math.sqrt(relSqRootOfAvgUtility / relCountUtility);
			...
			if (relStdevUtility < 0.1) {
				...
				for (Iterator g = bidsUtil.iterator(); g.hasNext();) {
					...
					if (bidUtil >= agentData.oppFirstBidUtility && (bidUtil < (relAvgUtility - relStdevUtility)
							|| bidUtil > (relAvgUtility + relStdevUtility))) {
						relSqRootOfAvgUtility += (bidUtil - relAvgUtility) * (bidUtil - relAvgUtility);
						relCountUtilityInSmallSD++;
					}
				}
				relStdevUtility = Math.sqrt(relSqRootOfAvgUtility / relCountUtilityInSmallSD);
			}
			...
			if (relCountUtility < 51) {
				...
				// }
				agentData.minThreshold = relAvgUtility
						+ this.discountingFactor * relStdevUtility * this.reservationValue;
			}
		} catch (Exception e) {
	...
	private void updateConcedeDegree_largeDF(AgentData agentData) {
		try {
			...
			agentData.concedePartOfOppBehaviour = (((((agentData.relCountLowerBoundMid2
					/ agentData.relCountUpperBoundMid2) * agentData.slopeOfSlopeOfSessionMax2)
					- ((agentData.relCountLowerBoundMid1 / agentData.relCountUpperBoundMid1)
							* agentData.slopeOfSlopeOfSessionMax1))
					/ this.k) / agentData.relStdevUtility) - this.N;
			if (agentData.startConcede == true) {
				i = agentData.concedePartOfdiscountingFactor + agentData.concedePartOfOppBehaviour;
			...
			System.out.println(e.getMessage() + " exception in method updateConcedeDegree_largeDF");
		}
	}
	...
	private AgentData MeasureConcedePartOfOppBehaviour(AgentData agentData) {
		try {
			...
			if (agentData.count < this.FirstTimeInterval) {
				if (utilitySpace
						.getUtility(agentData.oppPreviousBid) > agentData.utility_maximum_from_opponent_Session1) {
				...
				if (utilitySpace
						.getUtility(agentData.oppPreviousBid) > agentData.utility_maximum_from_opponent_Session2) {
					agentData.utility_maximum_from_opponent_Session2 = utilitySpace
							.getUtility(agentData.oppPreviousBid);
				}
			} else if (agentData.count < this.ThirdTimeInterval) {
				if (utilitySpace
						.getUtility(agentData.oppPreviousBid) > agentData.utility_maximum_from_opponent_Session3) {
			...
			System.out.println(e.getMessage() + " exception in method MeasureConcedePartOfOppBehaviour");
		}
		return agentData;
	}
	...
	private AgentData updateConcedeDegree_nonDF(AgentData agentData) {
		try {
			...
			System.out.println(e.getMessage() + " exception in method updateConcedeDegree_nonDF");
		}
	...
	private LinkedList<Bid> SortBidsList(LinkedList<Bid> Bids) {
		...
		try {
			...
			List<Map.Entry<Bid, Double>> list_Data = new LinkedList<Map.Entry<Bid, Double>>(sortedBidsMap.entrySet());

			// Sorting
			Collections.sort(list_Data, new Comparator<Map.Entry<Bid, Double>>() {
				public int compare(Map.Entry<Bid, Double> entry1, Map.Entry<Bid, Double> entry2) {
			...
			System.out.println(e.getMessage() + " exception in method SortBidsList");
		}

		return sortedBidsList;
	}

	private LinkedList<Bid> SortBidsList(LinkedList<Bid> Bids, LinkedList<Double> distList) {
		LinkedList<Bid> sortedBidsList = new LinkedList<Bid>();
		try {
			HashMap<Bid, Double> sortedBidsMap = new HashMap<Bid, Double>();
			...
			System.out.println(e.getMessage() + " exception in method SortBidsList (Distance)");
		}

		return sortedBidsList;
	}

	private double CalWhetherOppFirstBidGoodEnough(double minThreshold, double oppFirstBidUtility) {
		try {
			LinkedList<Bid> Bids = new LinkedList<Bid>();
			BidIterator myBidIterator = new BidIterator(this.utilitySpace.getDomain());
			for (; myBidIterator.hasNext();) {
			...
			double relCountUtility = 0;
			for (int i = sortedBidsList.size() / 2; i < sortedBidsList.size(); i++) {
				relSumUtility += this.utilitySpace.getUtility(sortedBidsList.get(i));
				relCountUtility += 1;
				// if(debug)
				// {
				// file = new PrintStream(new FileOutputStream(fileName, true));
			...
			System.out.println(e.getMessage() + " exception in method IsOppFirstBidGoodEnough");
		}

		return minThreshold;
	}

	private double Compare_MinThreshold_And_SecondMax(double minThreshold, double utility_SecondMaximum) {
		try {
			if (minThreshold > utility_SecondMaximum) {
				return (utility_SecondMaximum * 0.9);
			}
		} catch (Exception e) {
			System.out.println(e.getMessage() + "exception in method Compare_MinThreshold_And_SecondMax");
		}
		...
		try {
			...
			for (; myBidIterator.hasNext();) {
				try {
					...
					System.out.println(e.getMessage() + "exception in method PrintAllBids");
				}
			}
	...
	private void printBidAllValues(Bid bid) {
		try {
			...
			System.out.println("Exception in printAllBidValues(): " + e.getMessage());
		}
	}

	private double CompareAdaptiveMinUAndNormalMinU(double adaptiveMinU, double normalMinU, double timeControl) {
	...
	private Bid BidToOffer_original() {
		...
		try {
			...
			this.adaptiveMinUThreshold = this.offeredBidsAvg - this.offeredBidsSD * (1 - this.discountingFactor);
			// if(debug)
			// {
			// file = new PrintStream(new FileOutputStream(fileName, true));
			// System.setOut(file);
			...
			if (currentTIme <= this.avgConcedeTime) {
				if (this.discountingFactor <= 0.5) {
					this.agentData1.minThreshold = (this.MaximumUtility * this.discountingFactor)
							/ Math.pow(this.discountingFactor, this.agentData1.concedeTime);
					this.agentData2.minThreshold = (this.MaximumUtility * this.discountingFactor)
							/ Math.pow(this.discountingFactor, this.agentData2.concedeTime);

					double minThresholdAgent1 = CompareAdaptiveMinUAndNormalMinU(this.adaptiveMinUThreshold,
							this.agentData1.minThreshold, timeControlAdaptiveMinU);
					double minThresholdAgent2 = CompareAdaptiveMinUAndNormalMinU(this.adaptiveMinUThreshold,
							this.agentData2.minThreshold, timeControlAdaptiveMinU);

					this.finalMinUAgent1 = minThresholdAgent1;
					this.finalMinUAgent2 = minThresholdAgent2;
					this.agentData1.utilitythreshold = this.MaximumUtility - (this.MaximumUtility - minThresholdAgent1)
							* Math.pow((currentTIme / this.agentData1.concedeTime), alpha1);
					this.agentData2.utilitythreshold = this.MaximumUtility - (this.MaximumUtility - minThresholdAgent2)
							* Math.pow((currentTIme / this.agentData2.concedeTime), alpha1);
				} else {
					double minThresholdAgent1 = CompareAdaptiveMinUAndNormalMinU(this.adaptiveMinUThreshold,
							this.agentData1.minThreshold, timeControlAdaptiveMinU);
					double minThresholdAgent2 = CompareAdaptiveMinUAndNormalMinU(this.adaptiveMinUThreshold,
					...
					this.agentData1.utilitythreshold = minThresholdAgent1 + (this.MaximumUtility - minThresholdAgent1)
							* (1 - Math.sin((Math.PI / 2) * (currentTIme / this.agentData1.concedeTime)));
					this.agentData2.utilitythreshold = minThresholdAgent2 + (this.MaximumUtility - minThresholdAgent2)
							* (1 - Math.sin((Math.PI / 2) * (currentTIme / this.agentData2.concedeTime)));
				}
			} else {
				if (this.discountingFactor <= 0.5) {
					...
					this.agentData1.utilitythreshold = CompareAdaptiveMinUAndNormalMinU(this.adaptiveMinUThreshold,
							this.agentData1.minThreshold, 1);
					this.agentData2.utilitythreshold = CompareAdaptiveMinUAndNormalMinU(this.adaptiveMinUThreshold,
					...
					this.agentData1.utilitythreshold = minThresholdAgent1
							+ (this.MaximumUtility - minThresholdAgent1) / (1 - this.agentData1.concedeTime)
									* Math.pow((currentTIme - this.agentData1.concedeTime), this.discountingFactor);
					this.agentData2.utilitythreshold = minThresholdAgent2
							+ (this.MaximumUtility - minThresholdAgent2) / (1 - this.agentData2.concedeTime)
									* Math.pow((currentTIme - this.agentData2.concedeTime), this.discountingFactor);
				}
			}
			...
			System.out.println(e.getMessage() + " exception in method BidToOffer_original");
	...
	private boolean OtherAcceptCondition(Bid oppPreviousBid, boolean IsAccept) {
		try {
			...
			System.out.println(e.getMessage() + " exception in method OtherAcceptCondition");
			return IsAccept;
		}
		return IsAccept;
	}
	...
	private void MinAcceptCondition(Bid bidReturned) {
		try {
			...
			System.out.println(e.getMessage() + " exception in method MinAcceptCondition");
		}
	}

	@Override
	public String getDescription() {
		return "ANAC2016 NGent";
	}
}
