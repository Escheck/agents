...
public class AgentQuest extends Agent {
		...
		try {
			...
			if (this.partnerAction instanceof Offer) {
				...
				if (this.opponentLastBid == null) {
					...
					opponentUtilityInWindowSofar = this.myUtilitySpace.getUtilityWithDiscount(this.opponentLastBid,
							time);
				}
				...
				// Maintain best Bid:
				if (this.myUtilitySpace.getUtilityWithDiscount(partnerBid, time) > this.myUtilitySpace
		...
		if (this.opponentBidHistory.size() < this.MAX_BIDS_TO_STORE) {
			try {
				this.opponentBidHistory
						.add(new BidDetails(partnerBid, this.myUtilitySpace.getUtilityWithDiscount(partnerBid, time)));
			} catch (Exception e) {
				e.printStackTrace();
			}
		} else {
	...
	private void addBids(double time) {
		for (int i = 0; i < this.MAX_BIDS_TO_SAMPLE; i++) {
			...
			try {
				this.myBidHistory
						.add(new BidDetails(randomBid, this.myUtilitySpace.getUtilityWithDiscount(randomBid, time)));
			} catch (Exception e) {
				e.printStackTrace();
			}
		}
	...
	private double computeEuclideanDistancePerBid(BidDetails opponentBidDetails, BidDetails myBidDetails, double time)
			throws Exception {
	...
	private Offer genearteCounterOffer(Offer opponentOffer, double time) {
		...
		if (time > 0.0d) {
			...
			if (this.myUtilitySpace.getUtilityWithDiscount(this.myLastBid, time) > this.myUtilitySpace
					.getUtilityWithDiscount(this.myBidHistory.getBestBidDetails().getBid(), time)) {
				counterOffer = new Offer(getAgentID(), this.myLastBid);
			} else {
				BidHistory tempBidHistory = this.myBidHistory.filterBetweenUtility(
						this.myUtilitySpace.getUtilityWithDiscount(this.myLastBid, time), this.myUtilitySpace
								.getUtilityWithDiscount(this.myBidHistory.getBestBidDetails().getBid(), time));
				if (tempBidHistory != null && tempBidHistory.size() > 0) {
					...
					if (tempBidHistory.getMedianUtilityBid() != null) {
						...
						if (!this.myUtilitySpace.isDiscounted()) {
							...
							BidHistory anotherTempBidHistory = tempBidHistory.filterBetweenUtility(
									this.myUtilitySpace.getUtilityWithDiscount(
											tempBidHistory.getMedianUtilityBid().getBid(), time),
									this.myUtilitySpace
											.getUtilityWithDiscount(tempBidHistory.getBestBidDetails().getBid(), time));
			...
			try {
				...
				concessionProbability = Pconcede(
						this.myUtilitySpace.getUtilityWithDiscount(opponentOffer.getBid(), time),
						this.timeline.getTime());

				if ((this.hardlineCounter < this.alpha)
						&& (concessionProbability.compareTo(new BigDecimal(this.beta)) > 0)) {
					...
					if (bidUtility > this.myUtilitySpace.getUtilityWithDiscount(this.bestBid, time)
							&& bidUtility >= this.getMyReservationValue(time)) {
						counterOffer = new Offer(getAgentID(), tempBid);
						this.myLastBid = tempBid;
					} else if (this.myUtilitySpace.getUtilityWithDiscount(this.bestBid, time) >= this
					...
					{
						if (this.myUtilitySpace.getUtilityWithDiscount(this.bestBid, time) < this.myUtilitySpace
		...
		if (opponentOffer != null) {
			try {
				...
				if (opponentOfferUtility >= this.getMyReservationValue(time)
					...
					result = true;
				} else if ((opponentOfferUtility >= myOfferUtility) && (opponentOfferUtility >= 0.85d)
						&& time > 0.95d) {
					result = true;
				}
	...
	public String getDescription() {
		return "ANAC 2014 - AgentQuest (compatible with non-linear utility spaces)";
	}
