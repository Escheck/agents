...
public class JonnyBlack extends AbstractNegotiationParty {
	...
	public Action chooseAction(List<Class<? extends Action>> validActions) {
		...
		round++;
		if (round % 10 == 0) {
			this.agreeVal = Functions.calcStopVal(parties, topNofOpp, (AdditiveUtilitySpace) utilitySpace);
		...
		double d = 0;
		if (lastBid != null)
			d = Functions.getBidValue((AdditiveUtilitySpace) utilitySpace, lastBid);
	...
	public void receiveMessage(AgentID sender, Action action) {
		...
		if (sender != null) {
			...
			if (!parties.contains(p)) {
				parties.add(p);
				p.setOrderedBids(this.acceptableBids, (AdditiveUtilitySpace) utilitySpace);
			} else {
				p = parties.get(parties.indexOf(p));
			}
			if (lastBid != null && !action.equals(new Accept(getPartyId(), lastBid))) {
				List<Issue> issues = lastBid.getIssues();
				try {
					for (int i = 0; i < issues.size(); i++) {
						IssueDiscrete id = (IssueDiscrete) ((AdditiveUtilitySpace) utilitySpace).getIssue(i);
	...
	public Bid createBid() {
		...
		if (parties.size() > 0)
			for (int i = lastorder + 1; i < acceptableBids.size(); i++) {
				...
				if (bh.v > stopValue() && parties.get(agentToFavor).getPredictedUtility(bh.b,
	...
	public Vector<BidHolder> recurseBids(Bid b, Vector<BidHolder> v, int is) {
		...
		if (is == issueOrder.length) {
			...
			bh.b = b;
			bh.v = Functions.getBidValue((AdditiveUtilitySpace) utilitySpace, b);
			v1.addElement(bh);
