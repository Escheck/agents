...
public class Mercury extends AbstractNegotiationParty {
	...
	public void init(NegotiationInfo info) {
		...
		try {
			...
			this.utilitythreshold = utilitySpace.getUtility(bid_maximum_utility); // initial
																					// utility
																					// threshold
	...
	public void receiveMessage(AgentID sender, Action opponentAction) {
		...
		if (numberOfParty == -1) {
			...
			opponentBidHistory.initiPartyRec(numberOfParty, (AdditiveUtilitySpace) this.utilitySpace);
			...
			this.opponentBidHistory.updateOpponentModel(((Offer) ActionOfOpponent).getBid(), sender,
					utilitySpace.getDomain(), (AdditiveUtilitySpace) this.utilitySpace);
			...
			if ((myParOrder.indexOf(sender) + 1) % numberOfParty == myParOrder.indexOf(myparty)
					|| (myParOrder.indexOf(sender) - 1) % numberOfParty == myParOrder.indexOf(myparty)) {

				halfSucc = true; // true
				System.out.println("Half success action caused by " + sender + "," + opponentAction.toString());

				this.opponentBidHistory.updateOpponentModel(ownBidHistory.getLastBid(), sender,
						utilitySpace.getDomain(), (AdditiveUtilitySpace) this.utilitySpace);
				...
				this.opponentBidHistory.updateOpponentModel(opponentBidHistory.getLastOppBid(), sender,
						utilitySpace.getDomain(), (AdditiveUtilitySpace) this.utilitySpace);
	...
	public Action chooseAction(List<Class<? extends Action>> validActions) {
			...
			if (!validActions.contains(Accept.class)) {
				...
				if (myRounds <= 30) {
					...
					if (timeline.getTime() < 0.97) {// still have some rounds
						...
						if (IsAccept && !IsTerminate) {
							action = new Accept(getPartyId(), this.opponentBidHistory.getLastOppBid());
							...
							if (this.utilitySpace.getUtility(bid) < 0.75) {
								...
								if (timeline.getTime() > 0.9999) {
									...
									if (this.utilitySpace.getUtility(opponentBidHistory.getMiniBestOpp()) > 0.5) {
										bid = opponentBidHistory.getMiniBestOpp(); // more
																					// concessive
									} else if (this.utilitySpace
											.getUtility(opponentBidHistory.getBestBidInHistory()) > 0.5) {
										bid = opponentBidHistory.getBestBidInHistory();
									} else {
				...
				try {
					...
					if (this.utilitythreshold * acceConThres <= this.utilitySpace
						...
						System.out.println("accept an offer after seeing an acceptance action with a not bad result"
		...
		if (timeline.getCurrentTime() > timeline.getTotalTime() * 1.1) {
			System.out.println("exception in negotiation time for Ares!" + timeline.getCurrentTime() + ","
		...
		try {
			...
			if (this.discountingFactor == 1 && this.maximumOfBid > 3200) {
				...
				minimumOfBid = this.MaximumUtility * 0.94
						+ 0.06 * this.MaximumUtility * (1 - Math.pow(timeline.getTime(), 3));
				...
				if (timeline.getTime() <= this.concedeToDiscountingFactor) {
					...
					this.utilitythreshold = maximumOfBid - (maximumOfBid - minThreshold)
							* Math.pow((timeline.getTime() / this.concedeToDiscountingFactor), alpha1);
				} else {
	...
	private boolean AcceptOpponentOffer(Bid opponentBid, Bid ownBid) {
		...
		if (currentUtility >= this.utilitythreshold || currentUtility >= nextRoundUtility) {
			...
			double currentMaximumUtility = this.utilitySpace
					.getUtilityWithDiscount(opponentBidHistory.getBestBidInHistory(), timeline);
			if (currentMaximumUtility > predictMaximumUtility && timeline.getTime() > this.concedeToDiscountingFactor) {
				try {
					...
					if (utilitySpace.getUtility(
							opponentBid) >= utilitySpace.getUtility(opponentBidHistory.getBestBidInHistory()) - 0.01) {
						return true;
					} else {
