...
public class WhaleAgent extends Agent {
	...
	public void endSession(NegotiationResult result) {
		...
		WhaleSessionData sessionData = new WhaleSessionData(result.getLastBid(), getBestBidOpponent(),
				result.getMyDiscountedUtility(), result.isAgreement(), bidLists, sumUtil, actionType);
	...
	public Action actionType1() {
		...
		try {
			...
			if (timeline.getTime() > 0.995) {
				...
				if (util > utilitySpace.getReservationValueWithDiscount(timeline)) { // TODO:Resevation
																						// Value‚Æ”ä‚×‚é•K—v‚ ‚è
					return new Accept(getAgentID(), opponentLastBid);
				}
				// Žc‚èŽžŠÔ‚ª�­‚È‚¢Žž
	...
	public Bid getNeighborBid(Bid bid) {
		try {
			...
			for (int i = 0; i < bid.getIssues().size(); i++) {
				...
				Value valueInteger = new ValueInteger(issueValueInt + flag * step);// change
																					// value
	...
	public String getDescription() {
		return "ANAC 2014 - WhaleAgent (compatible with non-linear utility spaces)";
	}
...
class WhaleSessionData implements Serializable {
	...
	public WhaleSessionData(Bid accesptedBid, Bid offeredBid, double getUtil, boolean isAgreement,
			ArrayList<Bid> bidLists, double sumUtil, int actionType) {
