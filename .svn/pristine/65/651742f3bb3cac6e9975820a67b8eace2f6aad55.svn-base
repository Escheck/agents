...
public class AgentLG extends Agent {
			...
			if (myLastBid == null || oponnetLastBid == null) {
				...
				// most optimal bids
					...
					if (time >= 0.9995) {
						...
						if (utilitySpace.getUtilityWithDiscount(myLastBid, time) < utilitySpace
	...
	public void init() {
		...
		bidChooser = new BidChooser((AdditiveUtilitySpace) this.utilitySpace, getAgentID(), oppenentsBid);
	}
